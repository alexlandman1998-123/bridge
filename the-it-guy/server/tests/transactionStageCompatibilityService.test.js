...
assert.equal(mapParentStageToLegacyStage('SETUP', 'DEP'), 'DEP')
assert.equal(mapParentStageToDetailedStage('TRANSFER'), 'Transfer in Progress')
assert.equal(mapParentStageToDetailedStage('REGISTRATION'), 'Transfer Lodged')
assert.equal(mapParentStageToDetailedStage('COMPLETE'), 'Registered')
assert.equal(mapParentStageToDetailedStage('CANCELLED', 'Finance Pending'), 'Finance Pending')
assert.equal(mapParentStageToDetailedStage('CANCELLED', 'Offer Accepted'), 'Offer Accepted')
assert.equal(mapParentStageToDetailedStage('CANCELLED', 'Finance In Progress'), 'Finance')

const setupPayload = buildTransactionCompatibilityPayload(
  { id: 'tx-setup', stage: 'Reserved', current_main_stage: 'AVAIL' },
  { parentStage: 'SETUP' },
)
assert.equal(setupPayload.is_active, true)
assert.equal(setupPayload.stage, 'Reserved')

const agentSetupPayload = buildTransactionCompatibilityPayload(
  { id: 'tx-agent-setup', stage: 'Offer Accepted', current_main_stage: 'DEP' },
  { parentStage: 'SETUP' },
)
assert.equal(agentSetupPayload.stage, 'Offer Accepted')
assert.equal(agentSetupPayload.current_main_stage, 'DEP')

const cancelledPayload = buildTransactionCompatibilityPayload(
  { id: 'tx-cancelled', stage: 'Reserved', current_main_stage: 'AVAIL' },
  { parentStage: 'CANCELLED' },
)
assert.equal(cancelledPayload.is_active, false)
