...
assert.match(pageSource, /buildSellerPortalSecurityOverview\(/)
assert.match(pageSource, /buildSellerEmailDeliveryOverview\(/)
assert.match(pageSource, /resolveSellerLastContact\(\[/)
assert.equal(overview.includes('No recent contact'), false, 'The Overview must distinguish no contact evidence from stale listing updates.')
assert.match(pageSource, /No recorded seller contact/)
assert.match(overview, /Activate Portal/)
assert.match(overview, /Review FICA/)
assert.match(overview, /sellerPortalSecurityOverview\.failures24h \?\? '—'/)
assert.match(overview, /sellerEmailDeliveryOverview\.hasEvidence/)
