...
function writeReport(report, outputArg) {
  const output = outputArg || path.join(appRoot, 'outputs', 'private-property-controlled-publish-rehearsal.json')
...
async function run() {
  ...
  if (missing.length) {
    const report = {
      ...
      nextStep: 'Add the missing values, then re-run the controlled publish rehearsal.',
    }
  ...
  const report = await runPrivatePropertyControlledPublishRehearsal({
    client,
    listingId: options.listingId,
    environment: options.environment,
    secrets: env,
