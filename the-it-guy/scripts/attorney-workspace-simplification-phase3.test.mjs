...
assert.match(overview, /flex flex-col !border-emerald-200 !bg-emerald-50 p-5/)
assert.match(overview, /h-\[168px\] space-y-2 overflow-y-auto overscroll-contain/)
assert.match(overview, /\['buyer', 'seller', 'transfer_attorney', 'agency'\]/)
assert.match(overview, /Key parties<\/h2>/)
assert.match(overview, /sm:grid-cols-2 xl:grid-cols-4/)
assert.match(overview, /onOpenWorkspace\?\.\('stakeholders'\)\}>View all parties/)
assert.match(overview, /agencyDetail \|\| \(contact !== name/)
...
assert.doesNotMatch(source, /<AttorneyDealSetupHandoffPanel/)
