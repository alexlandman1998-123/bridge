...
function buildFinance(financeType) {
  ...
  return {
    ...
    credit_check_consent: normalized === 'cash' ? '' : 'yes',
    ...
    buyer_banks: normalized === 'cash' ? [] : ['fnb'],
    bond_preapproval_completed: normalized === 'cash' ? '' : 'yes',
    bond_preapproval_document_available: normalized === 'cash' ? '' : 'yes',
...
const scenarios = [
  ...
  buildScenario(2, {
    ...
    purchaserEntityType: 'individual',
    financeType: 'bond',
    expectedReviewText: ['Review & Submit', 'Bond amount', 'Banks', 'Pre-approval', 'Credit consent'],
  }),
  buildScenario(3, {
