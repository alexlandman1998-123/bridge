...
function selectLiveOtpTemplate(templates = []) {
...
async function readSnapshot() {
  ...
  assert.ok(serviceRoleKey, 'SUPABASE_SERVICE_ROLE_KEY is required for the read-only OTP baseline concept audit.')
  ...
  const { data: templates, error: templatesError } = await client
    ...
    .eq('packet_type', 'otp')
    .eq('template_key', 'otp_default_v1')
    .order('updated_at', { ascending: false })
  assert.ifError(templatesError)

  const template = selectLiveOtpTemplate(templates || [])
  assert.ok(template?.id, 'No global otp_default_v1 template was found.')
...
const outputPath = arg('out', 'docs/otp-template-vnext-phase1-baseline-concept-audit.md')
const jsonPath = arg('json-out', '')
const snapshot = await readSnapshot()
const audit = buildOtpTemplateBaselineConceptAudit(snapshot)
...
if (flag('json')) {
  ...
  console.log(JSON.stringify({
    ...
    coveredConceptCount: audit.conceptAudit.coveredCount,
    partialConceptCount: audit.conceptAudit.partialCount,
    missingConceptCount: audit.conceptAudit.missingCount,
