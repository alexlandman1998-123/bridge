...
assert.match(files.buyerApi, /getOrganisationOnboardingBrandingSources[\s\S]*agencyOnboarding\.branding[\s\S]*settingsJson\.branding[\s\S]*settingsJson/)
assert.match(files.sellerService, /const settingsBranding[\s\S]*resolveOnboardingBranding\(\s*branding,\s*settingsBranding,\s*settings/)
assert.doesNotMatch(files.sellerService, /primaryColour:\s*pickFirstText\(resolvedBranding\.primaryColour,\s*'#274C69'\)/, 'seller branding snapshots must not treat Arch9 defaults as resolved CI colours')
assert.doesNotMatch(files.sellerService, /secondaryColour:\s*pickFirstText\(resolvedBranding\.secondaryColour,\s*'#10273A'\)/, 'seller branding snapshots must not treat Arch9 defaults as resolved CI colours')
...
assert.match(files.sellerService, /resolveSellerOnboardingBrandingSnapshot\(client, normalizedToken, portalPayload\.listing\)/, 'seller onboarding portal payload should attach organisation settings branding')
