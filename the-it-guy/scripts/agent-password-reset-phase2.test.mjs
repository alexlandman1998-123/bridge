...
assert.match(authPage, /function isEmailNotConfirmedError/, 'Auth should detect Supabase email-not-confirmed login failures')
assert.match(authPage, /Seller Portal passwords only open the secure seller portal link/, 'Auth should explain seller portal passwords are separate from platform login')
assert.match(authPage, /setPendingVerificationEmail\(email\.trim\(\)\)[\s\S]*setError\(getEmailNotConfirmedMessage\(\)\)/, 'Email-not-confirmed login failures should expose the resend verification action')
