...
const [viteConfig, vercelConfig, appSource, packageJson, containmentGuard] = await Promise.all([
...
assert.match(containmentGuard, /Release identifier.*does not match checked-out commit/i, 'Release verifier must bind the public ID to Git HEAD.')
