...
try {
  ...
  const guidance = html.match(/<details[^>]*><summary[^>]*><span>Task guidance[\s\S]*?<\/details>/)?.[0]
  assert.ok(guidance, 'task guidance is an accessible disclosure')
  assert.doesNotMatch(guidance.slice(0, guidance.indexOf('>')), /\bopen\b/)
  assert.ok(guidance.includes(model.taskDescription))
  assert.ok(guidance.includes(model.completionMessage))
  assert.match(html, /Review documents/)
  assert.match(html, /Start task/)
  assert.match(html, /Complete task/)
  assert.match(render({ outcomeReason: 'Reviewed off-platform', applicabilitySuggestion: 'Check buyer entity type' }), /Reviewed off-platform/)
  ...
  console.log('Work tab simplification: rendering, collapsed guidance, labels and action gates passed')
} finally {
  await server.close()
}
