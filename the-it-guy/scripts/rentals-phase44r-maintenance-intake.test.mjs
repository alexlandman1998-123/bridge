...
for (const value of ['New request', 'Capture staff-reported work against a tenancy.', 'Maintenance queue', 'Acknowledge']) assert.ok(page.includes(value), `missing maintenance UI ${value}`)
assert.ok(app.includes('/agent/rentals/maintenance'), 'maintenance route is missing')
assert.ok(dashboard.includes("item.kind.includes('maintenance')"), 'dashboard does not link maintenance attention items')
