...
const run = spawnSync(process.execPath, ['scripts/legal-document-phase-o2-soak-gate.mjs'], { cwd: process.cwd(), env: process.env, encoding: 'utf8', timeout: 300_000, maxBuffer: 20 * 1024 * 1024 })
let o2
try { o2 = JSON.parse(run.stdout) } catch { o2 = { status: 'UNAVAILABLE', ready: false, mutatedData: false } }
...
if (o2.status === 'READY_FOR_O3' && record?.status === 'continued') {
  try {
    ...
    if (!url || !process.env.SUPABASE_SERVICE_ROLE_KEY) throw new Error('Supabase URL and service role key are required for O3.')
...
const assessment = assessLegalDocumentCohortExpansionProposal({ o2, record, pilot, candidates, storeAvailable })
console.log(JSON.stringify({
  phase: 'O3', status: assessment.status, ready: assessment.ready,
  ...
  evidence: { o2Status: o2.status || 'UNAVAILABLE', continuationState: continuationState.status || 'UNAVAILABLE', storeAvailable, storeError },
