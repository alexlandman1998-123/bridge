...
assertIncludes('kingstons-seller-pack-documents', 'Documents tab must expose the Kingston Seller Pack upload panel.')
assertIncludes('handleKingstonsSellerPackUpload', 'Seller Pack uploads must have a dedicated upload handler.')
assertIncludes('handleKingstonsSellerPackSellerTypeChange', 'FICA seller type must be captured before the portal workflow.')
...
assertIncludes("['Upload Seller Pack', Upload, () => setLeadWorkspaceTab('documents')]", 'Kingstons quick actions must prefer manual Seller Pack upload over generated mandate/PDF actions.')
