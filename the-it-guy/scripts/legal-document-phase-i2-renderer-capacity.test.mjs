...
for (const file of ['../supabase/functions/generate-otp/index.ts', '../supabase/functions/generate-mandate/index.ts']) {
  const source = fs.readFileSync(file, 'utf8')
  assert.match(source, /RENDERER_CONTRACT = "i2-v1"/)
  assert.match(source, /capacityProbe && bearer !== SUPABASE_SERVICE_ROLE_KEY/)
  assert.match(source, /RENDER_CAPACITY_FORBIDDEN/)
  assert.match(source, /mutatedData: false/)
  const probeExit = source.lastIndexOf('if (capacityProbe)')
  const upload = source.indexOf('.upload(', probeExit)
  assert.ok(probeExit > 0 && upload > probeExit, `${file} must return from capacity mode before upload`)
}
