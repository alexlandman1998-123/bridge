...
await test('seller portal invite trigger is anchored after mandate finalization', () => {
  ...
  assert.match(signerSigningAction, /async function resolveSellerPortalInviteListing/)
  assert.match(signerSigningAction, /\.eq\("mandate_packet_id", packetId\)/)
  assert.match(signerSigningAction, /\.from\("leads"\)[\s\S]*\.select\("lead_id, listing_id, mandate_packet_id"\)[\s\S]*\.eq\("mandate_packet_id", packetId\)/)
  assert.match(signerSigningAction, /for \(let attempt = 0; attempt < 3; attempt \+= 1\)[\s\S]*await delay\(750 \* \(attempt \+ 1\)\)/)
...
await test('seller portal invite phase 6 accepts linked signed mandate packet evidence', () => {
  ...
  assert.match(sellerOnboardingEmailHandler, /\.select\(\s*"id, organisation_id, mandate_status, listing_status, mandate_packet_id",?\s*\)/)
  assert.doesNotMatch(sellerOnboardingEmailHandler, /mandate_status, listing_status, status, mandate_packet_id/)
...
await test('seller portal invite includes post-mandate document checklist in branded email payload', () => {
  assert.match(signerSigningAction, /SELLER_POST_MANDATE_OUTSTANDING_REQUIREMENT_STATUSES/)
  assert.match(signerSigningAction, /async function resolveSellerPortalRequiredDocuments/)
  assert.match(signerSigningAction, /"private_listing_document_requirements"/)
  assert.match(signerSigningAction, /"private_listing_documents"/)
  assert.match(signerSigningAction, /if \(key === "signed_mandate"\) return false/)
  assert.match(signerSigningAction, /const requiredDocuments = await resolveSellerPortalRequiredDocuments/)
  assert.match(signerSigningAction, /buildSellerPortalInviteEmailPayload\(\{[\s\S]*requiredDocuments,/)
  assert.match(signerSigningAction, /requiredDocuments,\s*outstandingDocumentCount: requiredDocuments\.length/)
  assert.match(signerSigningAction, /documentPackRequirementKeys: requiredDocuments\.map/)
  assert.match(sellerOnboardingEmailHandler, /buildSellerOnboardingEmailHtml\(\{[\s\S]*requiredDocuments/)
})
