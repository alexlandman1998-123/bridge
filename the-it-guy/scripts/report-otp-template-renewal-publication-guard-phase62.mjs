...
const checkedAt = process.env.OTP_TEMPLATE_RENEWAL_PUBLICATION_GUARD_REPORT_TIME || new Date().toISOString()
...
const report = buildOtpTemplateRenewalPublicationGuardPhase62Audit({
  checkedAt,
  phase61Audit,
  packageJson,
})
const outputUrl = new URL('../docs/otp-template-renewal-publication-guard-phase62.md', import.meta.url)

await writeFile(outputUrl, formatOtpTemplateRenewalPublicationGuardPhase62Markdown(report), 'utf8')
