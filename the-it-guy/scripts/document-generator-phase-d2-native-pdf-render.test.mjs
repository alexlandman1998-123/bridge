...
assert.match(edge, /renderStructuredSectionsToPdfBytes/)
assert.match(edge, /buildPdfConverterUrl\(\)[\s\S]*renderHtmlToPdfBytes[\s\S]*renderStructuredSectionsToPdfBytes/)
...
assert.match(service, /!templateUsesNativeRenderer\(preflightTemplate, normalizedPacketType\)[\s\S]*requireDocumentConversionHealth\(\)/)
assert.match(service, /templateUsesNativeRenderer\(effectiveTemplate, validation\.packetType\)[\s\S]*\? 'native_structured'/)
