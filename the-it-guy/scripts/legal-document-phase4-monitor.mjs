...
const [eventsResult, packetsResult] = await Promise.all([
  client.from('document_packet_events').select('id, event_type, created_at, packet_id').gte('created_at', since).in('event_type', ['generation_started', 'version_generated', 'generation_failed', 'final_signed_otp_generated', 'legal_template_approval_blocked']),
