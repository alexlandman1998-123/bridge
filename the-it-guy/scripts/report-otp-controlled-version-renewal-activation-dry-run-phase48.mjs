...
import { buildOtpCommercialTermsPersistencePhase24Audit } from '../src/core/documents/otpCommercialTermsPersistencePhase24.js'
import { buildOtpCommercialTermsReviewPhase25Audit } from '../src/core/documents/otpCommercialTermsReviewPhase25.js'
import { buildOtpCommercialTermsRuntimePhase26Audit } from '../src/core/documents/otpCommercialTermsRuntimePhase26.js'
import { buildOtpGeneratedPdfProofPhase27Audit } from '../src/core/documents/otpGeneratedPdfProofPhase27.js'
import { buildOtpMatterAttorneyQuotePortalPhase28Audit } from '../src/core/documents/otpMatterAttorneyQuotePortalPhase28.js'
import { buildOtpFinalProductionReadinessGatePhase29Audit } from '../src/core/documents/otpFinalProductionReadinessGatePhase29.js'
import { buildOtpAgentControlledEditsPhase30Audit } from '../src/core/documents/otpAgentControlledEditsPhase30.js'
...
const checkedAt = process.env.OTP_CONTROLLED_VERSION_RENEWAL_ACTIVATION_DRY_RUN_REPORT_TIME || new Date().toISOString()
...
const phase24Audit = buildOtpCommercialTermsPersistencePhase24Audit({ checkedAt, migrationSql, serviceSource: persistenceServiceSource })
const phase25Audit = buildOtpCommercialTermsReviewPhase25Audit({ checkedAt, phase24Audit, reviewComponentSource })
const phase26Audit = buildOtpCommercialTermsRuntimePhase26Audit({ checkedAt, phase25Audit, serviceSource: runtimeServiceSource })
const renderEvidence = await renderOtpGeneratedPdfProofPhase27()
const phase27Audit = buildOtpGeneratedPdfProofPhase27Audit({ checkedAt, phase26Audit, renderEvidence })
const phase28Audit = buildOtpMatterAttorneyQuotePortalPhase28Audit({ checkedAt, phase27Audit, migrationSql, serviceSource: portalServiceSource })
const phase29Audit = buildOtpFinalProductionReadinessGatePhase29Audit({ checkedAt, phase27Audit, phase28Audit })
const phase30Audit = buildOtpAgentControlledEditsPhase30Audit({ checkedAt, phase29Audit })
const phase31Audit = buildOtpAgentReviewUiPhase31Audit({ checkedAt, phase30Audit, workspaceSource, packageJson })
...
const report = buildOtpControlledVersionRenewalActivationDryRunPhase48Audit({
  checkedAt,
  phase47Audit,
  packageJson,
})
const outputUrl = new URL('../docs/otp-controlled-version-renewal-activation-dry-run-phase48.md', import.meta.url)

await writeFile(outputUrl, formatOtpControlledVersionRenewalActivationDryRunPhase48Markdown(report), 'utf8')
