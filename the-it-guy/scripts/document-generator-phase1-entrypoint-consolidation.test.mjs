...
assert.match(handleSend, /const emailPayload = \{[\s\S]{0,500}type: 'otp_signing'[\s\S]{0,500}packetType: 'otp'/)
assert.match(handleSend, /invokeEdgeFunction\(useServerSendReady \? 'legal-document-job-runner' : 'send-mandate-signing-email'/)
assert.match(handleSend, /body: useServerSendReady[\s\S]*: emailPayload/)
...
assert.match(signingEmailSender, /RETIRED_SELLER_MANDATE_SIGNING_EMAIL_TYPES/)
assert.match(signingEmailSender, /RETIRED_SELLER_MANDATE_SIGNING_EMAIL_TYPES\.has\(type\)/)
assert.match(signingEmailSender, /seller_mandate_signing_links_retired/)
assert.match(signingEmailSender, /"otp_signing"/)
