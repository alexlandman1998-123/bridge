...
function parseArgs(argv) {
  ...
  for (const arg of argv) {
    ...
    } else if (arg.startsWith('--production-readiness-report=')) {
      options.productionReadinessReport = normalizePrivatePropertyText(arg.slice('--production-readiness-report='.length))
...
function writeReport(report, outputArg) {
  const output = outputArg || path.join(appRoot, 'outputs', 'private-property-go-live-closeout.json')
...
async function run() {
  ...
  if (missing.length) {
    const report = {
      ...
      nextStep: 'Add the missing values, then re-run the go-live closeout gate.',
    }
  ...
  const report = await runPrivatePropertyGoLiveCloseout({
    client,
    listingId: options.listingId,
    secrets: env,
    ...
    productionReadinessReport: readJsonFile(options.productionReadinessReport),
    evidence: {
  ...
  console.log(JSON.stringify({
    ...
    productionCommand: report.ready ? report.production.publishCommand : null,
