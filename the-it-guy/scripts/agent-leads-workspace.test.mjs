...
try {
  ...
  for (const actionId of ['edit_seller', 'assign_agent', 'open_journey', 'open_readiness', 'open_listing']) {
  ...
  assert.ok(workspaceSource.includes('selectedSellerJourneyCurrentRailStep'), 'seller journey should derive one current rail stage for actions')
  assert.ok(workspaceSource.includes('selectedSellerJourneyCanMarkCurrentStepComplete ? ('), 'seller journey mark-complete action should render from the container footer')
  assert.ok(workspaceSource.includes('stage={selectedSellerJourneyCurrentRailStep}'), 'seller journey mark-complete action should target the current stage from the footer')
  ...
  assert.ok(workspaceSource.includes('function SellerCommissionCard'), 'seller workspace should retain commission structure capture')
