...
try {
  ...
  assert.deepEqual(buyerTabKeys, [
    ...
    'tasks',
    'appointments',
  ], 'buyer lead workspace should expose five literal buyer journey tabs before the canonical onboarding / OTP constant tab')
  ...
  assert.ok(workspaceSource.includes('function LeadOfferReadinessPanel'), 'onboarding / OTP workspace should expose the offer readiness workflow')
  ...
  assert.ok(workspaceSource.includes('Send Offer Link'), 'onboarding / OTP workspace should expose the legacy offer-link action until the buyer portal flow is fully retired')
