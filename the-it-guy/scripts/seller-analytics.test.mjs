...
try {
  ...
  assert.equal(seller.overview.listingsCreated, 2)
  ...
  assert.equal(seller.overview.listingsAwaitingActivation, 1)
  assert.equal(seller.overview.blockedListings, 1)
  ...
  assert.equal(funnel.find((stage) => stage.key === 'listings_created').volume, 2)
  ...
  assert.equal(branch.listingsCreated, 2)
