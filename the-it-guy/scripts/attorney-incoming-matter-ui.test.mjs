...
assert.match(source, /Open \$\{matterLabel\}/, 'Incoming row action should open the attorney instruction using its matter label')
...
assert.match(source, /Accept \$\{matterLabel\}/, 'Incoming ready rows should expose matter acceptance')
...
assert.match(source, /Decline \$\{matterLabel\}/, 'Incoming row actions should expose matter decline')
