...
function parseArgs(argv) {
  ...
  for (const arg of argv) {
    if (arg === '--include-leads') {
      options.includeLeads = true
    } else if (arg === '--include-portal-checks') {
      options.includePortalChecks = true
    } else if (arg === '--include-statistics') {
      ...
      options.agentId = normalizeProperty24Text(arg.slice('--agent-id='.length))
    } else if (arg.startsWith('--from-date=')) {
      options.fromDate = normalizeProperty24Text(arg.slice('--from-date='.length))
...
function buildConfig(options) {
  ...
  const config = {
    ...
    agentId: normalizeProperty24Text(options.agentId || env.PROPERTY24_DEFAULT_AGENT_ID),
    fromDate: options.fromDate,
    after: options.after,
    includeLeads: options.includeLeads,
    includePortalChecks: options.includePortalChecks,
...
function writeReport(report, outputArg) {
  const output = outputArg || path.join(appRoot, 'outputs', 'property24-reconciliation.json')
...
async function run() {
  ...
  const report = await runProperty24ReconciliationJob({ supabase, property24, config })
  const output = writeReport(report, options.output)
  console.log(JSON.stringify({
    status: report.status,
    output,
    reconciliation: report.reconciliation.summary,
    updates: report.updates.summary,
    portalVisibility: report.portalVisibility?.summary || null,
    leads: report.leadImportPlan?.summary || null,
    statisticsIncluded: Boolean(report.statistics),
  }, null, 2))
