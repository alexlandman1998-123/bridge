...
async function run(script) {
  try {
    const { stdout } = await execFileAsync(process.execPath, [script], { cwd: process.cwd(), env: process.env, timeout: 1_100_000, maxBuffer: 30 * 1024 * 1024 })
...
const n2 = await run('scripts/legal-document-phase-n2-rollout-envelope.mjs')
let claimState
...
if (n2.status === 'READY_FOR_N3' && claim?.claimedAt) {
  try {
    const url = process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL || ''
    if (!url || !process.env.SUPABASE_SERVICE_ROLE_KEY) throw new Error('Supabase URL and service role key are required for N3.')
    ...
    const organisationIds = n2.rolloutEnvelope?.target?.organisationIds || []
    const packetsResult = await client.from('document_packets').select('id, organisation_id, packet_type, current_version_number, status, completed_at').in('organisation_id', organisationIds).eq('status', 'completed').gte('completed_at', claim.claimedAt).order('completed_at', { ascending: false }).limit(100)
...
const assessment = assessLegalDocumentCanaryAcceptance({ n2, claim, canaries, storeAvailable })
console.log(JSON.stringify({
  phase: 'N3', status: assessment.ready ? 'READY_FOR_N4' : 'NO_GO', ready: assessment.ready,
  blockerCount: assessment.blockers.length, blockers: assessment.blockers,
  acceptedCanaries: assessment.acceptedCanaries,
  evidence: { n2Status: n2.status || 'UNAVAILABLE', claimState: claimState.status || 'UNAVAILABLE', storeAvailable, storeError },
