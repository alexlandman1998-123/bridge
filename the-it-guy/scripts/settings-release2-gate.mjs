...
import { SETTINGS_RELEASE_1_VERSION } from '../src/lib/settingsRelease1.js'
import {
...
const config = JSON.parse(readFileSync(resolve(workspaceRoot, 'config/settings-release-2.json'), 'utf8'))
...
const requiredMigrationsPresent = SETTINGS_RELEASE_2_REQUIRED_MIGRATIONS.every((file) => migrationFiles.includes(file))
...
const release1Contract = run(process.execPath, [resolve(workspaceRoot, 'scripts/settings-release1-gate.test.mjs')])
const skipBuild = process.argv.includes('--skip-build')
const buildResult = skipBuild ? { pass: false, status: null, stdout: '', stderr: 'Build deliberately skipped.' } : run('npm', ['run', 'build'])

let liveEvidence = {}
const evidencePath = String(process.env.SETTINGS_RELEASE2_EVIDENCE || '').trim()
if (evidencePath) liveEvidence = JSON.parse(readFileSync(resolve(workspaceRoot, evidencePath), 'utf8'))
const release1PromotionVerified = liveEvidence?.release1?.status === 'GO'
  && liveEvidence?.release1?.version === SETTINGS_RELEASE_1_VERSION
  && Boolean(liveEvidence?.release1?.completedAt)

const report = buildSettingsRelease2Readiness({
  config,
  evidence: {
    uniqueMigrationVersions,
    requiredMigrationsPresent,
    phaseContractsPass: testResults.every((result) => result.pass),
    release1GatePass: release1Contract.pass,
    release1PromotionVerified,
...
console.log(JSON.stringify({
  ...
  evidence: {
    ...
    release1Contract: { pass: release1Contract.pass, status: release1Contract.status },
    release1PromotionVerified,
