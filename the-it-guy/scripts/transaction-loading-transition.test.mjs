...
assert.match(page, /const attorneyWorkflowIsLoading = workspaceRole === 'attorney'/)
assert.ok(page.includes("dataset !== 'workflow' && data && !data.__isShell"), 'Placeholder subprocesses must not suppress the actual workflow request')
assert.ok(page.includes("const attorneyWorkflowIsLoading = workspaceRole === 'attorney' && !data?.__workflowHydrated"), 'Background refresh must not unmount a hydrated workbench')
assert.match(page, /Loading attorney workflow/)
assert.match(page, /attorneyWorkflowIsLoading \? \(/)
assert.match(page, /workflow could not be loaded\. Retry to load the saved task progress\./)
