...
try {
  ...
  await page.getByRole('heading', { name: 'Alexander Landman' }).waitFor({ state: 'visible', timeout: 4_500 })
  await page.getByText('409 Queens Cres, Nr3, Menlo, Pretoria, Gauteng, 0081').first().waitFor({ state: 'visible', timeout: 4_500 })
  await page.getByText('Complete Listing').first().waitFor({ state: 'visible', timeout: 4_500 })
  await page.getByText('40%').first().waitFor({ state: 'visible', timeout: 4_500 })
  await page.getByText('Listing Created · Draft').first().waitFor({ state: 'visible', timeout: 4_500 })

  const hydratedMs = Date.now() - started
  assert.ok(hydratedMs < 6_000, `Lead workspace should hydrate from linked listing without the old 10s wait; took ${hydratedMs}ms.`)
  ...
  assert.equal(syncPatch.seller_onboarding_status, 'completed')
