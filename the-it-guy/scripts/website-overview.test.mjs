...
assert.ok(component.includes('BlogSidebarPanel label="Featured image"') && component.includes('Image uploaded. Add its alt text before saving the post.'), 'Featured-image selection should live in the sidebar and allow upload before its description is completed.')
assert.ok(component.includes('wlo-title-section') && component.includes('>Title</h2>'), 'The article title should have a clearly labelled editor container.')
assert.ok(styles.includes('.wlo-editor-block.paragraph textarea { min-height: 360px !important; }'), 'The article writing surface should give authors substantially more room for a paragraph.')
