...
const transactionalSeedMigration = read(resolve(repositoryRoot, 'supabase/migrations/20260906104500_public_websites_phase4_transactional_seed.sql'))
const indexMigration = read(resolve(repositoryRoot, 'supabase/migrations/20260906111000_public_websites_phase4_dark_launch_indexes.sql'))
...
assert.match(transactionalSeedMigration, /site\.status = 'draft'[\s\S]*set status = 'published'[\s\S]*published_revision_id = v_revision\.id/i, 'production publication is finalized inside one database transaction')
assert.match(indexMigration, /listing_id[\s\S]*website_site_id, organisation_id[\s\S]*dark_launch_id, organisation_id/i, 'dark-launch foreign keys have covering indexes')
