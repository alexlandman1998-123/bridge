...
assert.match(source, /signed_otp_upload/)
...
assert.match(source, /Buyer Onboarding \+ Signed OTP/)
assert.match(source, /Send Buyer Onboarding/)
assert.match(source, /Upload Signed OTP/)
assert.match(source, /Send onboarding and upload the already-signed OTP in parallel\./)
assert.match(source, /does not generate or facilitate the offer\./)
...
assert.doesNotMatch(source, />Request Changes</)
assert.doesNotMatch(source, />Send to Seller</)
