...
const packet = { id: '11111111-1111-4111-8111-111111111111', organisation_id: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', packet_type: 'custom', current_version_number: 2, status: 'signing_prep' }
...
assert.equal(assessSigningEnvelope({ packet, version, signers: [signer], fields: [field] }).ready, true)
assert.ok(assessSigningEnvelope({ packet: { ...packet, status: 'generated' }, version, signers: [signer], fields: [field] }).reasons.includes('E3_PACKET_NOT_READY_TO_SEND'))
assert.ok(assessSigningEnvelope({ packet, version: { ...version, version_number: 1 }, signers: [signer], fields: [field] }).reasons.includes('E3_VERSION_BINDING_INVALID'))
assert.ok(assessSigningEnvelope({ packet, version, signers: [{ ...signer, signer_email: 'pending+seller@bridge.local' }], fields: [field] }).reasons.includes('E3_SIGNER_EMAIL_INVALID'))
assert.ok(assessSigningEnvelope({ packet, version, signers: [signer], fields: [{ ...field, packet_version_id: '88888888-8888-4888-8888-888888888888' }] }).reasons.includes('E3_FIELD_VERSION_BINDING_INVALID'))
assert.ok(assessSigningEnvelope({ packet, version, signers: [signer], fields: [{ ...field, width: 0 }] }).reasons.includes('E3_FIELD_GEOMETRY_INVALID'))
assert.ok(assessSigningEnvelope({ packet, version, signers: [signer], fields: [{ ...field, required: false }] }).reasons.includes('E3_REQUIRED_SIGNATURE_FIELD_MISSING'))
assert.ok(assessSigningEnvelope({ packet: { ...packet, packet_type: 'mandate' }, version, signers: [signer], fields: [field] }).reasons.includes('E3_REQUIRED_SIGNER_MISSING'))
...
assert.doesNotMatch(api, /assertDraftLock\(\{ packet, version: targetVersion \}\)/)
