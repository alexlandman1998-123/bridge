...
assert.match(componentSource, /open=\{documentModalOpen\}/)
assert.match(componentSource, /cancellation_existing_bond_confirmed/)
assert.match(componentSource, /\['open_documents', 'upload_document', 'review_document'\]\.includes\(action\?\.id\)/)
assert.doesNotMatch(componentSource, /Current legal task|Attention required|Task guidance/)
