...
const update = async status => (await db.query('select bridge_update_attorney_workflow_step_v2($1,$2,$3,$4) as result', [matter, 'transfer', step, status])).rows[0].result
...
await db.close()
console.log('Atomic PostgreSQL tests passed: completion, reopening, missing rows, rollback, and unauthorised access.')
