...
const checkedAt = process.env.OTP_TEMPLATE_RENEWAL_ATTORNEY_RESPONSE_REPORT_TIME || new Date().toISOString()
...
const report = buildOtpTemplateRenewalAttorneyResponsePhase59Audit({
  checkedAt,
  phase58Audit,
  packageJson,
})
const outputUrl = new URL('../docs/otp-template-renewal-attorney-response-phase59.md', import.meta.url)

await writeFile(outputUrl, formatOtpTemplateRenewalAttorneyResponsePhase59Markdown(report), 'utf8')
