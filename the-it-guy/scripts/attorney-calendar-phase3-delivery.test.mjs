...
for (const [source, token, message] of [
  ...
  [notifications, "import { prepareNotificationOutbox } from './notificationOutboxService'", 'Failed appointment email delivery must have a durable outbox fallback.'],
  [notifications, 'enqueueAppointmentNotificationRetry({', 'Failed appointment emails must enqueue a retry event.'],
  [notifications, "status: 'queued'", 'Outbox retry must be surfaced as a queued email result.'],
  [notifications, "source: 'appointment_notifications'", 'Appointment retry outbox rows must be source tagged.'],
  ...
  [delivery, "queued: 'queued'", 'Queued appointment delivery must be a canonical status.'],
  [delivery, 'email queued with a calendar attachment', 'Queued appointment delivery must get accurate user feedback.'],
