...
test('direct listing operational summary allows creation but requires post-create compliance actions', () => {
  ...
  assert.ok(summary.followUpActions.some((action) => action.key === 'mandate' && action.complete === false && action.status === 'reported_held_pending_upload'))
  assert.ok(summary.followUpActions.some((action) => action.key === 'fica_form' && action.complete === false && action.status === 'reported_held_pending_upload'))
  ...
  assert.ok(summary.attentionItems.some((item) => item.includes('Mandate: reported received')))
...
test('AgentListingDetail renders the Phase 7 operational audit panel', () => {
  ...
  assert.match(agentListingDetailSource, /Manual intake handoff/)
  assert.match(agentListingDetailSource, /What still needs to happen/)
  assert.match(agentListingDetailSource, /Open document workspace/)
})
