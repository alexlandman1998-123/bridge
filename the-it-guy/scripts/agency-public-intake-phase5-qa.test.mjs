...
async function runSelectedListingJourney(page, baseUrl, state) {
  ...
  assert.equal(await page.getByLabel('Minimum budget').count(), 0, 'buyer intake should not expose manual minimum budget input')
  assert.equal(await page.getByLabel('Maximum budget').count(), 0, 'buyer intake should not expose manual maximum budget input')
  await page.getByRole('button', { name: 'R1.5m - R2m' }).click()
  ...
  assert.equal(state.listingRequests.at(-1)?.maxPrice, '2000000', 'listing API should receive budget max filter')
  ...
  assert.equal(payload.requirement.budgetMax, 2000000)
