...
alter table if exists organisation_users add column if not exists invitation_token text;
alter table if exists organisation_users add column if not exists invitation_expires_at timestamptz;
...
alter table if exists leads add column if not exists listing_id text;
...
alter table if exists leads drop constraint if exists leads_seller_onboarding_status_check;
alter table if exists leads
...
alter table if exists appointments add column if not exists appointment_date date;
alter table if exists appointments add column if not exists start_time time;
...
alter table if exists appointments add column if not exists client_feedback text;
alter table if exists appointments add column if not exists agent_notes text;
alter table if exists appointments add column if not exists next_step text;
alter table if exists appointments add column if not exists follow_up_date date;
alter table if exists appointments add column if not exists created_by uuid references profiles(id) on delete set null;
...
alter table if exists appointment_participants add column if not exists appointment_id uuid references appointments(appointment_id) on delete cascade;
alter table if exists appointment_participants add column if not exists organisation_id uuid references organisations(id) on delete cascade;
alter table if exists appointment_participants add column if not exists name text;
alter table if exists appointment_participants add column if not exists email text;
alter table if exists appointment_participants add column if not exists phone text;
alter table if exists appointment_participants add column if not exists participant_role text not null default 'Other Contact';
alter table if exists appointment_participants add column if not exists rsvp_status text not null default 'Pending';
alter table if exists appointment_participants add column if not exists proposed_new_time timestamptz;
alter table if exists appointment_participants add column if not exists responded_at timestamptz;
alter table if exists appointment_participants add column if not exists created_at timestamptz not null default now();
alter table if exists appointment_participants add column if not exists updated_at timestamptz not null default now();

alter table if exists appointment_participants drop constraint if exists appointment_participants_role_check;
alter table if exists appointment_participants
...
alter table if exists appointment_participants drop constraint if exists appointment_participants_rsvp_check;
alter table if exists appointment_participants
...
create table if not exists client_portal_contexts (
  ...
  client_email text,
  client_contact_id uuid references buyers(id) on delete set null,
  context_type text not null default 'buying',
  transaction_id uuid references transactions(id) on delete cascade,
  seller_lead_id uuid references leads(lead_id) on delete set null,
  listing_id text,
  mandate_packet_id uuid references document_packets(id) on delete set null,
...
create table if not exists client_seller_interest_requests (
  id uuid primary key default gen_random_uuid(),
  organisation_id uuid references organisations(id) on delete cascade,
  client_email text,
  client_contact_id uuid references buyers(id) on delete set null,
...
create unique index if not exists client_portal_links_active_transaction_idx
  ...
  where is_active;
create index if not exists client_portal_contexts_transaction_id_idx on client_portal_contexts (transaction_id);
create index if not exists client_portal_contexts_client_email_idx on client_portal_contexts (client_email);
create index if not exists client_portal_contexts_context_type_idx on client_portal_contexts (context_type);
create index if not exists client_seller_interest_requests_transaction_id_idx on client_seller_interest_requests (transaction_id);
create index if not exists client_seller_interest_requests_client_email_idx on client_seller_interest_requests (client_email);
...
drop trigger if exists trg_client_portal_contexts_updated_at on client_portal_contexts;
create trigger trg_client_portal_contexts_updated_at
before update on client_portal_contexts
for each row
execute function set_updated_at_timestamp();

drop trigger if exists trg_client_seller_interest_requests_updated_at on client_seller_interest_requests;
create trigger trg_client_seller_interest_requests_updated_at
before update on client_seller_interest_requests
for each row
execute function set_updated_at_timestamp();
...
alter table client_portal_contexts enable row level security;
alter table client_seller_interest_requests enable row level security;
...
-- Baseline schema intentionally does not install demo-wide public table access.
-- Apply targeted RLS/grant migrations for local fixtures and production data.
...
-- Transaction commission snapshot columns (Agency commission stabilisation)
...
create index if not exists transactions_commission_snapshot_idx
  on transactions (gross_commission_amount, agent_commission_amount, agency_commission_amount);


-- Attorney firm foundation (Phase 1)
create extension if not exists "pgcrypto";
...
create table if not exists public.transaction_attorney_assignments (
  ...
  matter_reference_source text not null default 'manual',
  matter_reference_updated_by uuid references auth.users(id) on delete set null,
  matter_reference_updated_at timestamptz,
...
drop trigger if exists trg_transaction_attorney_assignments_matter_reference_fields on public.transaction_attorney_assignments;
