...
-- Bridge RLS Pack 1
-- Replaces demo-open policies with authenticated, scoped access.
-- Apply only after:
-- 1) bridge_migration_pack_1.sql
-- 2) bridge_backfill_pack_1.sql
--
-- Important:
-- This pack assumes the client experience is moving to authenticated access
-- (magic link / password) rather than unrestricted anon token reads.

-- ---------------------------------------------------------------------------
-- Helper functions
-- ---------------------------------------------------------------------------

create or replace function bridge_current_user_email()
returns text
language sql
stable
as $$
...
create or replace function bridge_current_profile_role()
returns text
language sql
stable
as $$
...
create or replace function bridge_is_admin()
returns boolean
language sql
stable
as $$
...
create or replace function bridge_is_internal_user()
returns boolean
language sql
stable
as $$
  select bridge_is_admin()
    or bridge_current_profile_role() in ('developer', 'agent', 'attorney', 'bond_originator')
$$;

create or replace function bridge_has_development_access(target_development_id uuid)
returns boolean
language sql
stable
as $$
  select
    case
      ...
      when bridge_is_admin() then true
      when exists (
        select 1
        ...
        where dp.development_id = target_development_id
          ...
          and (
            dp.user_id = auth.uid()
            or (
              dp.participant_email is not null
              and lower(dp.participant_email) = bridge_current_user_email()
            )
          )
      ) then true
      when exists (
        ...
        where t.development_id = target_development_id
          ...
          and (
            ...
            or (
              ...
              and lower(tp.participant_email) = bridge_current_user_email()
            )
          )
      ) then true
      else false
...
create or replace function bridge_has_transaction_access(target_transaction_id uuid)
returns boolean
language sql
stable
as $$
    ...
    case
      ...
      when exists (
        ...
        where t.id = target_transaction_id
          and (
            (
              bridge_current_profile_role() = 'developer'
              and t.development_id is not null
              and bridge_has_development_access(t.development_id)
            )
            or (
              bridge_current_profile_role() = 'agent'
              and lower(coalesce(t.assigned_agent_email, '')) = bridge_current_user_email()
            )
            or (
              bridge_current_profile_role() = 'attorney'
              and lower(coalesce(t.assigned_attorney_email, '')) = bridge_current_user_email()
            )
            or (
              bridge_current_profile_role() = 'bond_originator'
              and lower(coalesce(t.assigned_bond_originator_email, '')) = bridge_current_user_email()
            )
          )
      ) then true
      when exists (
        ...
        join buyers b
          on b.id = t.buyer_id
        where t.id = target_transaction_id
          and bridge_current_profile_role() = 'client'
          and lower(coalesce(b.email, '')) = bridge_current_user_email()
      ) then true
      else false
    end
$$;

create or replace function bridge_can_view_internal_transaction_content(target_transaction_id uuid)
returns boolean
language sql
stable
as $$
  select bridge_has_transaction_access(target_transaction_id)
    and bridge_is_internal_user()
$$;

create or replace function bridge_has_document_access(
...
as $$
  select
    case
      when bridge_has_transaction_access(target_transaction_id) = false then false
      when bridge_is_admin() then true
      when bridge_current_profile_role() = 'developer' then true
      when bridge_current_profile_role() = 'agent' then true
      when bridge_current_profile_role() = 'attorney' then
        coalesce(target_visibility_scope, 'internal') in ('shared', 'client')
        or coalesce(target_bucket_key, '') in ('transfer', 'buyer_fica', 'sale')
      when bridge_current_profile_role() = 'bond_originator' then
        coalesce(target_visibility_scope, 'internal') in ('shared', 'client')
        or coalesce(target_bucket_key, '') in ('finance', 'buyer_fica', 'sale')
      when bridge_current_profile_role() = 'client' then
        coalesce(target_visibility_scope, 'internal') in ('shared', 'client')
      else false
    end
$$;

create or replace function bridge_can_edit_main_stage(target_transaction_id uuid)
returns boolean
language sql
stable
as $$
  select
    bridge_is_admin()
    or (
      bridge_has_transaction_access(target_transaction_id)
      and bridge_current_profile_role() in ('developer', 'agent')
    )
    or exists (
      select 1
      from transaction_participants tp
      where tp.transaction_id = target_transaction_id
        and (
          tp.user_id = auth.uid()
          or lower(coalesce(tp.participant_email, '')) = bridge_current_user_email()
        )
        and tp.can_edit_core_transaction = true
    )
$$;

create or replace function bridge_can_edit_finance_lane(target_transaction_id uuid)
returns boolean
language sql
stable
as $$
  select
    ...
    or exists (
      ...
      from transactions t
      where t.id = target_transaction_id
        and bridge_current_profile_role() = 'bond_originator'
        and lower(coalesce(t.assigned_bond_originator_email, '')) = bridge_current_user_email()
    )
$$;

create or replace function bridge_can_edit_attorney_lane(target_transaction_id uuid)
returns boolean
language sql
stable
as $$
  select
    ...
    or exists (
      ...
      from transactions t
      where t.id = target_transaction_id
        and bridge_current_profile_role() = 'attorney'
        and lower(coalesce(t.assigned_attorney_email, '')) = bridge_current_user_email()
    )
$$;

create or replace function bridge_can_manage_handover(target_transaction_id uuid)
returns boolean
language sql
stable
as $$
  select
    ...
    or (
      bridge_has_transaction_access(target_transaction_id)
      and bridge_current_profile_role() = 'developer'
    )
    or exists (
      select 1
      from transaction_participants tp
...
create or replace function bridge_can_manage_snags(target_transaction_id uuid)
returns boolean
language sql
stable
as $$
...
create or replace function bridge_can_approve_documents(target_transaction_id uuid)
returns boolean
language sql
stable
as $$
...
create or replace function bridge_can_view_client_contact_details(target_transaction_id uuid)
returns boolean
language sql
stable
as $$
  select
    bridge_is_admin()
    or bridge_current_profile_role() in ('developer', 'agent', 'attorney', 'bond_originator')
    and bridge_has_transaction_access(target_transaction_id)
$$;

-- ---------------------------------------------------------------------------
-- Ensure RLS is enabled on new pack tables
-- ---------------------------------------------------------------------------

alter table development_participants enable row level security;
alter table transaction_occupational_rent enable row level security;
...
using (
  id = auth.uid()
  or bridge_is_internal_user()
);

drop policy if exists profiles_update_self on profiles;
create policy profiles_update_self on profiles
for update to authenticated
using (id = auth.uid() or bridge_is_admin())
with check (id = auth.uid() or bridge_is_admin());
...
for select to authenticated
using (bridge_has_development_access(id));
...
with check (
  bridge_is_admin()
  or bridge_current_profile_role() = 'developer'
);
...
using (
  ...
  or (
    bridge_current_profile_role() = 'developer'
    and bridge_has_development_access(id)
  )
)
with check (
  bridge_is_admin()
...
for select to authenticated
using (bridge_has_development_access(development_id));
...
using (
  ...
  or (
    bridge_current_profile_role() = 'developer'
    and bridge_has_development_access(development_id)
  )
)
with check (
  bridge_is_admin()
...
-- ---------------------------------------------------------------------------
-- Development participants and settings/config
-- ---------------------------------------------------------------------------
...
using (
  exists (
    ...
    where dac.id = development_attorney_required_closeout_docs.development_attorney_config_id
      and bridge_has_development_access(dac.development_id)
  )
);
...
using (
  exists (
    ...
    where dbc.id = development_bond_required_closeout_docs.development_bond_config_id
      and bridge_has_development_access(dbc.development_id)
  )
);
...
for select to authenticated
using (bridge_has_transaction_access(id));
...
with check (
  bridge_is_admin()
  or bridge_current_profile_role() in ('developer', 'agent', 'attorney')
);
...
using (
  bridge_is_admin()
  or bridge_can_edit_main_stage(id)
)
with check (
  bridge_is_admin()
  or bridge_can_edit_main_stage(id)
...
for select to authenticated
using (bridge_has_transaction_access(transaction_id));
...
using (
  bridge_is_admin()
  or bridge_can_edit_finance_lane(transaction_id)
)
with check (
  bridge_is_admin()
  or bridge_can_edit_finance_lane(transaction_id)
...
for select to authenticated
using (
  bridge_has_transaction_access(transaction_id)
  and (
    visibility_scope = 'client_safe'
    or (
      visibility_scope = 'shared'
      and coalesce(bridge_current_profile_role(), '') <> 'client'
    )
    or bridge_is_internal_user()
  )
);
...
using (
  ...
  or (
    ...
    and bridge_can_edit_finance_lane(transaction_id)
  )
  or (
    process_type = 'attorney'
    and bridge_can_edit_attorney_lane(transaction_id)
  )
)
with check (
  bridge_is_admin()
...
using (
  exists (
    ...
    where tsp.id = transaction_subprocess_steps.subprocess_id
      and bridge_has_transaction_access(tsp.transaction_id)
      and (
    ...
    where tsp.id = transaction_subprocess_steps.subprocess_id
      and (
        ...
        or (
          ...
          and bridge_can_edit_finance_lane(tsp.transaction_id)
        )
        or (
          tsp.process_type = 'attorney'
          and bridge_can_edit_attorney_lane(tsp.transaction_id)
        )
      )
  )
)
...
create policy transaction_participants_modify_scoped on transaction_participants
for all to authenticated
using (
  bridge_is_admin()
  or (
    bridge_current_profile_role() = 'developer'
    and bridge_has_transaction_access(transaction_id)
  )
  or (
    bridge_can_edit_main_stage(transaction_id)
...
with check (
  ...
  or bridge_can_edit_main_stage(transaction_id)
);
...
using (
  bridge_is_admin()
  or bridge_is_internal_user() and bridge_has_transaction_access(transaction_id)
  or bridge_current_profile_role() = 'client' and bridge_has_transaction_access(transaction_id)
)
with check (
  bridge_is_admin()
  or bridge_is_internal_user() and bridge_has_transaction_access(transaction_id)
...
using (
  ...
  or (
    bridge_current_profile_role() = 'client'
    and bridge_has_transaction_access(transaction_id)
...
for select to authenticated
using (
  bridge_has_document_access(transaction_id, visibility_scope, bucket_key)
);
...
with check (
  ...
  and (
    bridge_is_internal_user()
    or bridge_current_profile_role() = 'client'
  )
);
...
using (
  ...
  or (
    bridge_is_internal_user()
    and bridge_has_document_access(transaction_id, visibility_scope, bucket_key)
  )
  or (
    bridge_current_profile_role() = 'client'
    and bridge_has_transaction_access(transaction_id)
...
for all to authenticated
using (
  bridge_can_manage_handover(transaction_id)
  or (
    bridge_current_profile_role() = 'client'
    and bridge_has_transaction_access(transaction_id)
  )
...
using (
  exists (
    ...
    where t.buyer_id = buyers.id
      and bridge_has_transaction_access(t.id)
  )
);

drop policy if exists buyers_modify_scoped on buyers;
create policy buyers_modify_scoped on buyers
for all to authenticated
using (
  bridge_is_admin()
  or exists (
    ...
    from transactions t
    where t.buyer_id = buyers.id
      and bridge_can_view_client_contact_details(t.id)
  )
)
with check (
  bridge_is_admin()
...
with check (
  (
    bridge_is_admin()
    or bridge_can_manage_snags(transaction_id)
  )
  or (
    bridge_current_profile_role() = 'client'
    and bridge_has_transaction_access(transaction_id)
...
for select to authenticated
using (bridge_is_internal_user() or bridge_current_profile_role() = 'client');
...
using (
  exists (
    ...
    where tac.id = transaction_attorney_closeout_documents.transaction_attorney_closeout_id
      and bridge_has_transaction_access(tac.transaction_id)
  )
);
...
using (
  exists (
    ...
    where tbc.id = transaction_bond_closeout_documents.transaction_bond_closeout_id
      and bridge_has_transaction_access(tbc.transaction_id)
  )
);
