...
export function ConveyancerActionCard({ item, expanded, busy, commandsDisabled = false, onToggle, onIntent, onWait, formatWhen }) {
  const missing = item.evidence?.missing || []
  const commandDisabled = commandsDisabled && !['open_documents', 'open_review', 'view'].includes(item.intent.type)
  return (
    <article className="min-w-0 overflow-hidden rounded-[15px] border border-borderSoft bg-white p-4 shadow-[0_8px_20px_rgba(15,23,42,0.035)]">
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <button type="button" className="min-w-0 flex-1 rounded-[10px] text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" onClick={onToggle} aria-expanded={expanded} aria-controls={`action-${item.actionKey}`}>
          <span className="flex items-start gap-3">
            ...
            <span className="min-w-0 flex-1"><strong className="block break-words text-sm font-semibold text-textStrong">{item.label}</strong><span className="mt-1 block break-words text-xs leading-5 text-textMuted">{item.description}</span></span>
        ...
        <div className="flex w-full min-w-0 shrink-0 flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:justify-end">
          {item.intent.type !== 'view' ? <Button type="button" size="sm" className="min-h-10 w-full max-w-full whitespace-normal text-center sm:min-h-9 sm:w-auto" onClick={() => onIntent(item)} disabled={busy || commandDisabled}>{busy ? <RefreshCw size={14} className="animate-spin" /> : item.intent.type === 'open_documents' ? <FileText size={14} /> : <CheckCircle2 size={14} />}{busy ? 'Saving…' : item.intent.label}</Button> : null}
          {['do_now', 'review'].includes(item.bucket) ? <Button type="button" size="sm" variant="secondary" className="min-h-10 w-full max-w-full whitespace-normal text-center sm:min-h-9 sm:w-auto" onClick={() => onWait(item)} disabled={busy || commandsDisabled}>Mark waiting</Button> : null}
        </div>
      </div>
      {commandDisabled ? <p className="mt-3 break-words rounded-[10px] border border-warning/25 bg-warningSoft px-3 py-2 text-xs leading-5 text-textBody">Updates are paused safely. Review and document navigation remain available.</p> : null}
      {expanded ? (
