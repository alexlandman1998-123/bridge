...
import { listLegalDocumentEditorSituations } from '../../core/documents/legalDocumentEditorSituations'
...
export default function LegalDocumentEditorContextPanel({ documentKey, documentLabel, scope, templateId = '', situationKey = '' }) {
    ...
    return (
      <section className="rounded-[18px] border border-[#cfe4d7] bg-[#f5fbf7] p-5 shadow-[0_10px_24px_rgba(15,23,42,0.035)]" aria-labelledby="situation-picker-heading">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#56806a]">Step 1 of 2</p>
            <h2 id="situation-picker-heading" className="mt-1 text-lg font-semibold text-[#18372a]">Which situation do you want to edit?</h2>
            <p className="mt-1 text-sm leading-6 text-[#5f786b]">Choose one first. The editor will show only wording used for that situation.</p>
          </div>
        </div>
        ...
        {selected ? (
          <p className="mt-4 flex items-center gap-2 rounded-[10px] border border-[#bfe0cc] bg-white px-3 py-2 text-sm font-semibold text-[#236d46]">
            ...
            Step 2: edit only the {selected.label.toLowerCase()} wording shown below.
          </p>
        ) : null}
      </section>
    )
  ...
  return (
    <section className="rounded-[18px] border border-[#dbe5ed] bg-white px-5 py-4" aria-labelledby="editor-context-heading">
      ...
      <p className="mt-1 text-sm leading-6 text-[#6c7e91]">{isSigning ? 'Signer rules are selected automatically from onboarding answers. Use the section list below to place signatures, initials and dates.' : 'For the safest edit, use Standard wording for shared text or Situation wording for company, trust, marriage, property and finance clauses.'}</p>
    </section>
  )
}
