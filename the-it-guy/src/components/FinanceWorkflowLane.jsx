import { CheckCircle2, Circle, Clock3, Lock, AlertCircle } from 'lucide-react'
    ...
    <section className="rounded-[20px] border border-[#e1e8f1] bg-white p-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)] md:p-5">
      <header className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <h3 className="text-[1.08rem] font-semibold tracking-[-0.025em] text-[#142132]">Finance Workflow</h3>
          <p className="mt-1 text-sm leading-6 text-[#6b7d93]">
            Controlled finance progression from funding setup through transfer readiness.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="inline-flex items-center rounded-full border border-[#dde4ee] bg-[#f7f9fc] px-3 py-1 text-[0.72rem] font-semibold text-[#66758b]">
            {snapshot.isLocked ? 'Waiting On Sales' : snapshot.readyForTransfer ? 'Ready' : 'In Progress'}
          </span>
        </div>
      </header>
      ...
      <footer className="mt-4 border-t border-[#e8eef5] pt-4">
        {!canEdit ? (
          <p className="text-sm text-[#6b7d93]">{roleLabel} can view Finance Workflow only.</p>
        ) : (
...
export default FinanceWorkflowLane
