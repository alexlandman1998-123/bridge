...
import KnowledgeFactoryReportPurchaseModal from "./KnowledgeFactoryReportPurchaseModal";
...
const FILTER_CLASS =
  "min-h-11 min-w-0 rounded-xl border border-slate-200 bg-white px-3 text-sm font-medium text-slate-700 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100";
...
function providerFilters(filters) {
  ...
  if (filters.transferPeriod === "since-2022")
    nextFilters.transferDateFrom = "2022-01-01";
  if (filters.transferPeriod === "since-2020")
    nextFilters.transferDateFrom = "2020-01-01";
...
export default function PropertySearchWorkspace() {
  ...
  const [selectedReportTypeIds, setSelectedReportTypeIds] = useState(() =>
    PROPERTY_REPORT_TYPE_LIST.map((reportType) => reportType.id),
  );
  ...
  const [mapPurpose, setMapPurpose] = useState(
    "Canvassing potential seller opportunities",
  );
    ...
    const timer = window.setTimeout(() => {
      ...
      propertyDataProvider
        .getPropertiesInBounds(
          propertyDataProvider.defaultBounds,
        ...
        .catch((error) => {
          if (active)
            setPropertyState({
              ...
              properties: [],
              total: 0,
              error: error?.message || "Property data is unavailable.",
            });
        });
    }, 180);
    return () => {
  ...
  useEffect(() => {
    ...
    getKnowledgeFactoryMapStatus({
      ...
      .then((status) => {
        if (!active) return;
        setKnowledgeFactoryState((previous) => ({
          ...previous,
          status: status.livePropertySearchEnabled ? "ready" : "blocked",
      ...
      .catch((error) => {
        if (active)
          setKnowledgeFactoryState((previous) => ({
            ...previous,
            status: "error",
            error: error?.message || "Property intelligence is unavailable.",
          }));
      });
    return () => {
      active = false;
  ...
  const focusedProperty = useMemo(
    ...
    [focusedPropertyId, propertyState.properties],
  );
  ...
  const isOrdering = ["submitting", "queued", "processing"].includes(
    orderState.status,
  );
      ...
      .catch((error) => {
        if (active)
          setQuoteState({
            status: "error",
            quote: null,
            error: error?.message || "Unable to price the selected reports.",
          });
      });
    return () => {
      active = false;
  ...
  function toggleProperty(property) {
    ...
    setSelectedProperties((previous) =>
      previous.some((selected) => selected.id === property.id)
  ...
  function toggleReportType(reportTypeId) {
    if (isOrdering) return;
    setSelectedReportTypeIds((previous) =>
  ...
  async function requestPropertyReports() {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 8; attempt += 1) {
        await delay(400);
        if (orderRunRef.current !== runId) return;
        const orders = await propertyDataProvider.getReportOrders({
          organisationId: currentWorkspace?.id || "",
        });
        const refreshedOrder = orders.items.find(
          (item) => item.id === order.id,
        );
        if (!refreshedOrder)
          throw new Error("The property report order could not be refreshed.");
        setOrderState({
      ...
      throw new Error(
        "The property reports took too long to generate. Please try again.",
      );
    } catch (error) {
      if (orderRunRef.current === runId)
        setOrderState({
          status: "error",
          order: null,
          error: error?.message || "Unable to generate property reports.",
        });
    }
  }
  ...
  async function searchMapArea(bounds) {
    ...
    try {
      ...
      const result = await searchKnowledgeFactoryMap({
        organisationId,
        purpose: mapPurpose,
        bounds,
        accessToken: session?.access_token,
      ...
      setKnowledgeFactoryState({
        ...
        message: result.count
          ? ""
          : "No mapped parcels were returned for this area.",
      });
    } catch (error) {
      setKnowledgeFactoryState((previous) => ({
        ...previous,
  ...
  if (KNOWLEDGE_FACTORY_MAP_ENABLED) {
    ...
    return (
      <section
        className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm xl:h-[calc(100dvh-14.5rem)] xl:min-h-[620px] xl:max-h-[760px]"
        data-canvassing-workspace="knowledge-factory-map"
      >
        <div className="flex min-h-[680px] flex-col xl:h-full xl:min-h-0">
          <div className="border-b border-slate-200 bg-white p-4">
            <div className="flex flex-wrap items-end justify-between gap-3">
              <label className="block min-w-[min(100%,420px)] flex-1">
                <span className="mb-1.5 block text-xs font-semibold text-slate-600">
                  Business purpose for this lookup
                </span>
                <input
                  value={mapPurpose}
                  onChange={(event) => setMapPurpose(event.target.value)}
                  maxLength={500}
                  className={`${FILTER_CLASS} w-full font-normal`}
                  placeholder="e.g. Canvassing potential seller opportunities"
                />
              </label>
              <div className="max-w-sm">
                <span className="inline-flex rounded-full bg-amber-100 px-2 py-0.5 text-[0.65rem] font-bold uppercase tracking-wide text-amber-900">
                  Controlled UAT
                </span>
                <p className="mt-1 text-xs leading-5 text-slate-500">
                  Parcel-only map search. Each search is permission-checked,
                  cost-audited, and capped at 25 results.
                </p>
              </div>
            </div>
            {knowledgeFactoryState.message || knowledgeFactoryState.error ? (
              <p
                role="status"
                className={`mt-3 text-sm ${knowledgeFactoryState.error ? "text-rose-700" : "text-slate-600"}`}
              >
                {knowledgeFactoryState.error || knowledgeFactoryState.message}
              </p>
            ) : null}
          </div>
          {blocked ? (
            <div className="grid min-h-[520px] flex-1 place-items-center bg-slate-50 p-6 text-center">
              <div className="max-w-md">
                <h3 className="font-semibold text-slate-900">
                  Property intelligence is not available
                </h3>
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  {knowledgeFactoryState.error ||
                    knowledgeFactoryState.message ||
                    "Your organisation needs an approved Knowledge Factory entitlement and named-user permission."}
                </p>
              </div>
            </div>
          ) : (
            <div className="grid min-h-[520px] flex-1 xl:min-h-0 xl:grid-cols-[minmax(0,1fr)_360px]">
              <KnowledgeFactoryParcelMap
                properties={knowledgeFactoryState.properties}
                loading={knowledgeFactoryState.status === "searching"}
                focusedProperty={focusedParcel}
              ...
              <KnowledgeFactoryPropertyPanel
                property={focusedParcel}
                onOpenReportBuilder={() => {
                  if (focusedParcel) setPurchaseProperty(focusedParcel);
                }}
              />
            </div>
          )}
        ...
        {purchaseProperty ? (
          <KnowledgeFactoryReportPurchaseModal
      ...
      <div className="grid min-h-[680px] xl:h-full xl:min-h-0 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="flex min-h-[580px] min-w-0 flex-col border-b border-slate-200 xl:min-h-0 xl:border-b-0 xl:border-r">
            ...
            <div className="grid gap-3 lg:grid-cols-[minmax(280px,1fr)_repeat(4,minmax(130px,auto))]">
              <label className="relative block">
                ...
                <input
                  ...
                  placeholder="Search address, suburb, estate or erf number"
                  className={`${FILTER_CLASS} w-full pl-10 font-normal`}
                />
              </label>
              <select
              ...
              <select
                ...
                value={filters.transferPeriod}
                onChange={(event) =>
                  updateFilter("transferPeriod", event.target.value)
                }
                className={FILTER_CLASS}
              >
                <option value="">Transfer date</option>
                <option value="since-2022">Since 2022</option>
              ...
              <select
                ...
                onChange={(event) =>
                  updateFilter("valueRange", event.target.value)
                }
                className={FILTER_CLASS}
              >
                <option value="">Value range</option>
                <option value="under-2m">Under R2m</option>
                <option value="2m-4m">R2m – R4m</option>
                <option value="over-4m">R4m+</option>
              </select>
            </div>
            <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500">
              <div className="flex flex-wrap items-center gap-2.5">
                <span>
                  {propertyState.status === "error"
                    ? propertyState.error
                    : `${propertyState.total} matching properties`}
                </span>
                {propertyDataProvider.isDemoData ? (
                  ...
                  >
                    <button
                      type="button"
                      className="inline-flex min-h-7 items-center gap-1 rounded-lg bg-blue-50 px-2 text-[0.66rem] font-bold tracking-[0.04em] text-blue-700 transition hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-blue-200"
                      aria-describedby="canvassing-demo-data-tooltip"
                    >
                      DEMO DATA <Info size={13} />
                    </button>
                    <span
                      id="canvassing-demo-data-tooltip"
                      role="tooltip"
                      className="pointer-events-none absolute left-0 top-full z-30 mt-2 w-64 rounded-xl border border-slate-200 bg-slate-950 px-3 py-2 text-xs font-medium leading-5 text-white opacity-0 shadow-xl transition group-hover:opacity-100 group-focus-within:opacity-100"
                    >
                      Demonstration property intelligence. Records are fictional
                      and no live provider charges apply.
                    </span>
                  </span>
                ) : null}
              </div>
              <span className="inline-flex items-center gap-1.5">
                ...
                {activeFilterCount
                  ...
                  : "Showing the current property area"}
              </span>
            </div>
          </div>
          ...
          <MockParcelMap
            ...
            bounds={propertyDataProvider.defaultBounds}
            focusedProperty={focusedProperty}
            selectedPropertyIds={selectedPropertyIds}
            isDemoData={propertyDataProvider.isDemoData}
            loading={propertyState.status === "loading"}
            onFocusProperty={(property) =>
              setFocusedPropertyId(property?.id || "")
            }
        ...
        <PropertyReportBasket
          ...
          onViewReports={() =>
            navigate("/pipeline/canvassing/property-reports")
          }
        />
      </div>
    </section>
