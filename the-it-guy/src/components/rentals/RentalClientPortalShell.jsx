import { Building2, FileText, Home, MessageCircle, Wrench } from 'lucide-react'
...
const tenantItems = [
  ...
  { key: 'home', label: 'My home', mobileLabel: 'Home', icon: Building2 },
...
const groupsFor = (role) => role === 'landlord'
  ...
  : [{ label: 'Home', items: tenantItems.slice(0, 1) }, { label: 'Your tenancy', items: tenantItems.slice(1, 4) }, { label: 'Account', items: tenantItems.slice(4) }]

export function rentalPortalItems(role = 'tenant') { return role === 'landlord' ? landlordItems : tenantItems }

export default function RentalClientPortalShell({ role = 'tenant', activeKey = 'overview', property = {}, subtitle = '', children }) {
  const items = rentalPortalItems(role)
  const theme = createBuyerPortalTheme({ primaryColour: '#10213a', secondaryColour: '#183f5b', accentColour: '#36b47e' })
  ...
  return <div className="min-h-screen bg-[#f5f7fa] text-[#101823]">
    <BuyerPortalDesktopSidebar
      brandName="Arch9 Rentals"
    ...
    <main className="mx-auto w-full max-w-[1640px] px-4 pb-24 pt-4 sm:px-6 sm:pt-6 lg:ml-[264px] lg:w-[calc(100%-264px)] lg:px-10 lg:pb-10 lg:pt-10">
      <div className="lg:hidden"><BuyerMobileHeader brand={theme} brandName="Arch9 Rentals" homePath="?section=overview" title="Arch9 Rentals" /></div>
      <BuyerMobilePropertyHero theme={theme} imageAlt={propertyName} className="min-h-[210px] lg:min-h-[250px]">
