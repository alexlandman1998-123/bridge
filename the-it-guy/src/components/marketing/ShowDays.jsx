...
export function ShowDaysOverview({ onOpenShowDay }) {
  ...
  return (
    <div className="wa-page show-days-page">
      <header className="wa-page-header"><div className="wa-title-block"><span className="wa-brand-icon show-days-brand-icon"><CalendarDays size={24} /></span><div><h1>Show Days</h1><p>Create, manage and track your property show days.</p></div></div><button className="wa-primary-button" type="button">Create Show Day <ChevronRight size={16} /></button></header>
      <ShowDaysStats />
      <section className="wa-campaigns-panel"><div className="wa-tabs" role="tablist" aria-label="Show day status">{showDaysTabs.map((tab) => <button className={activeTab === tab ? 'wa-tab-active' : ''} type="button" role="tab" aria-selected={activeTab === tab} onClick={() => setActiveTab(tab)} key={tab}>{tab}</button>)}</div><div className="wa-campaigns-content"><ShowDaysFilters /><div className="wa-campaign-list">{showDays.map((showDay) => <ShowDayCard showDay={showDay} onOpen={onOpenShowDay} key={showDay.id} />)}</div><footer className="wa-list-footer"><span>Showing 1–3 of 8 show days</span><div aria-label="Pagination">{[1, 2, 3].map((page) => <button type="button" className={`wa-page-number ${page === 1 ? 'wa-page-number-active' : ''}`} key={page}>{page}</button>)}</div></footer></div></section>
    </div>
  )
}
