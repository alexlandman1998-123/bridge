...
import { ArrowLeft, ArrowUpRight, CalendarDays, ChevronRight, FileText, Globe2, LockKeyhole, MessageCircle, Pencil, RefreshCw, UserPlus, Users } from 'lucide-react'
import { useAuthSession } from '../../context/AuthSessionContext'
import { createWebsiteBlogPost, createWebsiteDraft, createWebsiteSite, discardWebsiteDraft, getWebsiteWorkspaceOverview, manageWebsiteDomain, publishWebsiteDraft, resetWebsiteDraftBrand, saveWebsiteDraftBrand, saveWebsiteDraftPage, updateWebsiteBlogPost } from '../../services/websiteWorkspaceService'
  ...
  return (
    <div className="wa-page website-workspace website-editor">
      ...
      <section className="ww-operations" aria-label="Website operation status">
        <div className="ww-operations-heading"><div><h2>{overview.mode === 'loading' ? 'Checking your website…' : overview.mode === 'connected' ? 'Edit your website' : 'Ready for the first website setup'}</h2><p>Make the essential identity and content updates here. Your changes stay private until you publish.</p></div><button className="ww-refresh" type="button" onClick={() => void refresh()}><RefreshCw size={15} /> Refresh</button></div>
        {error && <p className="ww-error" role="status">{error}</p>}
        {notice && <p className="ww-notice" role="status">{notice}</p>}
