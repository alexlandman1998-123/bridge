    ...
    {showPreview ? <div className="wlo-blog-full-preview"><div className="wlo-preview-device-toggle"><button type="button" className={previewDevice === 'desktop' ? 'active' : ''} onClick={() => setPreviewDevice('desktop')}>Desktop</button><button type="button" className={previewDevice === 'mobile' ? 'active' : ''} onClick={() => setPreviewDevice('mobile')}>Mobile</button></div><BlogRenderedPreview post={editing} mediaAssets={mediaAssets} websiteListings={websiteListings} device={previewDevice} /></div> : <form className="wlo-blog-editor-page" onSubmit={(event) => { event.preventDefault(); void save() }}>
      <main className="wlo-blog-editor-canvas"><label className="wlo-blog-category">PROPERTY JOURNAL</label><input className="wlo-blog-title" disabled={!canEdit} required maxLength={160} value={editing?.title || ''} onChange={(event) => change('title', event.target.value)} placeholder="Give your article a clear, useful title" /><label className="wlo-blog-slug">your website /blog/<input disabled={!canEdit} required maxLength={80} value={editing?.slug || ''} onChange={(event) => change('slug', event.target.value)} placeholder="article-url" /></label>
        <section className="wlo-cover-image" aria-labelledby="cover-image-heading">
          ...
          <label>Cover image alt text<input disabled={!canEdit} required={Boolean(editing.coverImageUrl)} maxLength={240} value={editing.coverImageAlt} onChange={(event) => { change('coverImageAlt', event.target.value); setMediaUploadError('') }} placeholder="Describe the image for visitors who cannot see it" /></label>
          {selectedCoverAsset ? <div className="wlo-media-description"><small>Changing this description updates the media library wherever this image is used.</small><button className="ww-rollback" type="button" disabled={!canEdit || updatingMedia} onClick={() => void updateSelectedMediaDescription()}>{updatingMedia ? 'Saving description…' : 'Save image description'}</button></div> : null}
          {editing.coverImageUrl ? <img className="wlo-featured-image" src={editing.coverImageUrl} alt="" /> : <p className="wlo-cover-warning" role="status">A featured image is required before this article can be saved. It gives the public article and journal card a clear visual anchor.</p>}
          <label className="wlo-media-upload">Upload and use as featured image<input disabled={!canEdit || uploading} type="file" accept="image/jpeg,image/png,image/webp,image/avif" onChange={(event) => { const file = event.target.files?.[0]; if (!file) return; const altText = editing.coverImageAlt.trim(); if (!altText) { setMediaUploadNotice(''); setMediaUploadError('Add cover image alt text before uploading.'); event.target.value = ''; return }; setMediaUploadError(''); setMediaUploadNotice('Uploading image…'); void onUpload(file, altText, (asset) => { setEditing((current) => ({ ...current, coverImageUrl: asset.public_url, coverImageAlt: asset.alt_text })); setMediaUploadNotice('Image uploaded and selected as the featured image.'); event.target.value = '' }).then((asset) => { if (!asset) { setMediaUploadNotice(''); setMediaUploadError('The image could not be uploaded. Please try again.') } }) }} />{uploading ? <small>Uploading image…</small> : <small>JPG, PNG, WebP or AVIF, up to 10 MB.</small>}</label>
