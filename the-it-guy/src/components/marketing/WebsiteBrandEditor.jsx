    ...
    <section className="ww-brand-editor" aria-labelledby="website-brand-heading">
      ...
      <form onSubmit={submit}>
        <div className="ww-brand-fields">
          ...
          <div className="ww-logo-field"><div className="ww-logo-preview ww-logo-preview-light"><LogoPreview url={draft.logoLightUrl} name={draft.name} /></div><div><strong>Logo for light backgrounds</strong><p>Used in the white website header and footer.</p><label className="ww-upload-button"><ImageUp size={15} aria-hidden="true" /> {uploading === 'logoLightUrl' ? 'Uploading…' : 'Upload logo'}<input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" disabled={Boolean(uploading) || busy} onChange={(event) => void uploadLogo(event.target.files?.[0], 'logoLightUrl')} /></label><label className="ww-url-field"><span>Or use an HTTPS image URL</span><input type="url" value={draft.logoLightUrl} onChange={(event) => update('logoLightUrl', event.target.value)} placeholder="https://…" /></label></div></div>
          <div className="ww-logo-field ww-logo-field-dark"><div className="ww-logo-preview ww-logo-preview-dark"><LogoPreview url={draft.logoDarkUrl} name={draft.name} /></div><div><strong>Logo for dark backgrounds</strong><p>Used when the logo appears over the primary colour.</p><label className="ww-upload-button"><ImageUp size={15} aria-hidden="true" /> {uploading === 'logoDarkUrl' ? 'Uploading…' : 'Upload logo'}<input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" disabled={Boolean(uploading) || busy} onChange={(event) => void uploadLogo(event.target.files?.[0], 'logoDarkUrl')} /></label><label className="ww-url-field"><span>Or use an HTTPS image URL</span><input type="url" value={draft.logoDarkUrl} onChange={(event) => update('logoDarkUrl', event.target.value)} placeholder="https://…" /></label></div></div>
          {[
