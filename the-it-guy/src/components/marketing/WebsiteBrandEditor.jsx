    ...
    <section className="ww-brand-editor" aria-labelledby="website-brand-heading">
      ...
      <form onSubmit={submit}>
        <div className="ww-identity-layout">
          ...
          <section className="ww-identity-section"><header><h3>Logos</h3><p>Use the standard logo on light areas, then add an alternate version for the dark footer and other dark areas.</p></header><div className="ww-identity-logos"><div className="ww-identity-logo-card"><div className="ww-logo-preview ww-logo-preview-light"><LogoPreview url={draft.logoLightUrl} name={draft.name} /></div><div><strong>Standard logo</strong><span>Header and light areas</span></div><label className="ww-upload-button"><ImageUp size={15} aria-hidden="true" /> {uploading === 'logoLightUrl' ? 'Uploading…' : 'Upload logo'}<input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" disabled={Boolean(uploading) || busy} onChange={(event) => void uploadLogo(event.target.files?.[0], 'logoLightUrl')} /></label></div><div className="ww-identity-logo-card"><div className="ww-logo-preview ww-logo-preview-dark"><LogoPreview url={draft.logoDarkUrl || draft.logoLightUrl} name={draft.name} /></div><div><strong>Dark-background logo</strong><span>Footer and dark areas; optional</span></div><label className="ww-upload-button"><ImageUp size={15} aria-hidden="true" /> {uploading === 'logoDarkUrl' ? 'Uploading…' : 'Upload alternate logo'}<input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" disabled={Boolean(uploading) || busy} onChange={(event) => void uploadLogo(event.target.files?.[0], 'logoDarkUrl')} /></label></div></div></section>
