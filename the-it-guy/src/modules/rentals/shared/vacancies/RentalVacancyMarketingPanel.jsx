...
export function RentalVacancyMarketingPanel({ vacancy = {}, userId = '' }) {
  ...
  return <section className="space-y-5 rounded-xl border bg-white p-5 shadow-sm">
    ...
    <div><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="text-sm font-semibold">Media ({media.length})</h3><label className="inline-flex cursor-pointer items-center gap-1 rounded-lg border px-3 py-2 text-sm font-medium"><Upload className="h-4 w-4" />Add media<input type="file" accept={RENTAL_MEDIA_UPLOAD_ACCEPT} disabled={saving} onChange={(event) => void upload(event)} className="sr-only" /></label></div><p className="mt-2 text-xs text-slate-500">JPEG, PNG, WebP, MP4, or MOV · maximum 20 MB.</p>{media.length ? <ul className="mt-2 divide-y rounded-lg border">{media.map((item) => <li key={item.id} className="flex items-center justify-between gap-2 px-3 py-2 text-sm"><span className="truncate">{item.media_type} · {item.storage_path.split('/').pop()}</span><button disabled={saving} onClick={() => void remove(item)} className="rounded p-1 text-slate-500 hover:text-red-700" aria-label="Remove media"><X className="h-4 w-4" /></button></li>)}</ul> : <p className="mt-2 text-sm text-slate-500">Add at least one image before review.</p>}</div>
