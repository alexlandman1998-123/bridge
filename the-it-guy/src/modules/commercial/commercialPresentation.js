...
export function getCommercialNextAction(kind, record = {}) {
  ...
  if (kind === 'properties') {
    ...
    return Number(record.available_space_m2) > 0 ? 'Match vacancies' : 'Review occupancy'
  }

  if (kind === 'vacancies') {
    if (!isPresent(record.property_id)) return 'Link property'
    if (!isPresent(record.landlord_id)) return 'Add landlord details'
    ...
    if (['leased', 'occupied'].includes(status)) return 'Confirm lease record'
    if (status === 'hot_in_progress') return 'Track HOT'
    if (status === 'lease_pending') return 'Create lease'
    if (status === 'reserved' || status === 'under_negotiation' || status === 'under_offer') return 'Create deal'
  ...
  if (kind === 'listings') {
    ...
    if (listingStatus === 'coming_soon') return 'Prepare marketing'
    if (listingStatus === 'active') return 'Match requirements'
    if (listingStatus === 'under_offer') return 'Progress deal'
    if (['leased', 'sold'].includes(listingStatus)) return 'Confirm transaction'
    if (listingStatus === 'expired') return 'Review listing'
    return 'Review listing'
  }

  if (kind === 'requirements') {
    if (!isPresent(record.tenant_id)) return 'Link tenant'
    ...
    if (stage === 'viewing') return 'Schedule viewing'
    if (['proposal', 'negotiating', 'negotiation'].includes(stage)) return 'Create deal'
    if (['converted', 'lease_stage', 'closed_won'].includes(stage)) return 'Follow lease progress'
