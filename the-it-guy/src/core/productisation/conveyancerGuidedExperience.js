...
export function buildConveyancerGuidedExperience({ cockpit = null, context = {} } = {}) {
  ...
  const documentState = systemState({ available: documents.available, stopped: documents.control?.killSwitchEnabled, failed: Number(documents.counts?.failed || 0) > 0, observe: documents.control?.mode === 'observe', activeLabel: 'Documents connected', manualLabel: 'Manual documents' })
  const providerState = systemState({ available: runtime.available, stopped: operations.killSwitchActive || runtime.control?.killSwitchEnabled, failed: operations.snapshot?.health === 'fail' || runtime.health?.some?.((event) => event.circuit_state === 'open') || Number(transport.outbound?.dead_letter || 0) > 0, observe: runtime.control?.mode === 'observe', activeLabel: 'Providers connected', manualLabel: 'Manual provider work' })
  return freeze({
    ...
    systems: [
      ...
      { id: 'documents', label: 'Documents and signing', state: documentState.key, statusLabel: documentState.label, tone: documentState.tone, action: 'documents', detail: documents.available ? `${Number(documents.counts?.processing || 0)} processing · ${Number(documents.counts?.failed || 0)} need help` : 'Upload, prepare and sign through the existing document workspace.' },
      { id: 'providers', label: 'Banks, SARS, municipalities and Deeds', state: providerState.key, statusLabel: providerState.label, tone: providerState.tone, detail: runtime.available ? `${Number(transport.outbound?.queued || 0) + Number(transport.outbound?.retry_scheduled || 0)} queued · ${Number(transport.inbound?.awaiting_review || 0)} to review` : 'Record provider outcomes manually; no external connection is required.' },
    ],
