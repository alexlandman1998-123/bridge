...
export function resolveLegalDocumentGenerationRecovery(error = null, { packetType = 'mandate' } = {}) {
  ...
  if (['PACKETS_RLS_DENIED', 'FORBIDDEN', 'GENERATION_FORBIDDEN', 'PACKET_GENERATION_FORBIDDEN'].includes(code)) return withDiagnostics({ code, label: 'Access needs attention', message: `Your current organisation role cannot generate this ${label}.`, nextAction: 'Ask an organisation administrator to check your legal-document access.', retryable: false, actionKey: 'contact_admin', actionLabel: 'Copy admin reference' }, error)
