...
export const OTP_TEMPLATE_BRANDED_SHELL_VERSION = 'otp_template_vnext_phase5_branded_shell_v1'
export const OTP_BRANDED_SHELL_LAYOUT_CONTRACT = 'otp_branded_template_shell_phase5_v1'
...
export const OTP_BRANDED_SHELL_SLOTS = Object.freeze([
  ...
  shellSlot({
    ...
    label: 'Document details header',
    slotType: 'header_details',
    region: 'top_right',
    sortOrder: 1,
    sourceOwners: ['transaction_offer_terms', 'legal_template_registry'],
    placeholderKeys: ['otp_document_variant', 'transaction_reference', 'document_reference', 'template_version'],
  ...
  shellSlot({
    ...
    sortOrder: 90,
    sourceOwners: ['organisation_agent_settings'],
    placeholderKeys: ['organisation_trading_name', 'agent_full_name', 'agent_email', 'agent_phone', 'agent_ffc_number'],
    fallbackText: '{{organisation_trading_name}}',
    layout: {
      x: 48,
      y: 30,
  ...
  shellSlot({
    ...
    placeholderKeys: ['buyer_signature', 'seller_signature'],
  ...
  shellSlot({
    ...
    variants: ['new_development'],
    sourceOwners: ['signing_runtime'],
    placeholderKeys: ['buyer_signature', 'developer_signature'],
    blankRenderPolicy: 'runtime_generated',
    fallbackText: 'Signature fields generated at signing runtime',
    layout: {
      renderMode: 'signature_zone_only',
...
function buildChecks({ manifests = [], layoutEstimates = [] } = {}) {
  ...
  addCheck(checks, missingLayoutContracts.length === 0, 'PHASE5_LAYOUT_CONTRACT_ON_EVERY_SLOT', missingLayoutContracts.length ? `Missing layout contract: ${missingLayoutContracts.join(', ')}` : 'Every shell slot carries the Phase 5 layout contract.')
