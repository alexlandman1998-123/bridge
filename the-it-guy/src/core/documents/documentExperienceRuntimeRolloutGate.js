...
export function resolveDocumentExperienceRuntimeRolloutAccess({ organisationId = '', enforcementMode = 'shadow', schemaAvailable = false, rpcResult = null } = {}) {
  ...
  if (!schemaAvailable) {
    if (enforcement === 'shadow') return { contract: 'arch9-document-experience-runtime-rollout-gate-v1', allowed: true, status: 'shadow_allowed', code: 'N6_SHADOW_SCHEMA_PENDING', stage: 'legacy', revision: 0, configured: false }
  ...
  if (!organisationPresent) {
    if (enforcement === 'shadow') return { contract: 'arch9-document-experience-runtime-rollout-gate-v1', allowed: true, status: 'shadow_allowed', code: 'N6_SHADOW_UNSCOPED', stage: 'legacy', revision: 0, configured: false }
  ...
  if (!configured) {
    if (enforcement === 'shadow') return { contract: 'arch9-document-experience-runtime-rollout-gate-v1', allowed: true, status: 'shadow_allowed', code: 'N6_SHADOW_NOT_CONFIGURED', stage: 'legacy', revision: 0, configured: false }
