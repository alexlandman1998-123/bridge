...
test('canonical matrix validates before adapter mapping is used', () => {
  ...
  assert.equal(validation.counts.requirements, 66)
...
test('adapter maps legacy buyer, seller and attorney keys to canonical request keys', () => {
  ...
  assert.equal(resolveCanonicalDocumentRequestKey('signed_fica_declaration', 'buyer'), 'buyer_fica_declaration')
  assert.equal(resolveCanonicalDocumentRequestKey('buyer_fica_declaration', 'buyer'), 'buyer_fica_declaration')
