...
export function buildSellerOnboardingJourneyStatus({ formData = {}, onboardingSubmitted = false, mandateSigned = false } = {}) {
  ...
  return {
    ...
    steps: [
      ...
      { key: 'pack', label: 'FICA + mandate pack sent', complete: packSent },
    ...
    documents: [
      ...
      { key: 'fica', label: 'FICA declaration', status: packSent ? 'sent_for_signature' : reviewed ? 'ready_to_send' : 'awaiting_agent_review' },
      { key: 'mandate', label: 'Mandate', status: signed ? 'signed' : packSent ? 'sent_for_signature' : reviewed ? 'ready_to_send' : 'awaiting_agent_review' },
    ],
  }
}
