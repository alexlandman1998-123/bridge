...
export const ATTORNEY_THREE_ROLE_PHASE4_VERSION = 'attorney_bond_cockpit_phase4_v1'

const BOND_STAGES = ATTORNEY_WORKFLOW_STAGE_DEFINITIONS.bond.map((stage) => stage.key)

const DOMAIN_DEFINITIONS = Object.freeze([
  Object.freeze({ key: 'bank_instruction', label: 'Bank instruction & approval', start: 'bond_instruction_received', end: 'bond_approval_letter_received' }),
  Object.freeze({ key: 'bank_conditions', label: 'Bank requirements & conditions', start: 'bank_requirements_confirmed', end: 'bank_conditions_resolved' }),
  Object.freeze({ key: 'drafting_signing', label: 'Bond drafting & signing', start: 'bond_documents_prepared', end: 'buyer_signed_bond_documents' }),
  Object.freeze({ key: 'bank_lodgement_authority', label: 'Bank approval to lodge', start: 'bond_documents_sent_to_bank', end: 'bank_approval_to_lodge_received' }),
  Object.freeze({ key: 'guarantees_coordination', label: 'Guarantees & transfer handoff', start: 'guarantees_issued', end: 'guarantee_wording_accepted', dependency: true }),
  Object.freeze({ key: 'lodgement_registration', label: 'Lodgement, registration & close', start: 'bond_lodgement_ready', end: 'bond_close_out_complete' }),
])
...
function domainStatus({ definition, currentIndex, laneStatus, workflowState, coordination }) {
  const startIndex = BOND_STAGES.indexOf(definition.start)
  const endIndex = BOND_STAGES.indexOf(definition.end)
...
export function buildBondAttorneyCockpit(workflow = {}) {
  ...
  const currentIndex = BOND_STAGES.indexOf(stageKey)
  ...
  const domains = DOMAIN_DEFINITIONS.map((definition) => Object.freeze({
    ...
    current: currentIndex >= BOND_STAGES.indexOf(definition.start) && currentIndex <= BOND_STAGES.indexOf(definition.end),
  }))
  ...
  const blockers = [
    ...
    ...(readiness.assignment && !readiness.assignment.complete ? ['Bond attorney assignment is outstanding.'] : []),
    ...(blockedDependencies ? [`${blockedDependencies} linked legal ${blockedDependencies === 1 ? 'dependency is' : 'dependencies are'} blocking bond progress.`] : []),
  ]

  return Object.freeze({
    version: ATTORNEY_THREE_ROLE_PHASE4_VERSION,
    laneKey: 'bond',
    title: 'Bond Attorney Command Centre',
    valueProposition: 'A bank-compliant bond matter that stays synchronised with transfer readiness and registration.',
    controlLabel: 'Bond lane control',
    dependencyTitle: 'Transfer and registration handoffs',
    dependencyDescription: 'Transfer readiness is visible here without allowing the bond team to change the transfer lane.',
    emptyDependencyMessage: 'No linked transfer handoffs are required yet.',
    ...
    canAct,
    readOnlyReason: canAct ? '' : (lane.permissions?.readOnlyReason || 'This lane is visible for coordination, but only the assigned bond team may change it.'),
    primaryAction,
