...
const entitySellerTypes = new Set(['company', 'close_corporation', 'foreign_company', 'trust', 'foreign_trust', 'deceased_estate'])
const entitySeller = (seller = {}) => entitySellerTypes.has(text(seller.legalType).toLowerCase())
const missingFicaDetails = (details = {}, seller = {}) => {
  const naturalPerson = !entitySeller(seller)
  const trust = ['trust', 'foreign_trust'].includes(text(seller.legalType).toLowerCase())
  return [
    naturalPerson && !text(details.firstName) && 'first name',
    naturalPerson && !text(details.surname) && 'surname',
    naturalPerson && !text(details.idNumber) && 'ID or passport number',
    naturalPerson && !text(details.dateOfBirth) && 'date of birth',
    naturalPerson && !text(details.residentialAddress) && 'residential address',
    !naturalPerson && !text(details.entityName) && (trust ? 'trust name' : 'company name'),
    !naturalPerson && !text(details.entityRegistrationNumber) && (trust ? 'trust registration number' : 'company registration number'),
    !naturalPerson && !text(details.registeredAddress) && 'registered address',
  ].filter(Boolean)
}

function signingHeaderLogoCandidates(branding = {}) {
  return [branding.logoLightUrl, branding.logoDarkUrl, branding.logoIconUrl]
    .map(text)
...
function BrandMark({ branding }) {
  ...
  if (logo) {
    return <img
      src={logo}
      alt={`${name} logo`}
      className="h-12 max-w-[230px] object-contain object-left"
      onError={() => setFailedLogos((current) => current.includes(logo) ? current : [...current, logo])}
    />
  }
  return <span data-agency-brand="text-fallback" className="inline-flex min-h-11 items-center gap-2 text-sm font-bold tracking-wide text-white"><span className="grid h-9 w-9 place-items-center rounded-lg bg-white/15 text-xs ring-1 ring-white/20">{getOnboardingBrandInitials(name)}</span><span className="max-w-[190px] truncate">{name}</span></span>
}
...
function FicaField({ label, field, details, onChange, type = 'text', optional = false }) {
  return <label className="block min-w-0 overflow-hidden text-sm font-semibold text-[#47627b]">{label}{optional ? <span className="font-normal"> (if available)</span> : null}<input type={type} value={details[field] || ''} onChange={(event) => onChange(field, event.target.value)} className="mt-1.5 box-border block w-full min-w-0 max-w-full rounded-lg border border-[#cfdce8] bg-white px-3 py-2.5 font-normal text-[#172334] outline-none focus:border-[var(--signing-primary)] focus:ring-2 focus:ring-[#dcebf4]" style={{ inlineSize: '100%', minInlineSize: 0 }} /></label>
}

function SellerQuestionnaire({ documentKey, disclosureResponses, ficaDetails, seller, onDisclosureChange, onFicaChange, isPrimaryDocumentContact }) {
  ...
  if (documentKey === 'fica') {
    if (!isPrimaryDocumentContact) return <section className="mt-5 rounded-xl border border-[#d6e5f0] bg-[#f8fbfd] p-4"><h3 className="font-bold text-[#28425c]">FICA details are managed by the primary contact</h3><p className="mt-1 text-sm leading-5 text-[#60748b]">If anything is incorrect, flag it for correction.</p></section>
    const isEntity = entitySeller(seller)
    const isTrust = ['trust', 'foreign_trust'].includes(text(seller.legalType).toLowerCase())
    return <section className="mt-5 rounded-xl border border-[#d6e5f0] bg-[#f8fbfd] p-4 sm:p-5"><div><h3 className="font-bold text-[#28425c]">Confirm your FICA details</h3><p className="mt-1 text-sm leading-5 text-[#60748b]">Update anything that is incorrect.</p></div>{isEntity ? <><fieldset className="mt-5"><legend className="text-sm font-bold text-[#28425c]">{isTrust ? 'Trust details' : 'Company details'}</legend><div className="mt-3 grid gap-3 sm:grid-cols-2"><FicaField label={isTrust ? 'Trust name' : 'Company legal name'} field="entityName" details={ficaDetails} onChange={onFicaChange} /><FicaField label={isTrust ? 'Trust registration number' : 'Company registration number'} field="entityRegistrationNumber" details={ficaDetails} onChange={onFicaChange} /><label className="block min-w-0 text-sm font-semibold text-[#47627b] sm:col-span-2">Registered address<textarea value={ficaDetails.registeredAddress || ''} onChange={(event) => onFicaChange('registeredAddress', event.target.value)} className="mt-1.5 min-h-20 w-full min-w-0 max-w-full rounded-lg border border-[#cfdce8] bg-white p-2.5 font-normal text-[#172334] outline-none focus:border-[var(--signing-primary)] focus:ring-2 focus:ring-[#dcebf4]" /></label></div></fieldset><p className="mt-4 rounded-xl border border-[#dce6ef] bg-white p-3 text-sm leading-5 text-[#60748b]">Ask your agent to correct an authorised signer or authority record.</p></> : <><fieldset className="mt-5"><legend className="text-sm font-bold text-[#28425c]">Identity</legend><div className="mt-3 grid gap-3 sm:grid-cols-2"><FicaField label="First name" field="firstName" details={ficaDetails} onChange={onFicaChange} /><FicaField label="Surname" field="surname" details={ficaDetails} onChange={onFicaChange} /><FicaField label="ID or passport number" field="idNumber" details={ficaDetails} onChange={onFicaChange} /><FicaField label="Date of birth" field="dateOfBirth" details={ficaDetails} onChange={onFicaChange} type="date" /><FicaField label="Nationality" field="nationality" details={ficaDetails} onChange={onFicaChange} optional /><FicaField label="Country of residence" field="countryOfResidence" details={ficaDetails} onChange={onFicaChange} optional /></div></fieldset><fieldset className="mt-5 border-t border-[#dce6ef] pt-5"><legend className="text-sm font-bold text-[#28425c]">Contact details</legend><div className="mt-3 grid gap-3 sm:grid-cols-2"><FicaField label="Email address" field="email" details={ficaDetails} onChange={onFicaChange} type="email" /><FicaField label="Phone number" field="phone" details={ficaDetails} onChange={onFicaChange} type="tel" /><label className="block min-w-0 text-sm font-semibold text-[#47627b] sm:col-span-2">Residential address<textarea value={ficaDetails.residentialAddress || ''} onChange={(event) => onFicaChange('residentialAddress', event.target.value)} className="mt-1.5 min-h-20 w-full min-w-0 max-w-full rounded-lg border border-[#cfdce8] bg-white p-2.5 font-normal text-[#172334] outline-none focus:border-[var(--signing-primary)] focus:ring-2 focus:ring-[#dcebf4]" /></label></div></fieldset><fieldset className="mt-5 border-t border-[#dce6ef] pt-5"><legend className="text-sm font-bold text-[#28425c]">Tax and compliance</legend><div className="mt-3 grid gap-3 sm:grid-cols-2"><FicaField label="Income tax number" field="incomeTaxNumber" details={ficaDetails} onChange={onFicaChange} optional /></div><p className="mt-3 text-xs leading-5 text-[#60748b]">Additional AML information may be needed, depending on the transaction.</p></fieldset></>}</section>
  }
  return null
}
...
export default function ListingMandateSigning() {
  ...
  const [postSignatureCorrectionRequested, setPostSignatureCorrectionRequested] = useState(false)
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (result.error || result.data?.success === false) setError(result.error?.message || result.data?.error || 'This signing link is no longer available.')
      else if (result.data?.complete) setComplete({ groupComplete: result.data?.groupComplete === true })
      else {
        ...
        setFicaDetails({
          ...
          entityName: text(seller.companyName || seller.trustName || seller.name),
          entityRegistrationNumber: text(seller.companyRegistrationNumber || seller.trustRegistrationNumber),
          registeredAddress: text(seller.companyRegisteredAddress || seller.trustRegisteredAddress || seller.residentialAddress || seller.address),
        })
      }
      setLoading(false)
    }
  ...
  async function submit(event) {
    ...
    if (missingAcceptance) { setError(`Please review and accept the ${documentLabels[missingAcceptance] || 'document'} before signing.`); return }
    if (!text(signedName)) { setError('Enter your full legal name before submitting.'); return }
    if (!text(signature)) { setError('Add your signature before submitting.'); return }
  ...
  async function requestPostSignatureCorrection() {
    if (text(issue).length < 5) { setError('Describe the correction your agent needs to make.'); return }
    setSubmitting(true); setError('')
    const result = await invokeEdgeFunction('listing-mandate-signing', { body: { action: 'request-correction', token, issue } })
    setSubmitting(false)
    if (result.error || result.data?.success === false) { setError(result.error?.message || result.data?.error || 'Unable to send your correction request.'); return }
    setPostSignatureCorrectionRequested(true)
  }
  ...
  // Do not paint Arch9's historic blue before the recipient's pack has loaded.
  // The signing link owns its CI, so an unresolved page stays neutral and only
  // receives a brand colour once the frozen pack branding is available.
  const theme = { '--signing-primary': validHex(branding.primaryColour, '#111111'), '--signing-secondary': validHex(branding.secondaryColour, '#000000'), '--signing-accent': validHex(branding.accentColour, '#168044') }
  ...
  // The primary contact edits the FICA fields directly. Rendering the generated
  // document rows as well would show the same facts twice on this step.
  const showReadOnlyDocumentDetails = activeDocumentKey !== 'fica' || !isPrimaryDocumentContact
  ...
  if (loading) {
    return <main className="grid min-h-screen place-items-center bg-[#f7f8fa] text-[#172334]"><div className="flex flex-col items-center gap-3 text-sm font-semibold text-[#52657a]"><Loader2 className="animate-spin text-[#52657a]" size={28} /><span>Opening your secure documents…</span></div></main>
  }
  ...
  async function continueToNextStep() {
    ...
    if (isPrimaryDocumentContact && activeDocumentKey === 'fica') {
      ...
      setSession((current) => current ? {
        ...current,
        // The save response is the updated pack snapshot. Keep the branding that
        // was hydrated when the signing link opened: the database snapshot does
        // not carry the live agency CI on every save response.
        signingPack: { ...current.signingPack, ...result.data.signingPack, branding: current.signingPack?.branding },
        signingPackDigest: result.data.signingPackDigest,
      } : current)
    }
  ...
  return <main className="min-h-screen bg-[#edf3f7] text-[#172334]" style={theme}>
    <div className="min-h-[250px] bg-[linear-gradient(118deg,var(--signing-secondary),var(--signing-primary))] px-4 pb-20 pt-6 sm:px-6 sm:pt-9">
      <header className="mx-auto flex max-w-3xl items-center"><BrandMark branding={branding} /></header>
    ...
    <section className="mx-auto -mt-10 max-w-3xl px-4 pb-10 sm:px-6 sm:pb-16"><div className="overflow-hidden rounded-[24px] bg-white shadow-[0_18px_55px_rgba(16,42,67,0.16)] ring-1 ring-[#dbe6ef]">
      {error && !session ? <div className="m-6 rounded-2xl border border-[#f1c5be] bg-[#fff6f4] p-5 text-sm leading-6 text-[#9d3427]">{error}</div> : complete ? <div className="px-6 py-14 text-center sm:px-10"><span className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-[#eaf8ef] text-[#168044]"><CheckCircle2 size={34} /></span><h2 className="mt-5 text-2xl font-bold">Your documents are signed</h2><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-[#60748b]">{complete.groupComplete ? 'Every required signer has completed the secure pack. The signed documents have been saved with this listing.' : 'Your signed documents have been saved. The pack remains open while the other required signer completes their secure link.'}</p><section className="mx-auto mt-7 max-w-md rounded-2xl border border-[#dfe8f0] bg-[#f8fbfd] p-4 text-left"><p className="font-bold text-[#20374f]">Need a correction?</p><p className="mt-1 text-sm leading-5 text-[#60748b]">Tell your agent what needs correcting. Your signed record will remain unchanged; the agent can prepare a replacement pack if needed.</p>{postSignatureCorrectionRequested ? <p className="mt-3 rounded-xl bg-[#eaf8ef] p-3 text-sm font-semibold text-[#187446]">Correction request sent to your agent.</p> : <><textarea value={issue} onChange={(event) => setIssue(event.target.value)} rows={3} placeholder="Describe what needs correcting" className="mt-3 w-full rounded-xl border border-[#cfdce8] bg-white p-3 text-sm text-[#172334]" /><button type="button" onClick={() => void requestPostSignatureCorrection()} disabled={submitting} className="mt-3 rounded-xl border border-[#cfdce8] px-3 py-2 text-sm font-bold text-[#334d66] disabled:opacity-60">Request correction</button></>}</section>{error ? <p className="mx-auto mt-4 max-w-md rounded-xl border border-[#f1c5be] bg-[#fff6f4] p-3 text-left text-sm text-[#9d3427]">{error}</p> : null}</div> : <form onSubmit={submit}>
