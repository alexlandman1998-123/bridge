...
function FicaField({ label, field, details, onChange, type = 'text', optional = false }) {
  return <label className="block min-w-0 text-sm font-semibold text-[#47627b]">{label}{optional ? <span className="font-normal"> (if available)</span> : null}<input type={type} value={details[field] || ''} onChange={(event) => onChange(field, event.target.value)} className="mt-1.5 box-border w-full min-w-0 max-w-full rounded-lg border border-[#cfdce8] bg-white px-3 py-2.5 font-normal text-[#172334] outline-none focus:border-[var(--signing-primary)] focus:ring-2 focus:ring-[#dcebf4]" /></label>
}
...
export default function ListingMandateSigning() {
  ...
  const theme = { '--signing-primary': validHex(branding.primaryColour, '#173f5f'), '--signing-secondary': validHex(branding.secondaryColour, '#102a43'), '--signing-accent': validHex(branding.accentColour, '#2a9b65') }
