...
function getRelativeLuminance(hex = '#102236') {
  const { r, g, b } = hexToRgb(hex)
...
function isLightThemeColour(hex = '#ffffff') {
  return getRelativeLuminance(hex) >= 0.82
}
...
function getListingParkingCount(listing = {}) {
  return listing.garages || listing.garageCount || listing.parking || listing.parkingSpaces || listing.parkingCount || null
}
...
function buildTheme(agency = {}) {
  ...
  const accentIsLight = isLightThemeColour(accent)
  const accentSurface = accentIsLight ? secondary : accent
  const accentDisplay = accentIsLight ? secondary : accent
  return {
    primary,
    secondary,
    accent,
    accentDisplay,
    accentSurface,
    accentText: getContrastTextColour(accentSurface),
    page: `
      linear-gradient(128deg, transparent 0%, transparent 58%, ${hexToRgba(accent, accentIsLight ? 0.14 : 0.1)} 58%, ${hexToRgba(accent, accentIsLight ? 0.14 : 0.1)} 72%, transparent 72%),
      linear-gradient(145deg, ${hexToRgba(primary, 0.98)} 0%, ${hexToRgba(secondary, 0.94)} 58%, ${hexToRgba(primary, 0.98)} 100%)
    `,
...
function IntentCta({ icon: Icon, title, subtitle, href, onClick = null, tone = 'primary', stagger = false }) {
  ...
  const toneClass = tone === 'accent'
    ? 'bg-[linear-gradient(135deg,var(--card-accent-surface)_0%,var(--card-secondary)_100%)] text-[var(--card-accent-text)] shadow-[0_16px_34px_rgba(15,23,42,0.2)]'
...
function ListingCard({ listing, intakeSlug = '', onTrack = () => {} }) {
  ...
  const parkingCount = getListingParkingCount(listing)
  ...
  return (
    <article className="flex h-full w-full flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_18px_42px_rgba(15,23,42,0.1)] lg:min-h-[510px] lg:w-[min(31vw,320px)] lg:shrink-0 lg:scroll-ml-5 lg:snap-start xl:w-[320px]">
      <div className="aspect-[4/3] bg-slate-100 lg:aspect-auto lg:h-[255px]">
      ...
      <div className="flex flex-1 flex-col p-4">
        {price ? <p className="text-[1.02rem] font-semibold text-slate-950">{price}</p> : null}
        <h3 className="mt-1 line-clamp-2 min-h-[2.85rem] text-[1rem] font-semibold leading-6 text-slate-900">{listing.title || 'Property listing'}</h3>
        {location ? <p className="mt-2 truncate text-sm text-slate-500">{location}</p> : null}
        <div className="mt-3 flex flex-wrap gap-2 text-xs font-semibold text-slate-600">
          ...
          {parkingCount ? <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1"><Building2 size={13} /> {parkingCount}</span> : null}
        ...
        <a
          ...
          className="mt-4 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-lg bg-[var(--card-primary)] px-4 text-sm font-semibold text-white transition hover:brightness-110 focus:outline-none focus:ring-4 focus:ring-[var(--card-primary)]/20 lg:mt-auto"
        >
...
function CarouselButton({ direction = 'left', onClick, disabled = false }) {
  const Icon = direction === 'left' ? ArrowLeft : ArrowRight
  return (
    <button
      type="button"
      className={`hidden h-11 w-11 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-950 shadow-[0_14px_30px_rgba(15,23,42,0.14)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_36px_rgba(15,23,42,0.18)] disabled:pointer-events-none disabled:opacity-0 lg:flex ${direction === 'left' ? 'absolute left-0 top-[285px] -translate-x-1/2' : 'absolute right-0 top-[285px] translate-x-1/2'}`}
      disabled={disabled}
      aria-label={direction === 'left' ? 'Scroll listings left' : 'Scroll listings right'}
      onClick={onClick}
    >
      <Icon size={18} />
    </button>
...
export default function PublicAgentDigitalCardPage() {
  ...
  const [canScrollListingsBack, setCanScrollListingsBack] = useState(false)
  const [canScrollListingsForward, setCanScrollListingsForward] = useState(false)
  const trackedViewRef = useRef('')
  const listingsRailRef = useRef(null)
  ...
  useEffect(() => {
    ...
    if (!rail || !listings.length) {
      setCarouselProgress(0)
      setCanScrollListingsBack(false)
      setCanScrollListingsForward(false)
      return undefined
    }

    const updateCarouselState = () => {
      const maxScroll = Math.max(rail.scrollWidth - rail.clientWidth, 0)
      setCarouselProgress(maxScroll ? Math.min(rail.scrollLeft / maxScroll, 1) : 0)
      setCanScrollListingsBack(rail.scrollLeft > 4)
      setCanScrollListingsForward(rail.scrollLeft < maxScroll - 4)
    }
  ...
  const carouselProgressWidth = listings.length > 1 ? `${Math.max(18, carouselProgress * 100)}%` : '100%'
  ...
  function scrollListingsRail(direction = 1) {
    const rail = listingsRailRef.current
    if (!rail) return
    const firstCard = rail.querySelector('article')
    const scrollAmount = firstCard ? firstCard.getBoundingClientRect().width + 20 : rail.clientWidth * 0.85
  ...
  return (
    <main
      ...
      style={{
        ...
        '--card-accent-display': theme.accentDisplay,
        '--card-accent-surface': theme.accentSurface,
        '--card-accent-text': theme.accentText,
        background: theme.page,
      }}
    >
      <section className="mx-auto grid min-h-screen w-full max-w-6xl gap-6 px-4 py-5 sm:px-6 lg:max-w-[1480px] lg:grid-cols-[minmax(360px,420px)_minmax(0,1fr)] lg:items-start lg:gap-10 lg:px-12 lg:py-10">
        <aside className="overflow-hidden rounded-lg border border-white/15 bg-white shadow-[0_30px_80px_rgba(15,23,42,0.22)] lg:sticky lg:top-10 lg:flex lg:max-h-[calc(100vh-80px)] lg:flex-col">
          <div className="px-6 pb-24 pt-7 text-white lg:flex lg:min-h-[30%] lg:items-center lg:justify-center lg:pb-20 lg:pt-8" style={{ background: theme.hero }}>
          ...
          <div className="px-6 pb-6">
            <div className="-mt-20 flex flex-col items-center text-center">
              {agent.avatarUrl ? (
                <img src={agent.avatarUrl} alt="" className="h-40 w-40 rounded-full border-[6px] border-white object-cover shadow-[0_20px_44px_rgba(15,23,42,0.24)] lg:h-36 lg:w-36" />
              ) : (
                <div className="flex h-40 w-40 items-center justify-center rounded-full border-[6px] border-white bg-slate-100 text-5xl font-semibold text-[var(--card-primary)] shadow-[0_20px_44px_rgba(15,23,42,0.24)] lg:h-36 lg:w-36">
                  {agentName.slice(0, 1).toUpperCase()}
                </div>
              )}
              <h1 className="mt-5 text-[2.15rem] font-semibold leading-none text-slate-950 lg:text-[2rem]">{agentName}</h1>
              <p className="mt-3 flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-base">
                <span className="font-semibold text-[var(--card-accent-display)]">{jobTitle}</span>
        ...
        <section className="min-w-0 rounded-lg border border-white/20 bg-white/[0.1] p-5 shadow-[0_20px_60px_rgba(15,23,42,0.12)] backdrop-blur sm:p-6 lg:flex lg:flex-col lg:border-slate-200/70 lg:bg-white/95 lg:p-10 lg:text-slate-950">
          <div className="lg:flex lg:items-end lg:justify-between lg:gap-6">
            <div>
              <p className="text-xs font-semibold uppercase text-white/60 lg:text-slate-500">My Listings</p>
              <h2 className="mt-1 text-2xl font-semibold text-white/90 lg:text-slate-950">Featured Properties</h2>
            </div>
          </div>
          ...
          {listingLoading ? (
            ...
            <div className="relative mt-5 min-w-0 lg:mt-6">
              <div
                ref={listingsRailRef}
                className="grid gap-4 sm:grid-cols-2 lg:-mx-1 lg:flex lg:snap-x lg:snap-mandatory lg:gap-5 lg:overflow-x-auto lg:px-1 lg:pb-3 lg:[scrollbar-width:none] lg:[&::-webkit-scrollbar]:hidden"
              >
              ...
              {listings.length > 2 ? (
                <>
                  <CarouselButton direction="left" disabled={!canScrollListingsBack} onClick={() => scrollListingsRail(-1)} />
                  <CarouselButton direction="right" disabled={!canScrollListingsForward} onClick={() => scrollListingsRail(1)} />
                </>
              ) : null}
              {listings.length > 1 ? (
                <div className="mx-auto mt-2 hidden h-1 w-48 overflow-hidden rounded-full bg-slate-200 lg:block">
                  <div className="h-full rounded-full bg-white transition-all duration-300 lg:bg-slate-950" style={{ width: carouselProgressWidth }} />
                </div>
              ) : null}
            </div>
          ) : (
          ...
          <footer className="mt-8 flex flex-col gap-3 border-t border-white/20 pt-5 text-sm text-white/70 sm:flex-row sm:items-center sm:justify-between lg:border-slate-200 lg:text-slate-500">
            <span>{email || phone || agency.contactEmail || agency.contactPhone}</span>
            <span className="font-semibold text-white/90 lg:text-slate-700">Powered by ARCH9</span>
          </footer>
        </section>
      </section>
    </main>
