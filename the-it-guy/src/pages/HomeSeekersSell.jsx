...
const principles = [
  ['01', 'Start with clarity', 'A free comparative market assessment is only a few clicks away.', 'Get a valuation'],
  ['02', 'Market with purpose', 'Strong presentation and targeted marketing put your property in front of the right buyers.', 'See how we market'],
  ['03', 'A team behind you', 'Personal service, local knowledge and a clear plan from first conversation to sold.', 'Meet our people'],
]
...
function HomeSeekersSell() {
  ...
  return <main className="hs-site hs-sell-site">
    <header className={`hs-header hs-sell-header${headerScrolled ? ' hs-sell-header--scrolled' : ''}`}><a className="hs-logo" href="/demo/homeseekers"><img src="/brand/homeseekers/logo.png" alt="Home Seekers" /></a><nav className="hs-nav" aria-label="Main navigation">{nav.map((item, index) => <a className={item === 'Sell' ? 'is-active' : ''} href={item === 'Buy' ? '/demo/homeseekers/buy' : item === 'Sell' ? '/demo/homeseekers/sell' : item === 'Rent' ? '/demo/homeseekers/rent' : item === 'Developments' ? '/demo/homeseekers/developments' : item === 'Our people' ? '/demo/homeseekers/people' : item === 'About' ? '/demo/homeseekers/about' : item === 'Contact' ? '/demo/homeseekers/contact' : '/demo/homeseekers'} key={item}>{item}{index < 3 && <ChevronDown size={13} />}</a>)}</nav></header>
    <section className="hs-sell-hero"><div><p className="hs-eyebrow">Selling with Home Seekers</p><h1>Your home.<br />Properly represented.</h1><p>Clear strategy, intelligent marketing and people who know how to turn attention into the right offer.</p><a href="#valuation">Get a valuation <ArrowRight size={17} /></a></div></section>
    <section className="hs-sell-promise"><div><p className="hs-eyebrow">The Home Seekers approach</p><h2>Start with<br />the right<br />number.</h2><p>A free comparative market assessment gives you the confidence to decide what comes next.</p></div><div className="hs-sell-principles">{principles.map(([number, title, copy, action]) => <a href="#valuation" key={number}><span>{number}</span><h3>{title}</h3><p>{copy}</p><b>{action} <ArrowRight size={15} /></b></a>)}</div></section>
