...
import { isSharedInboxSchemaUnavailable, loadRevoInbox, saveRevoInboxPrivateMessage, updateRevoInboxConversation } from '../../modules/revo/sharedInbox/revoSharedInboxService'
...
const VIEWS = Object.freeze([{ key: 'all', label: 'All conversations' }, { key: 'unassigned', label: 'Unassigned' }, { key: 'mine', label: 'Assigned to me' }, { key: 'closed', label: 'Closed' }])
...
export default function RevoSharedInboxPage() {
  ...
  const [data, setData] = useState({ conversations: [], messages: [], activity: [] })
  ...
  return <main className="revo-inbox-page">
    ...
    <section className="revo-inbox-shell" aria-label="Revo shared inbox">
      ...
      <section className="revo-list"><header><div><strong>{VIEWS.find((item) => item.key === view)?.label}</strong><span>{visible.length} conversations</span></div><button className="revo-icon" type="button" aria-label="Filter conversations"><SlidersHorizontal size={18} /></button></header><div>{mode === 'loading' ? <p className="revo-empty"><Clock3 size={22} /> Loading conversations</p> : visible.map((item) => <button type="button" className={`revo-row ${item.id === selected?.id ? 'selected' : ''}`} key={item.id} onClick={() => { setSelectedId(item.id); setNotice('') }}><span className={`revo-channel ${item.channel}`}><ChannelIcon channel={item.channel} /></span><span><span><strong>{item.name}</strong><time>{item.updated}</time></span><em>{item.subject}</em><small>{item.preview}</small></span>{item.unread ? <i /> : null}</button>)}{mode !== 'loading' && !visible.length ? <p className="revo-empty"><Inbox size={22} /> No conversations found</p> : null}</div></section>
