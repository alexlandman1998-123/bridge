...
import { Archive, CalendarDays, CheckCircle2, ChevronDown, Clock3, FileText, Inbox, Mail, MapPin, MessageCircle, MoreHorizontal, Paperclip, Plus, Search, Send, SlidersHorizontal, Sparkles, UserRound } from 'lucide-react'
...
export default function RevoSharedInboxPage() {
  ...
  return <main className="revo-inbox-page">
    ...
    <section className="revo-inbox-shell" aria-label="Revo shared inbox">
      ...
      {selected ? <aside className="revo-details"><div className="revo-details-heading"><strong>Contact</strong><button type="button">Edit</button></div><div className="revo-contact"><div className="revo-avatar">{selected.name.split(' ').map((part) => part[0]).join('').slice(0, 2)}</div><div><h2>{selected.name}</h2><p><Mail size={14} /> {selected.address}</p><p><MapPin size={14} /> Contact location not added</p></div></div><dl><div><dt>Lead owner</dt><dd><button className="revo-assignment" type="button" disabled={saving} onClick={() => void performUpdate({ assignedUserId: selected.assignedUserId ? null : user?.id }, selected.assignedUserId ? 'Conversation unassigned.' : 'Conversation assigned to you.')}>{selected.assignedTo} <ChevronDown size={14} /></button></dd></div><div><dt>Buyer qualification <span>0 of 3</span></dt><dd className="revo-progress"><i /><i /><i /></dd></div><div><dt>Status</dt><dd><select aria-label="Conversation status" value={selected.status} disabled={saving} onChange={(event) => void performUpdate({ status: event.target.value }, 'Conversation status updated.')}><option value="open">Open</option><option value="waiting_on_us">Waiting on us</option><option value="waiting_on_client">Waiting on client</option><option value="closed">Closed</option><option value="spam">Spam</option></select></dd></div></dl><section><div className="revo-section-heading"><strong>Linked records</strong><button type="button">Edit</button></div>{selectedAssociations.length ? <ul>{selectedAssociations.map((association) => <li key={association.id}><FileText size={15} /> {association.isPrimary ? 'Primary ' : ''}{association.entityType}</li>)}</ul> : <p>No listing or transaction linked yet.</p>}</section><section><strong>Quick actions</strong><div className="revo-quick-actions"><button type="button" disabled={saving} onClick={() => void requestWorkflowAction('create_viewing')}><CalendarDays size={15} /> Create viewing</button><button type="button" disabled={saving} onClick={() => void requestWorkflowAction('create_transaction')}><FileText size={15} /> Create transaction</button></div></section></aside> : null}
    </section>
  </main>
}
