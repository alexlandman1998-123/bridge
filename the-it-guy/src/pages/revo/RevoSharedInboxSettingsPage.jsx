...
export default function RevoSharedInboxSettingsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setMode('loading')
    try {
      setChannels(await loadRevoInboxChannels(organisation.id))
      setMode('live')
    } catch (error) {
  ...
  return <main className="revo-channel-settings">
    ...
    <section className="revo-channel-grid" aria-label="Configured Revo inbox channels">
      ...
      {channels.map((channel) => {
        ...
        return <article key={channel.id}><div className={`revo-channel-icon ${channel.channel}`}><ChannelIcon channel={channel.channel} /></div><div className="revo-channel-card-head"><div><h2>{channel.displayName || channel.address}</h2><p>{channel.channel === 'whatsapp' ? 'WhatsApp' : 'Email'} · {REVO_INBOX_CHANNEL_PROVIDERS[channel.channel]?.find((item) => item.key === channel.providerKey)?.label || channel.providerKey}</p></div><span className={`revo-channel-status ${status.className}`}><status.Icon size={14} /> {status.label}</span></div><p className="revo-channel-address">{channel.address}</p><footer><button type="button" disabled={saving || channel.isDefault} onClick={() => void setDefault(channel.id)}>{channel.isDefault ? 'Default channel' : 'Make default'}</button><button type="button" disabled={saving} onClick={() => void togglePause(channel)}>{channel.connectionStatus === 'paused' ? 'Resume setup' : 'Pause'}</button></footer></article>
      })}
    </section>
