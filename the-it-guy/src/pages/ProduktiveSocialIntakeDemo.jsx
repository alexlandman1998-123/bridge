...
import {
  ...
  readProduktiveDemoState,
  resetProduktiveDemoState,
  sellerFeatureOptions,
  sellerPriceOptions,
} from '../lib/produktiveSocialIntakeDemo'
...
const PRODUKTIVE_LANDING_BACKGROUND =
...
function DemoShell({ children, compact = false }) {
  return (
    <main className="min-h-screen bg-[#f5f6f8] text-[#28256f] [letter-spacing:0]">
      <div
        className={classNames(
          'mx-auto flex min-h-screen w-full flex-col',
          compact
            ? 'max-w-[1120px] px-4 py-5 sm:px-5'
            : 'max-w-[430px] bg-white px-4 py-4 shadow-[0_24px_80px_rgba(40,37,111,0.08)] sm:my-5 sm:min-h-[860px] sm:rounded-[30px] sm:border sm:border-[#dce1e7]',
        )}
      >
        {children}
      </div>
    ...
    <header className="flex items-center justify-between gap-3 py-2">
      <div className="flex min-w-0 items-center gap-3">
        {onBack ? (
          <button
            type="button"
            className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-[#dce1e7] bg-white text-[#28256f] shadow-sm transition hover:border-[#d1ad61]"
            onClick={onBack}
            aria-label="Back"
          >
            <ArrowLeft size={18} />
        ...
        <div className="min-w-0">
          ...
          <p className="truncate text-sm font-black text-[#28256f] [letter-spacing:0]">Produktive Realty</p>
        </div>
      </div>
      {showAdminLink ? (
        <Link
          className="inline-flex h-10 items-center gap-2 rounded-lg border border-[#dce1e7] bg-white px-3 text-xs font-bold text-[#28256f] shadow-sm transition hover:border-[#d1ad61]"
          to="/demo/produktive-social-intake/admin"
        >
...
function LandingView({ onSelect }) {
  return (
    <DemoShell>
      <section className="-mx-4 -my-4 relative isolate flex min-h-[100dvh] flex-col overflow-hidden bg-[#17164a] px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-[max(1.25rem,env(safe-area-inset-top))] text-white sm:min-h-[860px] sm:rounded-[30px] sm:px-7 sm:py-7">
        <div
          aria-hidden
          className="absolute inset-0 bg-cover bg-center"
          style={{ backgroundImage: `url("${PRODUKTIVE_LANDING_BACKGROUND}")` }}
        />
        <div aria-hidden className="absolute inset-0 bg-[linear-gradient(180deg,rgba(23,22,74,0.34)_0%,rgba(23,22,74,0.76)_42%,rgba(23,22,74,0.96)_100%)]" />
        ...
        <div className="relative z-10 flex flex-1 flex-col">
          <section className="pt-[11vh] sm:pt-24">
            <p className="text-xs font-black uppercase text-[#d1ad61] [letter-spacing:0]">Produktive Realty</p>
            <h1 className="mt-3 max-w-[340px] text-[2.6rem] font-black leading-[0.95] text-white [letter-spacing:0]">
              Find the right move, faster.
            </h1>
            <p className="mt-5 max-w-[330px] text-sm font-medium leading-6 text-[#ece4d4] [letter-spacing:0]">
              Start with one quick choice. Produktive will match buyers to suitable homes or route sellers to a local valuation specialist.
            </p>
          ...
          <section className="mt-auto grid gap-3 pb-5 pt-8">
            <IntentCard
              icon={Search}
              title="I want to buy"
              description="Share your area, budget, and must-haves. We will show the closest Produktive matches."
              accent="buyer"
  ...
  return (
    <button
      ...
      className={classNames(
        ...
        isBuyer
          ? 'border-[#d1ad61]/38 bg-[#28256f]/84 text-white'
          : 'border-white bg-white text-[#28256f]',
      )}
      onClick={onClick}
    >
      <span
        className={classNames(
          ...
          isBuyer ? 'bg-[#d1ad61] text-[#28256f]' : 'bg-[#28256f] text-[#d1ad61]',
        )}
      >
        {createElement(Icon, { size: 22 })}
      </span>
      <span className="min-w-0 flex-1">
        <span className={classNames('block text-base font-black leading-tight [letter-spacing:0]', isBuyer ? 'text-white' : 'text-[#28256f]')}>
          {title}
        </span>
        <span className={classNames('mt-1.5 block text-xs font-semibold leading-5 [letter-spacing:0]', isBuyer ? 'text-[#ece4d4]' : 'text-[#556273]')}>
          {description}
        </span>
      </span>
      <span className={classNames(
        'inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full transition group-hover:translate-x-0.5',
        isBuyer ? 'bg-white/12 text-[#d1ad61]' : 'bg-[#d1ad61] text-[#28256f]',
      )}>
          ...
          <div key={step} className="flex items-center" aria-label={step}>
            <span
              className={classNames(
                ...
                index <= current ? 'bg-[#d1ad61]' : 'bg-[#dfe3e8]',
              )}
            />
            {index < steps.length - 1 ? (
              <span className={classNames('h-0.5 w-10', index < current ? 'bg-[#d1ad61]' : 'bg-[#dfe3e8]')} />
            ) : null}
          </div>
        ))}
      </div>
...
function FlowHeader({ title, subtitle, steps, current, onBack }) {
  return (
    <section className="pt-2">
      <button
        type="button"
        className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-[#e1e6ec] bg-white text-[#28256f] shadow-sm transition hover:border-[#d1ad61]"
        onClick={onBack}
        aria-label="Back"
      >
        <ArrowLeft size={16} />
      ...
      <h1 className="mt-9 text-xl font-black leading-tight text-[#28256f] [letter-spacing:0]">{title}</h1>
  ...
  return (
    <button
      ...
      className={classNames(
        ...
        selected
          ? 'border-[#28256f] bg-[#28256f] text-white shadow-[0_12px_24px_rgba(40,37,111,0.16)]'
    ...
    >
      <span>{children}</span>
      {selected ? (
        <span className="grid h-5 w-5 place-items-center rounded-full bg-[#d1ad61] text-[#28256f]">
        ...
        return (
          <button
            ...
            className={classNames(
              ...
              isSelected
                ? 'border-[#28256f] bg-[#28256f] text-white shadow-[0_10px_20px_rgba(40,37,111,0.14)]'
                : 'border-[#edf0f3] bg-[#f3f4f6] text-[#273447] hover:border-[#cfd6de] hover:bg-white',
            )}
            onClick={() => onToggle(option)}
          >
            {isSelected ? <Check size={13} className="text-[#d1ad61]" strokeWidth={3} /> : null}
            {option}
          </button>
        )
      })}
...
function SelectablePropertyCard({ property, selected, onToggle }) {
  return (
    <button
      type="button"
      className={classNames(
        'group w-full overflow-hidden rounded-xl border bg-white text-left shadow-[0_12px_30px_rgba(40,37,111,0.08)] transition hover:-translate-y-0.5',
        selected
          ? 'border-[#d1ad61] ring-4 ring-[#d1ad61]/18'
          : 'border-[#e1e6ec] hover:border-[#d1ad61]/70',
      )}
      onClick={onToggle}
    ...
    >
      <div className="grid grid-cols-[112px_minmax(0,1fr)]">
        <div className="relative min-h-[146px] overflow-hidden bg-[#dce1e7]">
          ...
          />
          {selected ? (
            <span className="absolute left-2 top-2 rounded-full bg-[#d1ad61] px-2 py-1 text-[10px] font-black uppercase text-[#28256f]">
              Selected
            </span>
          ) : null}
        </div>
        <div className="min-w-0 p-3">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <h3 className="text-sm font-black leading-tight text-[#28256f] [letter-spacing:0]">{property.title}</h3>
            ...
            <span
              className={classNames(
                ...
                selected ? 'border-[#d1ad61] bg-[#d1ad61] text-[#28256f]' : 'border-[#cbd3dc] bg-white text-transparent',
              )}
              aria-hidden="true"
            >
          ...
          <p className="mt-2 text-lg font-black text-[#28256f] [letter-spacing:0]">{formatRand(property.price)}</p>
...
function Field({ label, value, onChange, type = 'text', placeholder = '', inputMode }) {
  return (
    <label className="block">
      ...
      <input
        className="mt-2 h-12 w-full rounded-lg border border-[#e1e6ec] bg-white px-3 text-sm font-semibold text-[#28256f] outline-none transition [letter-spacing:0] placeholder:text-[#a0a8b2] focus:border-[#d1ad61] focus:ring-4 focus:ring-[#d1ad61]/15"
        type={type}
        value={value}
        inputMode={inputMode}
...
function SelectField({ label, value, onChange, options, placeholder = 'Select one' }) {
  return (
    <label className="block">
      <span className="text-xs font-bold text-[#657181] [letter-spacing:0]">{label}</span>
      <select
        className="mt-2 h-12 w-full rounded-lg border border-[#e1e6ec] bg-white px-3 text-sm font-semibold text-[#28256f] outline-none transition [letter-spacing:0] focus:border-[#d1ad61] focus:ring-4 focus:ring-[#d1ad61]/15"
        value={value}
        onChange={(event) => onChange(event.target.value)}
      >
  ...
  return (
    <div className="mt-auto grid gap-3 pb-4 pt-8">
      <button
        type="button"
        className={classNames(
          'inline-flex h-12 items-center justify-center gap-2 rounded-lg px-4 text-sm font-black shadow-[0_16px_28px_rgba(40,37,111,0.18)] transition disabled:cursor-not-allowed disabled:bg-[#cbd2da] disabled:text-white disabled:shadow-none [letter-spacing:0]',
          isLast ? 'bg-[#d1ad61] text-[#28256f] hover:bg-[#e1bf78]' : 'bg-[#28256f] text-white hover:bg-[#343080]',
        )}
        onClick={onNext}
      ...
      <button
        type="button"
        className="inline-flex h-10 items-center justify-center gap-2 rounded-lg px-4 text-xs font-bold text-[#657181] transition hover:text-[#28256f]"
        onClick={onBack}
      >
        <ArrowLeft size={14} />
        Back
...
function buyerStepSubtitle(step) {
  return [
    'Choose the suburb Produktive should prioritise first.',
    ...
    'Select one or more properties. Produktive will use this to match you with these homes or similar options.',
  ][step] || ''
}
  ...
  return (
    <>
      <section className="pt-2">
        ...
        <div className="mt-5 inline-flex items-center gap-2 rounded-lg bg-[#28256f] px-3 py-2 text-xs font-black text-white">
          <Check size={15} />
          Buyer Lead Created
        </div>
        <h1 className="mt-4 text-2xl font-black leading-tight text-[#28256f] [letter-spacing:0]">{resultMessage}</h1>
      ...
      <section className="mt-5 rounded-xl border border-[#e1e6ec] bg-white p-4 shadow-[0_12px_30px_rgba(40,37,111,0.06)]">
        <h2 className="text-sm font-black text-[#28256f] [letter-spacing:0]">Buyer request summary</h2>
        ...
        <div className="mt-2 rounded-lg border border-[#e1e6ec] bg-white px-3 py-2">
          ...
          <p className="mt-1 text-sm font-black text-[#28256f] [letter-spacing:0]">
      ...
      <section className="mt-5 rounded-xl border border-[#e1e6ec] bg-white p-4 shadow-[0_12px_30px_rgba(40,37,111,0.06)]">
        <h2 className="text-sm font-black text-[#28256f] [letter-spacing:0]">Homes you selected</h2>
        {selectedHomes.length ? (
          ...
          <p className="mt-3 rounded-lg border border-dashed border-[#cbd3dc] bg-[#f8fafc] px-3 py-3 text-xs font-bold leading-5 text-[#657181] [letter-spacing:0]">
            No specific homes selected. Produktive will match you with suitable options based on your preferences.
          </p>
        )}
      </section>
      ...
      <section className="mt-5 grid gap-4 pb-8">
        <div>
          <p className="text-xs font-black uppercase text-[#d1ad61] [letter-spacing:0]">Match result</p>
          <h2 className="mt-1 text-xl font-black text-[#28256f] [letter-spacing:0]">Closest Produktive matches</h2>
        </div>
        {matches.map((listing) => (
          <PropertyCard
            key={listing.id}
      ...
      <div className="grid gap-3 pb-8">
        <button
          type="button"
          className="inline-flex h-12 items-center justify-center gap-2 rounded-lg border border-[#dce1e7] bg-white px-4 text-sm font-black text-[#28256f] shadow-sm transition hover:border-[#d1ad61]"
          onClick={onRestart}
        >
          <Search size={16} />
          New buyer search
        ...
        <Link
          className="inline-flex h-12 items-center justify-center gap-2 rounded-lg bg-[#28256f] px-4 text-sm font-black text-white shadow-[0_16px_28px_rgba(40,37,111,0.18)] transition hover:bg-[#343080]"
          to="/demo/produktive-social-intake/admin"
        >
...
function CompactSelectedPropertyCard({ property }) {
  return (
    <article className="grid grid-cols-[68px_minmax(0,1fr)] gap-3 rounded-lg border border-[#eef2f6] bg-[#f8fafc] p-2">
      ...
      <div className="min-w-0">
        <p className="truncate text-sm font-black text-[#28256f] [letter-spacing:0]">{property.title}</p>
        <p className="mt-1 text-[11px] font-bold text-[#657181] [letter-spacing:0]">
          {(property.suburb || property.area)} · {formatRand(property.price)}
        </p>
        <div className="mt-1 flex flex-wrap gap-2 text-[11px] font-bold text-[#28256f] [letter-spacing:0]">
...
function ReceiptItem({ label, value }) {
  return (
    <div className="rounded-lg border border-[#e1e6ec] bg-white px-3 py-2">
      ...
      <p className="mt-1 text-sm font-black text-[#28256f] [letter-spacing:0]">{value || '-'}</p>
    </div>
  )
}

const propertyArtworkThemes = [
  { sky: '#b8d4f0', lawn: '#294b35', house: '#f3efe7', roof: '#0a1f36', accent: '#d1ad61', pool: '#2aaed6' },
  { sky: '#d8d2c8', lawn: '#39513b', house: '#d7c4ab', roof: '#17283a', accent: '#c89b34', pool: '#8bbccf' },
  { sky: '#cdd8e5', lawn: '#273f2e', house: '#ece6dc', roof: '#5b4732', accent: '#d1ad61', pool: '#4ca8c8' },
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-[27%] left-[13%] h-[28%] w-[70%] rounded-sm shadow-[0_18px_34px_rgba(0,0,0,0.22)]" style={{ backgroundColor: theme.house }}>
        ...
        <div className="absolute bottom-[16%] left-[10%] h-[44%] w-[16%] rounded-sm bg-[#28256f]/80" />
        <div className="absolute bottom-[16%] left-[32%] h-[44%] w-[16%] rounded-sm bg-[#28256f]/70" />
        <div className="absolute bottom-[16%] left-[54%] h-[44%] w-[16%] rounded-sm bg-[#28256f]/70" />
...
function PropertyCard({ listing, isSaved, infoRequested, onBookViewing, onRequestInfo, onToggleSaved }) {
  ...
  return (
    <article
      data-property-card
      className="overflow-hidden rounded-lg border border-[#28256f] bg-white shadow-[0_16px_34px_rgba(40,37,111,0.12)]"
    >
      <div className="relative aspect-[1.22] overflow-hidden bg-[#dce1e7]">
        ...
        <button
          ...
          className={classNames(
            ...
            isSaved ? 'text-[#d1ad61]' : 'hover:bg-black/35',
          )}
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xl font-black text-[#28256f] [letter-spacing:0]">{formatRand(listing.price)}</p>
            <p className="mt-1 inline-flex items-center gap-1 text-xs font-bold text-[#657181] [letter-spacing:0]">
              <MapPin size={14} />
              {listing.area}
            </p>
          </div>
          <p className="rounded-full bg-[#eef2f6] px-3 py-1 text-[11px] font-black text-[#28256f]">{listing.propertyType}</p>
        </div>

        <div className="mt-4 flex items-center gap-5 text-[#28256f]">
        ...
        <div className="mt-4 grid gap-2">
          <button
            type="button"
            className="inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-[#28256f] px-3 text-xs font-black text-white transition hover:bg-[#343080]"
          ...
          <div className="grid grid-cols-2 gap-2">
            <a
              className="inline-flex h-11 items-center justify-center gap-2 rounded-lg border border-[#dce1e7] bg-white px-3 text-xs font-black text-[#28256f] transition hover:border-[#d1ad61]"
              href={whatsappUrl}
              target="_blank"
              rel="noreferrer"
            >
...
function Metric({ icon: Icon, value, label }) {
  return (
    <div className="inline-flex items-center gap-1 text-xs font-bold text-[#28256f] [letter-spacing:0]">
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#031223]/70 px-4 py-4 sm:items-center">
      <div className="w-full max-w-[430px] rounded-lg bg-white shadow-[0_28px_70px_rgba(3,18,35,0.32)]">
        <div className="flex items-center justify-between border-b border-[#e1e6ec] px-4 py-4">
          <div>
            <p className="text-xs font-black uppercase text-[#d1ad61] [letter-spacing:0]">Book viewing</p>
            <h2 className="mt-1 text-lg font-black text-[#28256f] [letter-spacing:0]">{listing.title}</h2>
          </div>
          <button
            type="button"
            className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-[#dce1e7] text-[#657181] transition hover:border-[#d1ad61] hover:text-[#28256f]"
            onClick={onClose}
        ...
        {created ? (
          <section className="px-4 py-5">
            <span className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-[#d1ad61] text-[#28256f]">
              <Check size={24} />
            </span>
            <h3 className="mt-4 text-2xl font-black leading-tight text-[#28256f] [letter-spacing:0]">
              Your viewing request has been sent to Produktive.
            </h3>
            ...
            <div className="mt-5 grid gap-2">
              {['Lead captured', 'Viewing request created', 'Agent notified'].map((item) => (
                <div key={item} className="flex items-center gap-3 rounded-lg border border-[#e1e6ec] px-3 py-3">
                  ...
                  <span className="text-sm font-bold text-[#28256f] [letter-spacing:0]">{item}</span>
                </div>
              ))}
            </div>
            <button
              type="button"
              className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-[#28256f] px-4 text-sm font-black text-white"
              onClick={onClose}
            >
              Done
            </button>
          ...
          <section className="px-4 py-5">
            ...
            <button
              type="button"
              className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-[#d1ad61] px-4 text-sm font-black text-[#28256f] transition hover:bg-[#e1bf78] disabled:cursor-not-allowed disabled:bg-[#cbd2da] disabled:text-white"
...
function sellerStepSubtitle(step) {
  return [
    'Produktive will use this to route the valuation to the right area expert.',
    ...
    'Pick a convenient time for the Produktive team to confirm.',
  ][step] || ''
}

function SellerSuccess({ sellerLead, valuationRequest, onExit }) {
  return (
    <section className="-mx-4 -my-4 flex min-h-screen flex-col bg-[radial-gradient(circle_at_50%_0%,#0e5a4b_0%,#28256f_46%,#00372f_100%)] px-4 py-8 text-white sm:min-h-[860px] sm:rounded-[30px]">
      <div className="mx-auto mt-5 grid h-20 w-20 place-items-center rounded-full border border-[#d1ad61] text-[#d1ad61]">
      ...
      <section className="mt-7 rounded-lg bg-white px-4 py-4 text-[#28256f] shadow-[0_18px_44px_rgba(0,0,0,0.16)]">
        <div className="inline-flex w-fit items-center gap-2 rounded-lg bg-[#28256f] px-3 py-2 text-xs font-black text-white">
          <ShieldCheck size={15} className="text-[#d1ad61]" />
          Seller Lead Created
        </div>
      ...
      <section className="mt-5 rounded-lg bg-white px-4 py-4 text-[#28256f] shadow-[0_18px_44px_rgba(0,0,0,0.12)]">
        <h2 className="text-sm font-black text-[#28256f] [letter-spacing:0]">Here's what happens next:</h2>
        ...
        {[
          ...
          <div key={item[0]} className="flex items-center gap-3">
            <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#eef2f6] text-[#28256f]">
              <Check size={16} className="text-[#d1ad61]" />
            </span>
            <span>
              <span className="block text-xs font-black text-[#28256f] [letter-spacing:0]">{item[0]}</span>
      ...
      <div className="mt-auto grid gap-3 pt-6">
        <button
          type="button"
          className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-white px-4 text-sm font-black text-[#28256f] shadow-[0_16px_28px_rgba(0,0,0,0.14)] transition hover:bg-[#d1ad61]"
          onClick={onExit}
        >
          Back to home
        </button>
        <Link
          className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-lg border border-white/25 px-4 text-sm font-black text-white transition hover:border-[#d1ad61]"
          to="/demo/produktive-social-intake/admin"
        >
          <ClipboardCheck size={16} />
          View admin preview
...
function AdminPreview() {
  const [demoState, setDemoState] = useState(() => readProduktiveDemoState())

  function resetDemo() {
    setDemoState(resetProduktiveDemoState())
  }
  ...
  return (
    <DemoShell compact>
      ...
      <section className="mt-5 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs font-black uppercase text-[#d1ad61] [letter-spacing:0]">CRM-ready records</p>
          <h1 className="mt-2 text-3xl font-black leading-tight text-[#28256f] [letter-spacing:0]">
            Produktive social intake admin
          </h1>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        <Link
          className="inline-flex h-12 items-center justify-center gap-2 rounded-lg bg-[#28256f] px-4 text-sm font-black text-white shadow-[0_16px_28px_rgba(40,37,111,0.18)] transition hover:bg-[#343080]"
          to="/demo/produktive-social-intake"
        >
          <ArrowLeft size={16} />
          Back to intake
        </Link>
...
function AdminStat({ label, value }) {
  return (
    <div className="rounded-lg border border-[#dce1e7] bg-white px-4 py-3 text-center shadow-sm">
      <p className="text-2xl font-black text-[#28256f] [letter-spacing:0]">{value}</p>
...
function SectionTitle({ icon: Icon, title }) {
  return (
    <div className="flex items-center gap-3">
      <span className="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-[#28256f] text-[#d1ad61]">
        {createElement(Icon, { size: 18 })}
      </span>
      <h2 className="text-xl font-black text-[#28256f] [letter-spacing:0]">{title}</h2>
    </div>
  )
}
...
function AdminLeadCard({ children }) {
  return (
    <article className="grid gap-2 rounded-lg border border-[#dce1e7] bg-white p-4 shadow-[0_12px_30px_rgba(40,37,111,0.06)]">
      {children}
    </article>
  )
}
...
function AdminRow({ label, value }) {
  return (
    <div className="flex items-start justify-between gap-3 border-b border-[#eef2f6] py-2 last:border-b-0">
      ...
      <span className="max-w-[62%] text-right text-sm font-black text-[#28256f] [letter-spacing:0]">{value || '-'}</span>
    </div>
  )
}
...
export default function ProduktiveSocialIntakeDemo({ view = 'intake', initialMode = 'landing' }) {
  const navigate = useNavigate()
  const mode = ['buyer', 'seller'].includes(initialMode) ? initialMode : 'landing'
  const homePath = '/demo/produktive-social-intake'
