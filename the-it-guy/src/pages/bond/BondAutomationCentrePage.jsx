    ...
    <div className="overflow-x-auto [scrollbar-width:thin]">
      <table className="w-full min-w-[920px] divide-y divide-slate-200 text-left text-sm">
        ...
        <tbody className="divide-y divide-slate-100">
          {rows.map((row, index) => (
            <tr key={row.id || row.ruleId || `${row.name || row.title || 'row'}-${index}`} className="align-top transition hover:bg-slate-50/70">
              {columns.map((column) => (
...
export default function BondAutomationCentrePage() {
  ...
  const isSettingsEmbedded = location.pathname.startsWith('/settings')
  const Shell = isSettingsEmbedded ? 'div' : 'main'
  const shellClassName = isSettingsEmbedded
    ? 'space-y-6 text-slate-950'
    : 'min-h-screen bg-slate-50 px-4 py-8 text-slate-950 sm:px-5 lg:px-8'
  const contentClassName = isSettingsEmbedded ? 'space-y-6' : 'mx-auto max-w-[1440px] space-y-8'
