    ...
    <div className="overflow-x-auto [scrollbar-width:thin]">
      <table className="w-full min-w-[920px] divide-y divide-slate-200 text-left text-sm">
        ...
        <tbody className="divide-y divide-slate-100">
          {rows.map((row, index) => (
            <tr key={row.id || row.branchId || row.partnerId || row.type || `${row.periodDays || 'row'}-${index}`} className="align-top transition hover:bg-slate-50/70">
              {columns.map((column) => (
      ...
      <div className="mx-auto max-w-[1440px] space-y-8">
        ...
        <section className="grid gap-4 lg:grid-cols-4">
          {Object.entries(dashboard.executive).map(([key, value]) => (
            <article key={key} className="min-h-[132px] rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
