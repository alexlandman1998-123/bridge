...
export default function PublicDevelopmentResponsiveRoute() {
  ...
  const developmentLogoUrls = new Set(
    [media.developmentLogoUrl, media.developmentLogoLightUrl, media.developmentLogoDarkUrl]
      .map(text)
      .filter(Boolean),
  );
  const heroCandidate = media.coverImageUrl || media.heroImageUrl || "";
  const hero = developmentLogoUrls.has(text(heroCandidate)) ? "" : heroCandidate;
  const images = [
    ...new Set(
      [...list(media.galleryImageUrls), ...list(media.imageUrls), hero]
        .filter((url) => Boolean(url) && !developmentLogoUrls.has(text(url))),
    ),
  ];
  const enquiry =
