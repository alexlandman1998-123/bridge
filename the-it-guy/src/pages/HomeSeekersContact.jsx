...
function HomeSeekersContact() {
  ...
  return <main className="hs-site hs-contact-site">
    ...
    <section className="hs-contact-details"><div className="hs-contact-methods"><a href="tel:+27128803127"><Phone size={22} /><span><small>Call us</small><strong>+27 12 880 3127</strong></span><ArrowRight size={18} /></a><a href="mailto:info@homeseeker.co.za"><Mail size={22} /><span><small>Email us</small><strong>info@homeseeker.co.za</strong></span><ArrowRight size={18} /></a><div><MapPin size={22} /><span><small>Head office</small><strong>786 Witdoring Avenue, Moreleta Park, Pretoria</strong></span></div></div><form className="hs-contact-form" onSubmit={submit}><div><p className="hs-eyebrow">Send an enquiry</p><h2>How can we help?</h2></div>{sent ? <div className="hs-contact-success"><strong>Thank you.</strong><p>This demo has recorded your enquiry on screen only.</p><button type="button" onClick={() => setSent(false)}>Send another enquiry <ArrowRight size={16} /></button></div> : <><label>Your name<input required name="name" placeholder="Your full name" /></label><label>Email address<input required type="email" name="email" placeholder="you@email.com" /></label><label>I’m interested in<select name="interest" defaultValue=""><option value="" disabled>Select an option</option><option>Buying a home</option><option>Selling a home</option><option>Renting a home</option><option>Something else</option></select></label><label>Tell us a little more<textarea name="message" placeholder="What can we help with?" rows="4" /></label><button type="submit">Preview enquiry <ArrowRight size={17} /></button></>}</form></section>
