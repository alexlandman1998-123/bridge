...
const nameOf = (application = {}) => {
  ...
  return (
    ...
    text(application.data?.tenantName) ||
    "Applicant pending"
  );
};
...
const statusTone = (status) =>
  status === "approved"
    ...
    : status === "declined"
      ? "border-[#f1cdc8] bg-[#fff5f4] text-[#9f3028]"
      : ["submitted", "under_review", "screening"].includes(status)
        ? "border-[#d8e5f5] bg-[#eff6ff] text-[#2563a4]"
        : "border-[#efdcb7] bg-[#fff9ec] text-[#8a641d]";
const applicationImage = (application = {}) => {
...
const consentLabel = (value) =>
  ({
    ...
    credit_check: "Credit check consent",
  ...
  return (
    {
      draft: [
        ...
        "Complete the applicant details and submit when ready.",
      ],
      submitted: [
        "Review submitted documents",
        "Check the documents and begin screening.",
      ],
      under_review: [
        "Complete screening review",
        "Confirm the outstanding screening checks.",
      ],
      screening: [
        "Complete screening review",
        "Confirm the outstanding screening checks.",
      ...
      approved: [
        "Prepare lease workflow",
        "Convert this approved application into a tenancy.",
      ],
      declined: ["Decision recorded", "This application has been declined."],
    }[text(status)] || [
      "Review application",
      "Review the current application information.",
    ]
  );
}
...
function Signal({ icon, label, value, detail }) {
  return (
    <div className="flex items-center gap-3 border-b border-[#edf2f7] px-4 py-3 last:border-b-0">
      <span className="grid h-8 w-8 place-items-center rounded-lg bg-[#f5f8fb] text-[#315f8f]">
        {createElement(icon, { size: 15 })}
      </span>
      <div className="min-w-0 flex-1">
        <p className="text-xs font-semibold text-[#24384f]">{label}</p>
        <p className="mt-0.5 text-[.68rem] text-[#718399]">{detail}</p>
      </div>
      <p className="shrink-0 text-right text-xs font-semibold text-[#36516e]">
        {value}
      </p>
    </div>
  );
...
export default function RentalApplicationDetailPage() {
  ...
  const organisationId = useMemo(
    () =>
      text(
        workspace.workspace?.id || workspace.currentMembership?.organisation_id,
      ),
    [workspace],
  );
  ...
  if (loading)
    return (
      <main className="mx-auto grid min-h-56 w-full max-w-[1600px] place-items-center py-6 text-sm text-[#60758b]">
  ...
  if (!application)
    return (
      <main className="mx-auto w-full max-w-[1600px] py-2">
        ...
        <p className="mt-5 rounded-xl border border-dashed border-[#d8e4f0] bg-white p-8 text-sm text-[#60758b]">
  ...
  const missingConsents = REQUIRED_CONSENT_TYPES.filter(
    (type) => !acceptedConsentTypes.has(type),
  );
  const photo = applicationImage(application);
  const applicant = nameOf(application);
  const address =
    text(
      property.address ||
        property.addressLine ||
        property.name ||
        application.data?.propertyAddress,
    ) || "Property details pending";
  const rent = Number(
    property.monthlyRent ||
      property.monthly_rent ||
      application.data?.rentalInfo?.monthlyRent ||
  ...
  const journey = [
    ...
    [
      "Documents",
      documents.required && documents.progress === 100
        ? "Complete"
        : "In progress",
    ],
    [
  ...
  const overview = (
    <>
      <section className="rounded-[18px] border border-[#dfe7f0] bg-white p-4 shadow-[0_12px_30px_rgba(15,23,42,.045)] sm:p-5">
        <p className="text-[.68rem] font-semibold uppercase tracking-[.1em] text-[#6f8398]">
          Application journey
        </p>
        <p className="mt-1 text-sm text-[#60758b]">
          Track the application from capture through tenancy conversion.
        </p>
        <div className="mt-5 grid gap-4 sm:grid-cols-5">
          {journey.map(([label, detail], index) => {
            ...
            const current = index === currentJourney && !complete;
            return (
              <div
                key={label}
                className="relative min-w-0 text-center sm:text-left"
              >
                <span
                  className={`mx-auto grid h-8 w-8 place-items-center rounded-full text-xs font-bold sm:mx-0 ${complete ? "bg-[#16894f] text-white" : current ? "bg-[#1769d1] text-white" : "border border-[#d7e1eb] bg-white text-[#718399]"}`}
                >
                  {complete ? <CheckCircle2 size={15} /> : index + 1}
                </span>
                {index < journey.length - 1 ? (
                  <span
                    className={`absolute left-[calc(50%+18px)] right-[calc(-50%+18px)] top-4 hidden h-0.5 sm:block ${index < currentJourney ? "bg-[#16894f]" : "bg-[#e5edf5]"}`}
                  />
                ) : null}
                <p className="mt-2 text-xs font-semibold text-[#24384f]">
                  {label}
                </p>
                <p className="mt-1 text-[.65rem] text-[#718399]">{detail}</p>
              </div>
            );
          })}
        </div>
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(300px,.8fr)]">
        <div className="grid gap-4 md:grid-cols-2">
          <section className="rounded-[18px] border border-[#dfe7f0] bg-white p-4 shadow-[0_12px_30px_rgba(15,23,42,.045)]">
            <p className="text-sm font-semibold text-[#142132]">
              Next best action
            </p>
            <div className="mt-4 rounded-xl border border-[#dbe6f1] bg-[#f8fbff] p-3">
              ...
              <button
                ...
                className="mt-4 inline-flex h-9 items-center rounded-lg bg-[#0f2743] px-3 text-xs font-semibold text-white"
              >
                Open next step
              </button>
            </div>
        ...
        <section className="rounded-[18px] border border-[#dfe7f0] bg-white p-4 shadow-[0_12px_30px_rgba(15,23,42,.045)]">
          <p className="text-sm font-semibold text-[#142132]">Property</p>
          <div className="mt-4 flex gap-3">
            <div className="grid h-16 w-20 shrink-0 place-items-center overflow-hidden rounded-xl bg-[#edf5ff] text-[#315f8f]">
              {photo ? (
                <img
                  src={photo}
                  alt=""
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-[#20364d]">
                {property.name || property.title || "Rental property"}
              </p>
              <p className="mt-1 text-xs text-[#60758b]">{address}</p>
              <p className="mt-2 text-xs font-semibold text-[#315f8f]">
                {money(rent)} / month
              </p>
            </div>
          </div>
          <div className="mt-4 border-t border-[#edf2f7] pt-3">
            ...
            {documents.required ? (
              <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[#e7edf4]">
                <span
                  className="block h-full rounded-full bg-[#2f80ed]"
                  style={{ width: `${documents.progress}%` }}
                />
              </div>
            ) : null}
          </div>
  ...
  const content =
    tab === "Overview" ? (
        ...
        <div className="mt-6 border-t border-[#edf2f7] pt-5">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {REQUIRED_CONSENT_TYPES.map((type) => {
              const consent = consents.find((item) => item.type === type);
              return (
                <Field
                  key={type}
                  ...
                  value={
                    consent
                      ? `Accepted ${date(consent.accepted_at)}`
                      : "Not recorded"
                  }
                />
              );
      ...
      <section className="rounded-[18px] border border-[#dfe7f0] bg-white p-5 shadow-[0_12px_30px_rgba(15,23,42,.045)]">
        ...
        <p className="mt-1 text-sm text-[#60758b]">
          This workspace is available once the application reaches the relevant
          stage. Current status: {title(application.status)}.
        </p>
      </section>
    );
  return (
    <main className="mx-auto w-full max-w-[1600px] py-2">
      <section className="space-y-4 pb-6">
        ...
        <section className="overflow-hidden rounded-[20px] border border-[#dfe7f0] bg-white shadow-[0_16px_36px_rgba(15,23,42,.055)]">
          <div className="grid lg:grid-cols-[minmax(0,1.1fr)_minmax(380px,.9fr)]">
            <section className="bg-[radial-gradient(circle_at_top_left,#183f65,#071b31_72%)] p-5 text-white sm:p-6">
              <div className="flex flex-wrap items-start gap-2">
                <span className="rounded-full border border-sky-300/45 bg-sky-400/10 px-3 py-1 text-xs font-semibold text-sky-100">
                  Rental application
                </span>
                <span className="rounded-full bg-[#149b67] px-3 py-1 text-xs font-semibold text-white">
                  {title(application.status)}
                </span>
              </div>
              <div className="mt-7 flex items-start gap-3">
                <span className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-white/10 text-base font-bold text-white ring-1 ring-white/10">
                  {initials(applicant)}
                </span>
                <div className="min-w-0">
                  <h1 className="truncate text-3xl font-semibold tracking-[-.045em] text-white">
                    {applicant}
                  </h1>
                  <p className="mt-3 flex items-center gap-2 text-sm text-[#d7e6f5]">
                    <MapPin size={16} />
                    {address}
                  </p>
                </div>
              </div>
              <div className="mt-9 flex flex-wrap gap-x-6 gap-y-3 text-sm text-[#e6f0fa]">
            ...
            <section className="p-5 sm:p-6">
              <div className="flex items-center justify-between">
                <p className="text-[.68rem] font-semibold uppercase tracking-[.12em] text-[#516d89]">
                  Application readiness
                </p>
                <span
                  className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${statusTone(application.status)}`}
                >
                  {title(application.status)}
                </span>
              </div>
              <div className="mt-4 grid gap-4 sm:grid-cols-[130px_minmax(0,1fr)] sm:items-center">
                <div
                  className="relative mx-auto grid h-28 w-28 place-items-center rounded-full"
                  style={{
                    background: `conic-gradient(#16894f ${progress}%, #e8f0f5 0)`,
                  }}
                >
                  <div className="grid h-[88px] w-[88px] place-items-center rounded-full bg-white text-center">
                    <div>
                      <p className="text-2xl font-semibold tracking-[-.04em] text-[#13263b]">
                        {progress}
                      </p>
                      <p className="text-[.63rem] font-semibold text-[#718399]">
                        READY
                      </p>
                    </div>
                  </div>
                </div>
                <div className="overflow-hidden rounded-xl border border-[#e3eaf2]">
                  <Signal
                    icon={Building2}
                    label="Property"
                    value={money(rent)}
                    detail={property.name || "Rental property"}
                  />
                  <Signal
                    icon={FileText}
                    label="Documents"
                  ...
                  <Signal
                    icon={CalendarDays}
                    label="Submitted"
                    value={date(application.submittedAt)}
        ...
        <section className="overflow-x-auto rounded-[16px] border border-[#dfe7f0] bg-white p-1.5 shadow-[0_10px_24px_rgba(15,23,42,.04)]">
          <div className="grid min-w-[720px] grid-cols-8 gap-1">
            {TABS.map((item) => (
              <button
                ...
                key={item}
                onClick={() => setTab(item)}
                className={`w-full min-w-0 whitespace-nowrap rounded-[11px] px-3 py-3 text-sm font-semibold ${tab === item ? "bg-[#f8fafc] text-[#142132] shadow-sm ring-1 ring-[#e3eaf2]" : "text-[#60758b] hover:bg-[#f8fafc]"}`}
              >
                {item}
              </button>
            ))}
