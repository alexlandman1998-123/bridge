...
function HomeSeekersRent() {
  ...
  return <main className="hs-site hs-rent-site">
    <header className={`hs-header hs-rent-header${headerScrolled ? ' hs-rent-header--scrolled' : ''}`}><a className="hs-logo" href="/demo/homeseekers"><img src="/brand/homeseekers/logo.png" alt="Home Seekers" /></a><nav className="hs-nav" aria-label="Main navigation">{nav.map((item,index) => <a className={item === 'Rent' ? 'is-active' : ''} href={item === 'Buy' ? '/demo/homeseekers/buy' : item === 'Sell' ? '/demo/homeseekers/sell' : item === 'Rent' ? '/demo/homeseekers/rent' : item === 'Developments' ? '/demo/homeseekers/developments' : item === 'Our people' ? '/demo/homeseekers/people' : item === 'About' ? '/demo/homeseekers/about' : item === 'Contact' ? '/demo/homeseekers/contact' : '/demo/homeseekers'} key={item}>{item}{index < 3 && <ChevronDown size={13} />}</a>)}</nav></header>
