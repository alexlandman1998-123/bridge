...
import {
  ...
  getQuoteLeadMessage as getYoungLawQuoteLeadMessage,
  SARS_CGT_SOURCE_URL,
...
const LOGO_SRC = '/brand/tuckers-attorneys-logo-transparent.png'
const CONTACT_EMAIL = 'info@tuckers.co.za'
const CONTACT_PHONE = '011 897 1900'
const TUCKERS_ATTORNEYS_WEBSITE_URL = 'https://tuckers.co.za/'
...
function buildQuoteRequestMailto(type, result, draft = {}) {
  ...
  const subject = encodeURIComponent(`Tuckers Attorneys quote request - ${summary.title}`)
  const lines = [
...
function getQuoteLeadMessage(type, result) {
  return getYoungLawQuoteLeadMessage(type, result).replaceAll('Young Law', 'Tuckers Attorneys')
}
...
function LogoMark({ compact = false }) {
  return (
    <img
      src={LOGO_SRC}
      alt="Tuckers Attorneys Inc."
      className={`${compact ? 'h-16 w-16' : 'h-[7.5rem] w-[7.5rem]'} object-contain object-left`}
    />
  )
}
...
function StickyResultBar({ label, value, type, result, tone = 'dark', onQuote }) {
  ...
  return (
    <div className="-mx-4">
      ...
      {isDocked ? (
        <div className={shellClass} data-tuckers-attorneys-result-shell={label.toLowerCase()}>
...
function QuoteRequestSheet({ request, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#171412]/42 px-3 pb-3 pt-12 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="tuckers-attorneys-quote-title">
      ...
      <section className="relative grid max-h-[calc(100dvh-3rem)] w-full max-w-[480px] overflow-hidden rounded-lg border border-[#d8d2c5] bg-[#f7f6f1] shadow-[0_24px_70px_rgba(0,0,0,0.24)]">
        ...
        <div className="grid gap-4 overflow-y-auto px-4 py-4">
          <div>
            ...
            <h2 id="tuckers-attorneys-quote-title" className="mt-1 font-serif text-[1.65rem] font-normal leading-tight text-[#141210]">Request a formal quote</h2>
            <p className="mt-2 text-sm font-normal leading-6 text-[#626766]">Send Tuckers Attorneys a clean snapshot of this estimate and your preferred contact details.</p>
          </div>
    ...
    <section className="relative grid min-h-[100dvh] content-between gap-6 overflow-hidden px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(0.9rem,env(safe-area-inset-top))]">
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between gap-4">
          <LogoMark />
          <a href={`tel:${CONTACT_PHONE.replace(/\s/g, '')}`} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-lg border border-[#d8d2c5] bg-white px-3 text-xs font-medium text-[#171412] shadow-[0_8px_20px_rgba(32,27,20,0.04)]" aria-label={`Call Tuckers Attorneys on ${CONTACT_PHONE}`}>
            <Phone size={17} />
        ...
        <div className="mt-8">
          ...
          <p className="mt-4 max-w-[28rem] text-[0.96rem] font-normal leading-7 text-[#505655]">
            Estimate a transfer, sale or estate matter, then ask Tuckers Attorneys for a formal quote.
          </p>
        </div>
      ...
      <footer className="grid gap-3 rounded-lg border border-[#d8d2c5] bg-[#171412] p-4 text-white shadow-[0_10px_28px_rgba(0,0,0,0.12)]">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <a href={TUCKERS_ATTORNEYS_WEBSITE_URL} target="_blank" rel="noreferrer" className="rounded-lg border border-white/15 px-3 py-2 text-center text-xs font-medium text-white">Website</a>
        </div>
      </footer>
    </section>
  )
  ...
  return (
    <header className="px-4 pb-3 pt-[max(1rem,env(safe-area-inset-top))]">
      <div className="flex items-center justify-between gap-3">
        <LogoMark compact />
        <a href={`tel:${CONTACT_PHONE.replace(/\s/g, '')}`} className="inline-flex min-h-10 shrink-0 items-center justify-center gap-2 rounded-lg border border-[#d8d2c5] bg-white px-3 text-xs font-medium text-[#171412] shadow-[0_8px_20px_rgba(32,27,20,0.04)]" aria-label={`Call Tuckers Attorneys on ${CONTACT_PHONE}`}>
          <Phone size={15} />
      ...
      <div className="grid gap-4 px-4 pb-5">
        ...
        {transferStep === 'finance' ? (
          <FlowStep
            step="1"
            title="Finance type"
            detail="Choose whether Tuckers Attorneys should include bond registration work in the estimate."
          >
            <div className="grid gap-2">
...
function TuckersAttorneysCalculatorsPage() {
  ...
  return (
    ...
    >
      ...
      <a
        href={TUCKERS_ATTORNEYS_WEBSITE_URL}
        target="_blank"
        rel="noreferrer"
...
export default TuckersAttorneysCalculatorsPage
