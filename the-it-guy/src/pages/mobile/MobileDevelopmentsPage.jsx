...
export default function MobileDevelopmentsPage() {
  ...
  return (
    <>
      <MobileTopBar title="Developments" subtitle="Portfolio Overview" />
      ...
      {state.loading ? (
        ...
        <>
          <MobileSection title="Portfolio Snapshot" eyebrow="Overview">
            <div className="grid grid-cols-2 gap-3">
          ...
          <MobileSection title="All Developments" eyebrow="Executive Scan">
                    ...
                    <Link key={development.id} to={`/m/developments/${development.id}`} className="block transition-transform duration-200 active:scale-[0.992]">
                      <MobileCard className="p-0">
                        <div className="border-b border-[#e8edf5] px-4 py-4">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <h3 className="truncate text-xl font-semibold tracking-[-0.03em] text-[#101828]">{development.name}</h3>
                              <div className="mt-2 flex items-center gap-2 text-sm text-[#687c95]">
                                <MapPin className="h-4 w-4 text-[#8ea0b5]" />
                            ...
                            <div className="flex shrink-0 items-start gap-2">
                              ...
                              <span className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-[#e6ebf3] bg-[#fbfcfe] text-[#10243a]">
                          ...
                          <div className="mt-3 flex items-center justify-between gap-3">
                            <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#8a9ab2]">
                              {formatRelativeTimestamp(development.lastUpdated)}
                            </span>
                            {development.attentionCount > 0 ? (
                              <span className="rounded-full border border-[#f4d9c0] bg-[#fff7ed] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#b46213]">
                        ...
                        <div className="space-y-4 px-4 py-4">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div className="rounded-[20px] border border-[#edf2f7] bg-[#fbfcfe] px-3 py-3">
                              <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#8799af]">Total Units</p>
                              <strong className="mt-1 block text-[28px] font-semibold tracking-[-0.04em] text-[#101828]">
                                {integerFormatter.format(development.totalUnits || 0)}
                              </strong>
                            </div>
                            <div className="rounded-[20px] border border-[#edf2f7] bg-[#fbfcfe] px-3 py-3">
                              <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#8799af]">Conversion</p>
                              <strong className="mt-1 block text-[28px] font-semibold tracking-[-0.04em] text-[#101828]">
                                {formatPercent(development.sellThroughPercent || 0)}
                              </strong>
                            </div>
                            <div className="rounded-[20px] border border-[#edf2f7] bg-[#fbfcfe] px-3 py-3">
                              <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#8799af]">Live Deals</p>
                              <strong className="mt-1 block text-[28px] font-semibold tracking-[-0.04em] text-[#101828]">
                                {integerFormatter.format(development.liveDeals || 0)}
                              </strong>
                            </div>
                            <div className="rounded-[20px] border border-[#edf2f7] bg-[#fbfcfe] px-3 py-3">
                              <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#8799af]">Cash vs Bond</p>
                              <strong className="mt-1 block text-sm font-semibold leading-5 text-[#101828]">{development.financeMixLabel}</strong>
                            </div>
                          </div>
                        </div>
                      </MobileCard>
