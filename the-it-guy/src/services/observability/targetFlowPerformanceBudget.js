...
function violation(metric, actual, limit) {
  const normalizedActual = Math.round(finiteNumber(actual) * 10) / 10
  return normalizedActual > limit ? { metric, actual: normalizedActual, limit } : null
}
...
export function evaluateTargetFlowPerformanceBudget(summary = {}) {
  ...
  if (!page) {
    return { contract: CONTRACT_VERSION, targeted: false, status: 'NOT_TARGETED', page: '', violations: [] }
  }

  const baseBudget = TARGET_FLOW_PERFORMANCE_BUDGETS[page]
  const source = String(summary?.metadata?.source || '').trim().toLowerCase()
  const fastPathBudget = FAST_PATH_BUDGETS[source] || null
  const budget = fastPathBudget ? { ...baseBudget, ...fastPathBudget } : baseBudget
  const violations = Object.entries(budget)
    .map(([metric, limit]) => violation(metric, summary?.[metric], limit))
    .filter(Boolean)
  ...
  return {
    ...
    fastPath: Boolean(fastPathBudget),
    budget,
    violations,
  }
}
...
export function evaluateTargetFlowReleaseGate(history = [], { minimumSamplesPerFlow = 3 } = {}) {
  ...
  const flows = Object.fromEntries(REQUIRED_TARGET_FLOWS.map((page) => {
    const pageSamples = samples.filter((sample) => {
      const evaluation = evaluateTargetFlowPerformanceBudget(sample)
      return evaluation.targeted && evaluation.page === page
    })
    const evaluations = pageSamples.map((sample) => evaluateTargetFlowPerformanceBudget(sample))
    return [page, {
      ...
      firstUsefulContentP95Ms: percentile(pageSamples.map((sample) => sample.firstUsefulContentMs), 0.95),
      requestCountP95: percentile(pageSamples.map((sample) => sample.requestCount), 0.95),
      covered: pageSamples.length >= minimumSamplesPerFlow,
    }]
  }))
  ...
  return {
    contract: CONTRACT_VERSION,
    status: missingFlows.length ? 'INSUFFICIENT_DATA' : failingFlows.length ? 'FAIL' : 'PASS',
    ready: missingFlows.length === 0,
    minimumSamplesPerFlow,
    missingFlows,
    failingFlows,
    flows,
  }
...
export function readTargetFlowReleaseGate(storage = typeof window !== 'undefined' ? window.sessionStorage : null, options = {}) {
  ...
  try {
    ...
    return evaluateTargetFlowReleaseGate(history, options)
  } catch {
...
function sanitizeEvidenceSample(sample = {}) {
  ...
  const summary = {
    ...
    routeStartedAt: String(sample.routeStartedAt || ''),
    routeCompletedAt: String(sample.routeCompletedAt || ''),
    shellVisibleMs: sample.shellVisibleMs == null ? null : finiteNumber(sample.shellVisibleMs),
    ...
    requestP95Ms: finiteNumber(sample.requestP95Ms),
...
export function buildTargetFlowPerformanceEvidence(history = [], options = {}) {
  const samples = (Array.isArray(history) ? history : [])
    .map(sanitizeEvidenceSample)
    .filter((sample) => sample.targetBudget.targeted)
  return {
    contract: EVIDENCE_CONTRACT_VERSION,
    generatedAt: new Date().toISOString(),
    source: 'bounded_session_telemetry',
    gate: evaluateTargetFlowReleaseGate(samples, options),
    samples,
  }
}
...
export function verifyTargetFlowPerformanceEvidence(evidence = {}, options = {}) {
  if (evidence?.contract !== EVIDENCE_CONTRACT_VERSION || !Array.isArray(evidence?.samples)) {
    return {
      contract: VERIFICATION_CONTRACT_VERSION,
      status: 'INVALID_EVIDENCE',
      ready: false,
      reason: 'evidence_contract_or_samples_invalid',
      gate: evaluateTargetFlowReleaseGate([], options),
    }
  }
  const samples = evidence.samples.map(sanitizeEvidenceSample)
  const gate = evaluateTargetFlowReleaseGate(samples, options)
  return {
    contract: VERIFICATION_CONTRACT_VERSION,
    status: gate.status,
    ready: gate.ready && gate.status === 'PASS',
    reason: gate.status === 'PASS' ? 'target_flow_performance_ready' : 'target_flow_performance_not_ready',
    gate,
  }
}
