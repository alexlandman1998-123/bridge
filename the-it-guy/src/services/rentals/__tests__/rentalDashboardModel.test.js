...
assert.equal(buildRentalDashboardSnapshot({ applications: [{ status: 'submitted' }, { status: 'approved' }, { status: 'declined' }] }).openApplications, 1)
assert.equal(buildRentalDashboardSnapshot({ leases: [{ status: 'active' }, { status: 'closed' }] }).activeTenancies, 1)
