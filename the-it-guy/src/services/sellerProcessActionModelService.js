...
const KINGSTONS_ACTION_BLUEPRINTS = Object.freeze([
  ...
  Object.freeze({
    ...
    label: 'Seller Pack',
    description: 'Seller pack actions are intentionally deferred for Kingston.',
...
function buildAction(blueprint = {}, evaluation = {}) {
  ...
  const enabled = blueprint.enabled === false ? false : !deferred
...
function selectCurrentAction(actions = []) {
  return actions.find((action) => action.pending) || actions[actions.length - 1] || null
}
