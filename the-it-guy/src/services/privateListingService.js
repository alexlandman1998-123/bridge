...
function isProperty24MigrationImportRow(row = {}) {
  const facts = row?.seller_canonical_facts_json || row?.sellerCanonicalFacts || {}
  const source = normalizeKey(row?.stock_source || row?.stockSource || row?.listing_source || row?.listingSource)
...
async function appendSellerPortalMandateInviteEvent() {
  // Seller invitation activity is recorded by createSellerPostMandateActivityFromAudit.
  // Signed uploads no longer write events into the retired generator.
  return null
}
