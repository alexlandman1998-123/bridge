...
function getStageAction(lead = {}) {
  ...
  if (stageKey === RESIDENTIAL_OFFER_STAGE_KEYS.offerSubmitted) return 'Review buyer conditions before manual signed OTP upload'
  if (stageKey === RESIDENTIAL_OFFER_STAGE_KEYS.agentReviewRequired) return 'Approve or rewrite buyer conditions'
  if (stageKey === RESIDENTIAL_OFFER_STAGE_KEYS.readyToGenerateOtp) return 'Upload signed OTP'
  if (stageKey === RESIDENTIAL_OFFER_STAGE_KEYS.otpGenerated) return 'Confirm signed OTP upload'
  ...
  if (stage.includes('offer')) return 'Review buyer conditions before manual signed OTP upload'
