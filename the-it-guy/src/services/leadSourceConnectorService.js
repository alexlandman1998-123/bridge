...
export function mapPrivatePropertyPayload(payload = {}) {
  return buildCanonicalLeadPayload({
    ...
    name: pickFirst(payload, ['name', 'fullName', 'full_name', 'contactName', 'contact_name', 'leadName', 'customer.name']),
    phone: pickFirst(payload, ['phone', 'mobile', 'telephone', 'contactNumber', 'contact_number', 'leadPhoneNumber', 'leadPhone', 'customer.phone']),
    email: pickFirst(payload, ['email', 'leadEmail', 'customer.email']),
    message: pickFirst(payload, ['message', 'leadMessage', 'comments', 'comment', 'enquiryText', 'enquiry_text', 'body']),
    listingReference: pickFirst(payload, ['listingReference', 'listing_reference', 'listingExternalReference', 'privatePropertyListingId', 'private_property_listing_id', 'propertyId', 'property_id', 'listing.id']),
    listingId: pickFirst(payload, ['listingId', 'listing_id', 'privateListingId', 'private_listing_id']),
    receivedAt: pickFirst(payload, ['receivedAt', 'received_at', 'createdAt', 'created_at', 'timestamp']),
    rawPayload: payload,
  }, 'Private Property')
