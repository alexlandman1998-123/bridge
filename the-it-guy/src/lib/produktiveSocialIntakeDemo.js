const STORAGE_KEY = 'arch9:produktive-social-intake:v1'
...
export function readProduktiveDemoState() {
...
export function writeProduktiveDemoState(nextState) {
  if (typeof window === 'undefined' || !window.localStorage) return { ...emptyState }
...
export function resetProduktiveDemoState() {
  return writeProduktiveDemoState(emptyState)
}
...
export function createBuyerLead(buyer = {}, matchedProperties = []) {
  ...
  const state = readProduktiveDemoState()
  ...
  writeProduktiveDemoState({ ...state, buyerLeads: [lead, ...state.buyerLeads] })
  return lead
}
...
export function createSellerLead(seller = {}) {
  ...
  writeProduktiveDemoState({ ...state, sellerLeads: [lead, ...state.sellerLeads] })
  return lead
}
