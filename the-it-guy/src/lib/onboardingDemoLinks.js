...
function getDemoSellerPortalSeed(token = SELLER_PORTAL_DEMO_TOKEN) {
  ...
  const portalData = {
    ...
    // A seller listing does not create a transaction. The legal-sale record
    // is added only once an offer has been accepted.
    transaction: null,
    unit: {
...
export function getDemoSellerOnboardingListing(token = SELLER_ONBOARDING_DEMO_TOKEN, formDataOverride = null) {
  const formData = formDataOverride || {
    ...
    preferredTransferAttorney: {
      preferredPartnerId: 'demo-preferred-transfer-attorney',
      companyName: 'Young Law Inc.',
      contactPerson: 'Transfer Department',
      email: 'transfers.demo@arch9.co.za',
