...
async function authenticateRequest({ request, supabase, organisationId } = {}) {
  ...
  if (!token) return { ok: false, response: buildResponse(401, { error: 'unauthorized', message: 'Sign in before syncing Property24 statistics.' }) }
  ...
  if (!(membership.data || []).some(hasManageSettingsRole)) {
    return { ok: false, response: buildResponse(403, { error: 'forbidden', message: 'Organisation admin access is required to sync Property24 statistics.' }) }
  }
  return { ok: true, user }
}
...
export default async function handler(request, response) {
  if (request.method === 'OPTIONS') {
    writeNodeJsonResponse(response, buildResponse(204, null))
    return
  }
  if (request.method !== 'POST') {
    writeNodeJsonResponse(response, buildResponse(405, { error: 'method_not_allowed', message: 'Property24 statistics sync only supports POST.' }))
    return
  }
  ...
  try {
    ...
    if (!supabaseUrl || !serviceRoleKey) {
      writeNodeJsonResponse(response, buildResponse(503, { error: 'missing_configuration', missingConfiguration: ['SUPABASE_URL', 'SUPABASE_SERVICE_ROLE_KEY'] }))
      return
    }
    const body = await readJsonBody(request)
    const organisationId = normalizeProperty24Text(body.organisationId)
    if (!organisationId) {
      writeNodeJsonResponse(response, buildResponse(400, { error: 'missing_configuration', missingConfiguration: ['organisationId'] }))
      return
    }
    ...
    if (!connection.configured || !connection.enabled) {
      writeNodeJsonResponse(response, buildResponse(409, {
        ...
        message: connection.configured ? 'Enable the Property24 connection before syncing statistics.' : 'Save the Property24 agency connection before syncing statistics.',
      }))
      return
    }
    ...
    const report = await syncProperty24ListingStatistics({
      ...
      config: {
        ...
        sourceApiVersion: property24Runtime.apiVersion,
    ...
    writeNodeJsonResponse(response, buildResponse(200, { route: 'property24StatisticsSync', report }))
  } catch (error) {
    writeNodeJsonResponse(response, buildResponse(Number(error.status || 500), {
      error: error.code || 'property24_statistics_sync_failed',
      message: error.message || 'Property24 statistics sync failed.',
