...
async function authenticateRequest({ request, supabase, organisationId } = {}) {
  ...
  if (!token) {
    return { ok: false, response: buildResponse(401, { error: 'unauthorized', message: 'Sign in before reconciling Property24.' }) }
  }
  ...
  if (!(membership.data || []).some(hasManageSettingsRole)) {
    return {
      ...
      response: buildResponse(403, {
        error: 'forbidden',
        message: 'Organisation admin access is required to reconcile Property24.',
      }),
    }
  }
  return { ok: true, user }
...
export default async function handler(request, response) {
  ...
  if (request.method !== 'POST') {
    writeNodeJsonResponse(response, buildResponse(405, {
      ...
      message: 'Property24 reconciliation only supports POST.',
    }))
    return
  }
  ...
  try {
    ...
    if (!connection.configured) {
      writeNodeJsonResponse(response, buildResponse(409, {
        error: 'property24_connection_not_configured',
        message: 'Save the Property24 agency connection before running reconciliation.',
      }))
      return
    }
    if (!connection.enabled) {
      writeNodeJsonResponse(response, buildResponse(409, {
        error: 'property24_connection_disabled',
        message: 'Enable the Property24 connection before running reconciliation.',
      }))
      return
    }
    ...
    const report = await runProperty24ReconciliationJob({
      ...
      config: {
        ...
        includePortalChecks: body.includePortalChecks !== false,
        includeLeads: false,
    ...
    writeNodeJsonResponse(response, buildResponse(200, {
      route: 'property24SettingsReconciliation',
      organisationId,
    ...
    writeNodeJsonResponse(response, buildResponse(Number(error.status || 500), {
      error: error.code || 'property24_reconciliation_failed',
      message: error.message || 'Property24 reconciliation failed.',
