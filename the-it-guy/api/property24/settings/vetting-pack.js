...
} from '../../../server/property24/vettingPackService.js'
...
async function authenticateRequest({ request, supabase, organisationId } = {}) {
  ...
  if (!token) {
    return { ok: false, response: buildResponse(401, { error: 'unauthorized', message: 'Sign in before generating a Property24 vetting pack.' }) }
  }
  ...
  if (!(membership.data || []).some(hasManageSettingsRole)) {
    return {
      ...
      response: buildResponse(403, {
        error: 'forbidden',
        message: 'Organisation admin access is required to generate a Property24 vetting pack.',
      }),
    }
  }
  return { ok: true, user }
...
function getRequiredSupabaseEnvironment(env = {}) {
  const missing = []
...
export default async function handler(request, response) {
  ...
  if (request.method !== 'POST') {
    writeNodeJsonResponse(response, buildResponse(405, {
      ...
      message: 'Property24 vetting-pack generation only supports POST.',
    }))
    return
  }
  ...
  try {
    ...
    const missingSupabaseConfiguration = getRequiredSupabaseEnvironment(env)
    if (missingSupabaseConfiguration.length) {
      writeNodeJsonResponse(response, buildResponse(503, {
        error: 'missing_configuration',
        missingConfiguration: missingSupabaseConfiguration,
      }))
      return
    }
    ...
    const connection = await fetchOrganisationProperty24Connection({ supabase, organisationId, environment: 'exdev' })
    if (!connection.configured || !connection.enabled) {
      writeNodeJsonResponse(response, buildResponse(409, {
        error: connection.configured ? 'property24_connection_disabled' : 'property24_connection_not_configured',
        message: connection.configured
          ? 'Enable the Property24 connection before generating the vetting pack.'
          : 'Save the Property24 agency connection before generating the vetting pack.',
      }))
      return
    }
    if (connection.environment !== 'exdev') {
      writeNodeJsonResponse(response, buildResponse(409, {
        error: 'property24_vetting_requires_exdev',
        message: 'The Phase 6 vetting pack can only be generated for an ExDev connection.',
      }))
      return
    }

    const property24Runtime = resolveProperty24EnvironmentCredentials({ env, environment: 'exdev' })
    ...
    const pack = await createProperty24OrganisationVettingPack({
      supabase,
      property24,
      organisationId,
      connection,
    ...
    writeNodeJsonResponse(response, buildResponse(200, {
      route: 'property24SettingsVettingPack',
      organisationId,
      status: pack.status,
      pack,
      markdown: renderProperty24VettingPackMarkdown(pack),
    }))
  } catch (error) {
    writeNodeJsonResponse(response, buildResponse(Number(error.status || 500), {
      error: error.code || 'property24_vetting_pack_failed',
      message: error.message || 'Property24 vetting-pack generation failed.',
