...
async function assertRateLimit(db, organisationId, userId) {
  ...
  const { count, error } = await db.from('knowledge_factory_audit_log').select('id', { count: 'exact', head: true }).eq('organisation_id', organisationId).eq('actor_id', userId).eq('operation', 'map_properties').gte('created_at', since)
...
async function publicEgressIp() {
  try {
    const response = await fetch('https://api.ipify.org?format=json', { signal: AbortSignal.timeout(3_000) })
    const body = await response.json()
    return text(body?.ip, 100) || null
  } catch {
    return null
  }
}
...
async function supplierToken(config) {
  ...
  const response = await fetch(config.endpoint, {
    ...
    body: JSON.stringify({
      query: 'mutation ($input: LoginInput!) { login(login: $input) { tokenPayload { expiresUtc token } errors { ... on ArgumentError { message paramName } ... on ArgumentNullError { message paramName } ... on Error { message } } } }',
      variables: { input: { email: config.email, password: config.password } },
    }),
  })
  const body = await response.json().catch(() => ({}))
  if (!response.ok) {
    console.error('Knowledge Factory supplier login rejected', {
      endpoint: new URL(config.endpoint).host,
      status: response.status,
      egressIp: await publicEgressIp(),
...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (body.action === 'status') return json(response, 200, {
    ...
    if (!allowed) return json(response, 403, { error: 'Knowledge Factory map access has not been granted.' })
