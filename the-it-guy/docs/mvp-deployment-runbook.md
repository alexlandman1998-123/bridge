...
1. Run the Phase 3 ledger gate from the release-worktree root. Do not deploy if it returns `no_go`:

   ```bash
   npm run mvp:phase3:plan -- --ledger=docs/staging-migration-ledger.json
   ```

2. Apply the approved migrations in order to the confirmed staging environment:
