...
async function resolveSenderOrganisationBranding(
  ...
  if (
    ...
    console.error(
      "[seller_onboarding_submitted] organisation lookup failed",
      organisationQuery.error,
    );
  }
  ...
  if (
    ...
    console.error(
      "[seller_onboarding_submitted] organisation logo lookup failed",
      logoQuery.error,
    );
  }
  ...
  if (
    ...
    console.error(
      "[seller_onboarding_submitted] organisation branding lookup failed",
      brandingQuery.error,
    );
  }
  ...
  if (
    ...
    console.error(
      "[seller_onboarding_submitted] organisation settings lookup failed",
      settingsQuery.error,
    );
  }
...
function resolveSellerPortalLink(
  req: Request,
  ...
  const token = normalizeText(
    payload.sellerPortalToken ?? payload.seller_portal_token,
  );
  if (!token) return "";

  const appBaseUrl = resolveAppBaseUrl(req) || "https://app.arch9.co.za";
  return `${appBaseUrl.replace(/\/+$/, "")}/client/${
...
async function fetchProfilesByIds(
  supabase: any,
  userIds: string[],
): Promise<Map<string, ProfileRecipient>> {
  ...
  if (query.error) {
    console.error(
      "[seller_onboarding_submitted] profile recipient list lookup failed",
      query.error,
    );
  ...
  return new Map(
    (Array.isArray(query.data) ? query.data : []).map((profile: any) => [
...
async function fetchOrganisationPrincipalRecipients(
  ...
  for (const selectClause of selectCandidates) {
    ...
    if (
      ...
      isMissingTableError(query.error, "organisation_users")
    ) {
      continue;
    }

    console.error(
      "[seller_onboarding_submitted] organisation principal recipient lookup failed",
      query.error,
    );
    break;
  }
  ...
  const principalRows = rows.filter((row) => {
    const role = row.workspace_role || row.organisation_role ||
      row.organization_role || row.role;
...
async function resolveInternalNotificationRecipients(
  ...
  const agentIds = collectUnique([payload.assignedAgentId]).map(
    normalizeUuidLike,
  ).filter(Boolean);
  const organisationIds = collectUnique([payload.organisationId]).map(
    normalizeUuidLike,
  ).filter(Boolean);
  ...
  addInternalRecipient(recipients, {
    email: normalizeEmail(payload.to) || normalizeEmail(payload.agentEmail) ||
      normalizeEmail(payload.assignedAgentEmail),
    name: resolvedAgentName,
    role: "agent",
    resolvedBy: normalizeEmail(payload.to)
      ? "payload.to"
      : "payload.agentEmail",
  ...
  if (listingId) {
    const listingQuery = await supabase
      ...
      .select(
        "id, organisation_id, assigned_agent_email, assigned_agent_id, seller_lead_id, originating_crm_lead_id",
      )
      .eq("id", listingId)
      .maybeSingle();
    ...
    if (
      !listingQuery.error ||
      isMissingTableError(listingQuery.error, "private_listings")
    ) {
      const listing = listingQuery.data || {};
      if (listing.organisation_id) {
        organisationIds.push(normalizeUuidLike(listing.organisation_id));
      }
      ...
      if (listing.assigned_agent_id) {
        agentIds.push(normalizeUuidLike(listing.assigned_agent_id));
      }
      addInternalRecipient(recipients, {
      ...
      console.error(
        "[seller_onboarding_submitted] listing recipient lookup failed",
        listingQuery.error,
      );
    }
  }
  ...
  for (const leadId of collectUnique(leadIds)) {
    const leadQuery = await supabase
      .from("leads")
      .select(
        "lead_id, organisation_id, assigned_agent_email, assigned_agent_id",
      )
      .eq("lead_id", leadId)
      .maybeSingle();
    ...
    if (!leadQuery.error || isMissingTableError(leadQuery.error, "leads")) {
      ...
      if (lead.organisation_id) {
        organisationIds.push(normalizeUuidLike(lead.organisation_id));
      }
      if (lead.assigned_agent_id) {
        agentIds.push(normalizeUuidLike(lead.assigned_agent_id));
      }
      addInternalRecipient(recipients, {
      ...
      console.error(
        "[seller_onboarding_submitted] lead recipient lookup failed",
        leadQuery.error,
      );
    }
  }
  ...
  for (const organisationId of collectUnique(organisationIds)) {
    const principalRecipients = await fetchOrganisationPrincipalRecipients(
      supabase,
      organisationId,
    );
...
export async function handleSellerOnboardingSubmittedEmail(
  ...
  const supabase = supabaseUrl && serviceRoleKey
    ...
    : null;
  const internalResolution = await resolveInternalNotificationRecipients(
    supabase,
    payload,
  );
  ...
  const sellerPortalLink = deferSellerPortalLinkUntilMandateSigned
    ? ""
    : resolveSellerPortalLink(req, payload);
  const requestedActionLink = normalizeText(payload.actionLink);
  const appBaseUrl = resolveAppBaseUrl(req);
  const actionLink = requestedActionLink ||
    (appBaseUrl && leadId
      ? `${appBaseUrl}/pipeline/leads/${
        encodeURIComponent(leadId)
      }/legal/mandate`
      : "");

  const baseSender = normalizeText(Deno.env.get("RESEND_FROM_EMAIL")) ||
  ...
  const organisationName = normalizeText(payload.organisationName) ||
    ...
    "Arch9";
  let supportEmail = normalizeText(Deno.env.get("BRIDGE_SUPPORT_EMAIL")) ||
    normalizeText(Deno.env.get("SUPPORT_EMAIL"));
  let supportPhone = normalizeText(Deno.env.get("BRIDGE_SUPPORT_PHONE")) ||
  ...
  if (organisationId && supabase) {
    try {
      ...
      console.error(
        "[seller_onboarding_submitted] template override lookup failed",
        error,
      );
    }
  }
  ...
  if (!internalRecipients.length) {
    console.error(
      "[seller_onboarding_submitted] unable to resolve internal recipients",
      internalResolution.lookupContext,
    );
    skipped.push({
      target: "internal",
      ...
      payload as Record<string, unknown>,
      {
        ...
        context: {
          ...
          assignedUserId: recipient.userId ||
            normalizeText(payload.assignedAgentId),
          metadata: {
  ...
  if (!sellerEmail || !sellerPortalLink) {
    ...
    const sellerSubject = buildSellerOnboardingSubmittedSellerSubject(
      propertyTitle,
    );
    ...
    const sellerDelivery = await prepareEmailDelivery(
      payload as Record<string, unknown>,
      {
    ...
    if (!sellerResult.ok) {
      await markEmailDeliveryFailed(sellerDelivery?.id || "", {
        errorMessage: sellerResult.error?.message ||
  ...
  if (!sent.length) {
    return jsonResponse(400, {
      error:
        "No seller onboarding submission notification recipients were available.",
      skipped,
      lookupContext: internalResolution.lookupContext,
    });
  }
