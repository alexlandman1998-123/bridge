...
async function assertProfessionalAccess(
  ...
  const profile = await supabase.from("profiles").select("role").eq(
    "id",
    userId,
  ).maybeSingle();
  ...
  if (!PROFESSIONAL_ROLES.has(role)) {
    throw new Error(
      "Only an authorised transaction professional may dispatch progress notifications.",
    );
  }
...
function buildEmail(event: Record<string, unknown>, branding: EmailBranding) {
  ...
  const subject = normalizeText(event.subject) ||
    `Arch9 transaction update: ${processLabel}`;
  const fields = [
    { label: "Process", value: processLabel },
    { label: "Current update", value: title },
    { label: "Status", value: status },
    safeExplanation
      ? { label: "What is holding it up", value: safeExplanation }
      : null,
    expectedNextStep
      ? { label: "Expected next step", value: expectedNextStep }
      : null,
  ].filter(Boolean) as Array<{ label: string; value: string }>;
  const intro = [description];
  if (safeExplanation && safeExplanation !== description) {
    intro.push(safeExplanation);
  }
  const contentHtml = [
  ...
  const html = renderBridgeEmailLayout({
    ...
    contentHtml,
    securityBody:
      "This message contains only the progress information made visible to your transaction role. Private legal notes and documents are not included.",
    helpBody:
...
async function cloneForResend(supabase: any, eventId: string) {
  ...
  if (source.error || !source.data) {
    throw new Error("Progress notification was not found.");
  }
  ...
  const inserted = await supabase.from("notification_events").insert(copy)
...
async function recordDelivery(
  ...
  const inserted = await supabase.from("communication_deliveries").insert({
    ...
    metadata_json: {
      source: "transaction_progress_dispatch",
...
export async function handleTransactionProgressDispatchEmail(
  ...
  if (!transactionId && eventId) {
    const lookup = await supabase.from("notification_events").select(
      "transaction_id",
  ...
  if (!transactionId && !serviceRequest) {
    return jsonResponse(400, {
      error: "Transaction id or notification event id is required.",
    });
  }

  try {
    if (!serviceRequest) {
      await assertProfessionalAccess(req, supabase, transactionId);
    }
    if (isResend) {
      if (!eventId) {
        return jsonResponse(400, {
          error: "Notification event id is required for a resend.",
        });
      }
  ...
  const claim = await supabase.rpc(
    "bridge_claim_transaction_progress_notifications_phase3",
    {
  ...
  for (const event of claim.data || []) {
    const payloadJson = record(event.payload_json);
    const metadataJson = record(event.metadata_json);
    const branding = await resolveEmailBranding({
      ...
      defaults: {
        organisationName: normalizeText(
          payloadJson.organisationName || metadataJson.organisationName,
        ) || "Arch9",
      },
    });
    const content = buildEmail(event, branding);
    if (!resendKey) {
    ...
    if (delivery.ok) {
      ...
      const log = await recordDelivery(
        supabase,
        event,
        content,
        "sent",
      ...
      await supabase.from("notification_events").update({
        ...
        communication_delivery_id: log?.id || null,
      ...
      const message = normalizeText(delivery.error?.message) ||
        "Resend rejected the transaction progress email.";
      const log = await recordDelivery(
        supabase,
        event,
        content,
