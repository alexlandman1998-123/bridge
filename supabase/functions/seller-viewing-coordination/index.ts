...
function normalizePropertyResponses(value: unknown, allowedIds: Set<string>) {
  ...
  return value
    ...
    .map((item) => {
      ...
      const canHostViewing =
        row.canHostViewing === true ||
        row.can_host_viewing === true ||
        row.wantsToView === true ||
        normalizeText(row.canHostViewing || row.can_host_viewing || row.wantsToView).toLowerCase() === "yes";
      return { propertyId, canHostViewing };
    })
    .filter(Boolean) as Array<{ propertyId: string; canHostViewing: boolean }>;
}
...
function parseNoteBlock(notes = "") {
  ...
  const labelToKey: Record<string, string> = {
    ...
    "Buyer email delivery status": "sellerEmailDeliveryStatus",
    "Buyer email delivery id": "sellerEmailDeliveryId",
    "Buyer email provider message id": "sellerEmailProviderMessageId",
    "Buyer email delivery failure": "sellerEmailDeliveryFailure",
...
function buildNoteBlock(plan: JsonRecord) {
  return [
    ...
    `Buyer email delivery status: ${normalizeText(plan.sellerEmailDeliveryStatus, 80)}`,
    `Buyer email delivery id: ${normalizeText(plan.sellerEmailDeliveryId, 120)}`,
    `Buyer email provider message id: ${normalizeText(plan.sellerEmailProviderMessageId, 120)}`,
    `Buyer email delivery failure: ${normalizeText(plan.sellerEmailDeliveryFailure, 800)}`,
...
Deno.serve(async (req) => {
  ...
  if (!supabaseUrl || !anonKey || !serviceRoleKey) {
    return jsonResponse(503, { error: "Viewing coordination is temporarily unavailable.", code: "service_unavailable" });
  }
  ...
  if (action === "create") {
    ...
    if (!user?.id) return jsonResponse(401, { error: "Sign in before creating a viewing coordination link.", code: "not_authenticated" });
    ...
    const membership = await supabase
      ...
      .maybeSingle();
    if (membership.error) {
      console.error("[seller-viewing-coordination] membership lookup failed", { code: membership.error.code, message: membership.error.message });
    ...
    const insert = await supabase
      ...
      .insert({
        ...
        lead_id: leadId,
        seller_email: normalizeText(body.sellerEmail || body.contactEmail || body.to, 320).toLowerCase() || null,
        organisation_name: normalizeText(body.organisationName || body.organisation_name, 180) || null,
        seller_name: normalizeText(body.sellerName || body.recipientName, 180) || null,
        buyer_name: normalizeText(body.buyerName || body.buyer_name, 180) || null,
        ...
        properties,
        buyer_availability_windows: normalizeText(body.availabilityWindows || body.availability_windows, 1200) || null,
        coordination_notes: normalizeText(body.coordinationNotes || body.coordination_notes || body.note || body.message, 1200) || null,
        created_by: user.id,
      ...
      .single();
    if (insert.error) {
      console.error("[seller-viewing-coordination] create failed", { code: insert.error.code, message: insert.error.message });
      return jsonResponse(500, { error: "We could not create the viewing coordination link.", code: "create_failed" });
    }

    return jsonResponse(200, {
      ok: true,
      linkId: insert.data?.id || null,
      sellerCoordinationLink: `${origin}/seller-viewing/${encodeURIComponent(token)}`,
  ...
  if (!LINK_TOKEN_PATTERN.test(token)) {
    return jsonResponse(404, { error: "This viewing coordination link is unavailable.", code: "link_unavailable" });
  }
  const tokenHash = await sha256UrlSafe(token);
  const linkResult = await supabase
    .from("seller_viewing_coordination_links")
    .select("id, organisation_id, lead_id, created_by, seller_email, organisation_name, seller_name, buyer_name, agent_name, agent_email, status, selected_property_ids, properties, buyer_availability_windows, coordination_notes, response, created_at, last_sent_at, expires_at, submitted_at")
    .eq("token_hash", tokenHash)
    .maybeSingle();
  if (linkResult.error) {
    console.error("[seller-viewing-coordination] resolve failed", { code: linkResult.error.code, message: linkResult.error.message });
    return jsonResponse(500, { error: "We could not load this viewing coordination link.", code: "resolve_failed" });
  }
  const link = linkResult.data as JsonRecord | null;
  if (!link) return jsonResponse(404, { error: "This viewing coordination link is unavailable.", code: "link_unavailable" });
  const expired = new Date(normalizeText(link.expires_at)).getTime() < Date.now();
  if (expired && link.status === "pending") {
    await supabase.from("seller_viewing_coordination_links").update({ status: "expired", updated_at: new Date().toISOString() }).eq("id", link.id);
  }

  const publicSession = {
    id: link.id,
    organisationName: link.organisation_name || "",
    sellerName: link.seller_name || "",
    ...
    buyerAvailabilityWindows: link.buyer_availability_windows || "",
    coordinationNotes: link.coordination_notes || "",
  ...
  if (expired || link.status !== "pending") {
    return jsonResponse(409, { error: "This viewing coordination link is no longer accepting responses.", code: "link_closed", session: publicSession });
  }
  ...
  const confirmedPropertyIds = propertyResponses.filter((item) => item.canHostViewing).map((item) => item.propertyId);
  ...
  const update = await supabase
    ...
    .single();
  if (update.error) {
    console.error("[seller-viewing-coordination] submit failed", { code: update.error.code, message: update.error.message });
    return jsonResponse(500, { error: "We could not save your viewing coordination.", code: "submit_failed" });
  }
