  ...
  return value
    ...
    .map((item) => {
      ...
      return {
        ...
        title: normalizeText(row.title || row.name || row.address, 180) ||
          "Selected property",
  ...
  return value
    ...
    .map((item) => {
      ...
      const propertyId = normalizeText(
        row.propertyId || row.property_id || row.id,
        120,
      );
      if (!propertyId || (allowedIds.size && !allowedIds.has(propertyId))) {
        return null;
      }
      const wantsToView = row.wantsToView === true ||
        normalizeText(row.wantsToView).toLowerCase() === "yes";
...
function normalizeWindows(value: unknown) {
  if (Array.isArray(value)) {
    return value.map((item) => normalizeText(item, 160)).filter(Boolean).slice(
      0,
      5,
    );
  }
  ...
  notes
    ...
    .forEach((line) => {
      ...
      if (key) {
        ...
        parsed[activeKey] = [parsed[activeKey], normalizeText(line)].filter(
          Boolean,
        ).join("\n");
      }
    });
...
function buildNoteBlock(plan: JsonRecord) {
  return [
    ...
    `Buyer availability windows: ${
      normalizeText(plan.availabilityWindows, 1000)
    }`,
    ...
    `Seller coordination notes: ${
      normalizeText(plan.sellerCoordinationNotes, 1200)
    }`,
    ...
    `Seller availability requested at: ${
      normalizeText(plan.sellerRequestedAt, 80)
    }`,
    `Viewing appointments booked at: ${normalizeText(plan.bookedAt, 80)}`,
    `Buyer email: ${normalizeText(plan.recipientEmail, 320)}`,
    `Buyer email delivery status: ${
      normalizeText(plan.buyerEmailDeliveryStatus, 80)
    }`,
    `Buyer email delivery id: ${normalizeText(plan.buyerEmailDeliveryId, 120)}`,
    `Buyer email provider message id: ${
      normalizeText(plan.buyerEmailProviderMessageId, 120)
    }`,
    `Buyer email delivery failure: ${
      normalizeText(plan.buyerEmailDeliveryFailure, 800)
    }`,
    `Seller email delivery status: ${
      normalizeText(plan.sellerEmailDeliveryStatus, 80)
    }`,
    `Seller email delivery ids: ${
      normalizeText(plan.sellerEmailDeliveryIds, 1000)
    }`,
    `Seller email provider message ids: ${
      normalizeText(plan.sellerEmailProviderMessageIds, 1000)
    }`,
    `Seller email delivery failure: ${
      normalizeText(plan.sellerEmailDeliveryFailure, 800)
    }`,
...
Deno.serve(async (req) => {
  ...
  if (!supabaseUrl || !anonKey || !serviceRoleKey) {
    return jsonResponse(503, {
      error: "Viewing preferences are temporarily unavailable.",
  ...
  try {
    ...
    const tooLarge =
      normalizeText((error as Error)?.message) === "request_too_large";
  ...
  if (action === "create") {
    const organisationId = normalizeUuid(
    ...
    if (!user?.id) {
      return jsonResponse(401, {
        error: "Sign in before creating a viewing link.",
        code: "not_authenticated",
      });
    }
    if (!organisationId || !leadId) {
      return jsonResponse(400, {
        error: "A valid organisation and lead are required.",
    ...
    const membership = await supabase
      ...
      .maybeSingle();
    if (membership.error) {
      console.error("[buyer-viewing-preferences] membership lookup failed", {
        code: membership.error.code,
        message: membership.error.message,
      });
      return jsonResponse(500, {
        error: "We could not verify workspace access.",
    ...
    const selectedPropertyIds =
      (Array.isArray(body.selectedPropertyIds) ? body.selectedPropertyIds : [])
    ...
    const expiresInDays = Math.min(
      Math.max(Number(body.expiresInDays || body.expires_in_days || 14), 1),
      45,
    );
    ...
    const insert = await supabase
      ...
      .insert({
        ...
        lead_id: leadId,
        contact_email:
          normalizeText(body.buyerEmail || body.contactEmail || body.to, 320)
            .toLowerCase() || null,
        organisation_name:
          normalizeText(body.organisationName || body.organisation_name, 180) ||
          null,
        buyer_name: normalizeText(body.buyerName || body.recipientName, 180) ||
          null,
      ...
      .single();
    if (insert.error) {
      console.error("[buyer-viewing-preferences] create failed", {
        code: insert.error.code,
        message: insert.error.message,
      });
      return jsonResponse(500, {
        error: "We could not create the viewing link.",
        code: "create_failed",
      });
    }
    ...
    return jsonResponse(200, {
      ...
      preferenceLink: `${origin}/viewing-preferences/${
  ...
  if (!LINK_TOKEN_PATTERN.test(token)) {
    return jsonResponse(404, {
      error: "This viewing link is unavailable.",
  ...
  const linkResult = await supabase
    .from("buyer_viewing_preference_links")
    .select(
      "id, organisation_id, lead_id, created_by, contact_email, organisation_name, buyer_name, agent_name, agent_email, status, selected_property_ids, properties, response, created_at, last_sent_at, expires_at, submitted_at",
    )
    .eq("token_hash", tokenHash)
    .maybeSingle();
  if (linkResult.error) {
    console.error("[buyer-viewing-preferences] resolve failed", {
      code: linkResult.error.code,
      message: linkResult.error.message,
    });
    return jsonResponse(500, {
      error: "We could not load this viewing link.",
      code: "resolve_failed",
    });
  }
  ...
  const expired =
    new Date(normalizeText(link.expires_at)).getTime() < Date.now();
  if (expired && link.status === "pending") {
    await supabase.from("buyer_viewing_preference_links").update({
      status: "expired",
  ...
  if (action === "resolve") {
    return jsonResponse(200, { ok: true, session: publicSession });
  }
  if (action !== "submit") {
    return jsonResponse(400, {
  ...
  if (expired || link.status !== "pending") {
    return jsonResponse(409, {
      error: "This viewing link is no longer accepting responses.",
      code: "link_closed",
  ...
  const properties = Array.isArray(link.properties)
    ? link.properties as JsonRecord[]
    : [];
  const allowedIds = new Set(
    (Array.isArray(link.selected_property_ids)
  ...
  const responseNotes = normalizeText(
    body.responseNotes || body.response_notes,
    1200,
  );
  if (!confirmedPropertyIds.length) {
    return jsonResponse(400, {
      error: "Choose at least one property you would like to view.",
      code: "properties_required",
    });
  }
  if (!availabilityWindows.length) {
    return jsonResponse(400, {
      error: "Add at least one preferred viewing time.",
  ...
  if (availabilityWindows.length !== 3) {
    return jsonResponse(400, {
      ...
      code: "three_availability_windows_required",
    });
  }
    ...
    if (agentEmail) {
      try {
        await invokeSendEmailFunction({
          type: "buyer_viewing_times_submitted_agent",
          eventKind: "buyer_viewing_times_submitted_agent",
          to: agentEmail,
          recipientName: normalizeText(link.agent_name, 180) || "there",
          organisationId,
          organisationName: normalizeText(link.organisation_name, 180),
          leadId,
          leadName: normalizeText(link.buyer_name, 180) || "Buyer lead",
          ...
          actionLink: `${appOrigin()}/agency/pipeline?leadId=${
            encodeURIComponent(leadId)
          }`,
          idempotencyKey: `buyer-viewing-times-submitted:${link.id}:${now}`,
          metadata: {
        ...
        console.error("[buyer-viewing-preferences] agent notification failed", {
