...
create index if not exists partner_routing_rules_relationship_idx
  on public.partner_routing_rules (relationship_id)
  where relationship_id is not null;

comment on column public.partner_routing_rules.relationship_id is
  'Accepted organisation partner relationship used to validate direct-consultant routing targets.';
...
begin
  ...
  select ou.organisation_id
   ...
   where ou.user_id = auth.uid()
     and coalesce(ou.status, ou.membership_status, 'active') = 'active'
    ...
    select coalesce(jsonb_agg(person order by person->>'full_name'), '[]'::jsonb)
      ...
      from (
        select jsonb_build_object(
          ...
          'branch_id', b.id,
          'branch_name', b.name,
          'is_active', coalesce(ou.status, ou.membership_status, 'active') = 'active'
        ) as person
        from public.organisation_users ou
  ...
  if v_can_view_branch_managers then
    select coalesce(jsonb_agg(person order by person->>'branch_name', person->>'full_name'), '[]'::jsonb)
      ...
      from (
        ...
        where ou.organisation_id = v_partner_organisation_id
          and coalesce(ou.status, ou.membership_status, 'active') = 'active'
          and lower(coalesce(nullif(ou.workspace_role, ''), nullif(ou.organisation_role, ''), nullif(ou.role, ''))) in (
            'branch_manager',
            'manager',
            'agency_manager',
            'bond_originator',
