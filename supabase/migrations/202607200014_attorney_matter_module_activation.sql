...
select
  ...
  'cancellation',
  coalesce(transfer_department.is_active, true)
from public.attorney_firms f
left join public.attorney_firm_departments transfer_department
  on transfer_department.firm_id = f.id
  and transfer_department.department_type = 'transfer'
...
begin
  ...
  values
    ...
    (target_firm_id, 'Cancellation Department', 'cancellation', 'cancellation' = any(normalized_active_types)),
