-- Corrective migration for non-runnable row 202607220004.
-- Reconciles the Phase 2 canonical OTP signing guard objects under a new
-- reviewed version because the historical migration is already partially live.
...
begin
  ...
  if not found
     ...
     or v_dispatch.dispatch_kind is distinct from (
       case when coalesce(p_is_resend, false) then 'resend' else 'initial' end
     ) then
