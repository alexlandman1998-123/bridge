-- Corrective migration for non-runnable row 202607270012.
-- Live Phase 20 audit showed only the old attorney workflow step updater and
-- both lifecycle constraints present; the canonical lifecycle helper functions
-- were absent. This new version restates the reviewed target state and
-- normalizes legacy workflow stages before validating the canonical constraint.
  ...
  if to_regclass('public.transactions') is not null
    and exists (
      ...
      where c.conrelid = 'public.transactions'::regclass
        and c.conname = 'transactions_current_main_stage_check'
    )
  then
...
on conflict (transaction_id) do update set
  current_stage = case
    when lower(coalesce(excluded.status, '')) = 'completed'
      and transaction_lifecycle_workflows.current_stage in (
        'instruction',
        'documents',
        'finance',
        'transfer_duty',
      ...
      then transaction_lifecycle_workflows.current_stage
...
alter table if exists public.transaction_lifecycle_workflows
  validate constraint transaction_lifecycle_workflows_stage_check;
...
comment on function public.bridge_update_attorney_workflow_step(uuid, text, uuid, text, text, text, jsonb) is
  'Atomically updates an attorney workflow step, advances lane focus, recomputes matter lifecycle, writes lane history, and emits a transaction audit event.';
