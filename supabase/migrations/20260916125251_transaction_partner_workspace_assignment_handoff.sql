...
begin
  ...
  into v_agency_organisation_id
  from public.transactions
  where id = new.transaction_id;

  if v_agency_organisation_id is null then
    return new;
  end if;

  v_delivery_type := case
    when new.role_type = 'bond_originator' then 'bond_application_request'
    else 'attorney_instruction'
  end;
  v_service_type := case
    when new.role_type = 'bond_originator' then 'bond_origination'
    else 'property_transfers'
  end;
  ...
  if not exists (
    ...
    insert into public.transaction_partner_assignments (
      ...
      new.transaction_id,
      v_agency_organisation_id,
      v_partner_organisation_id,
      null,
      v_service_type,
      new.role_type,
      null,
