...
create table if not exists public.rental_financial_charges (
  id uuid primary key default gen_random_uuid(), organisation_id uuid not null references public.organisations(id) on delete restrict, tenancy_id uuid not null references public.rental_tenancies(id) on delete restrict, currency_code text not null default 'ZAR' check (currency_code = 'ZAR'), charge_type text not null check (charge_type in ('opening_balance', 'rent', 'deposit', 'utility', 'fee', 'adjustment_debit', 'adjustment_credit')), effective_date date not null, due_date date, amount numeric(14,2) not null check (amount > 0), description text not null check (length(btrim(description)) > 0), source_key text, reversal_of_charge_id uuid references public.rental_financial_charges(id) on delete restrict, created_by uuid not null references auth.users(id) on delete restrict, created_at timestamptz not null default now(), unique(tenancy_id, source_key)
);
...
create table if not exists public.rental_financial_payments (
  id uuid primary key default gen_random_uuid(), organisation_id uuid not null references public.organisations(id) on delete restrict, tenancy_id uuid not null references public.rental_tenancies(id) on delete restrict, currency_code text not null default 'ZAR' check (currency_code = 'ZAR'), received_date date not null, amount numeric(14,2) not null check (amount > 0), payment_reference text not null check (length(btrim(payment_reference)) > 0), payment_method text not null check (payment_method in ('bank_transfer', 'cash', 'card', 'other')), evidence_link text, reversal_of_payment_id uuid references public.rental_financial_payments(id) on delete restrict, created_by uuid not null references auth.users(id) on delete restrict, created_at timestamptz not null default now(), unique(organisation_id, payment_reference)
);
...
create table if not exists public.rental_financial_allocations (
  id uuid primary key default gen_random_uuid(), organisation_id uuid not null references public.organisations(id) on delete restrict, tenancy_id uuid not null references public.rental_tenancies(id) on delete restrict, payment_id uuid not null references public.rental_financial_payments(id) on delete restrict, charge_id uuid not null references public.rental_financial_charges(id) on delete restrict, amount numeric(14,2) not null check (amount > 0), allocated_on date not null default current_date, created_by uuid not null references auth.users(id) on delete restrict, created_at timestamptz not null default now(), unique(payment_id, charge_id, id)
);
...
create table if not exists public.rental_financial_adjustments (
  id uuid primary key default gen_random_uuid(), organisation_id uuid not null references public.organisations(id) on delete restrict, tenancy_id uuid not null references public.rental_tenancies(id) on delete restrict, currency_code text not null default 'ZAR' check (currency_code = 'ZAR'), adjustment_type text not null check (adjustment_type in ('debit', 'credit')), effective_date date not null, amount numeric(14,2) not null check (amount > 0), reason text not null check (length(btrim(reason)) > 0), reversal_of_adjustment_id uuid references public.rental_financial_adjustments(id) on delete restrict, created_by uuid not null references auth.users(id) on delete restrict, created_at timestamptz not null default now()
);
...
returns trigger language plpgsql security invoker set search_path = '' as $$ begin raise exception 'Rental financial rows are append-only; use a compensating entry or reversal command'; end; $$;
alter table public.rental_financial_charges enable row level security; alter table public.rental_financial_payments enable row level security; alter table public.rental_financial_allocations enable row level security; alter table public.rental_financial_adjustments enable row level security;
