...
declare
  ...
  v_counter_terms jsonb := case
    when p_counter_terms is not null and jsonb_typeof(p_counter_terms) = 'object' then p_counter_terms
    else '{}'::jsonb
  end;
...
begin
     ...
     set status = v_decision,
         ...
         metadata_json = coalesce(metadata_json, '{}'::jsonb) ||
           jsonb_strip_nulls(jsonb_build_object(
             ...
             'counterTerms', case when v_decision = 'countered' and v_counter_terms <> '{}'::jsonb then v_counter_terms else null end
           ))
   where id = v_session.id
     ...
     set status = v_next_offer_status,
         ...
         conditions_json = coalesce(conditions_json, '{}'::jsonb) ||
           jsonb_strip_nulls(jsonb_build_object(
             ...
             'sellerCounterTerms', case when v_decision = 'countered' and v_counter_terms <> '{}'::jsonb then v_counter_terms else null end,
             'sellerCounterRequestedAt', case when v_decision = 'countered' then now() else null end
           ))
   where id = v_offer.id
  ...
  if v_offer.buyer_lead_id is not null then
    if v_decision = 'accepted' then
      ...
      if v_counter_terms <> '{}'::jsonb then
        v_activity_note := v_activity_note || ' Structured counter terms were supplied.';
      end if;
    else
      v_stage := 'Lost';
...
as $$
begin
  return public.bridge_submit_seller_offer_decision(p_token, p_decision, p_notes, null);
end;
$$;
