-- The public development landing is a deliberate, curated projection. Cover
-- images are uploaded as a distinct document type, so include that public
-- asset alongside the existing public marketing files.
...
as $$
  ...
  select jsonb_build_object(
    ...
    'inventory', coalesce((
      ...
      from public.units u where u.development_id = published.id
    ), '[]'::jsonb),
    'assets', coalesce((
      select jsonb_agg(jsonb_build_object(
