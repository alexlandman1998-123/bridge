...
begin
  ...
  select lease.* into lease_row from public.rental_leases lease where lease.tenancy_id = tenancy_row.id for update;
  if not found then raise exception 'Tenancy has no lease'; end if;
  select unit.* into unit_row from public.rental_units unit where unit.id = tenancy_row.unit_id for update;
  ...
  if tenancy_row.status = 'active' then
    ...
    select id into activation_event_id from public.rental_tenancy_activation_events where tenancy_id = tenancy_row.id;
    return jsonb_build_object(
      'tenancy_id', tenancy_row.id, 'lease_id', lease_row.id, 'unit_id', unit_row.id,
      'activation_event_id', activation_event_id, 'status', 'active', 'idempotent', true
    );
  end if;
  ...
  update public.rental_units set status = 'occupied', active_tenancy_id = tenancy_row.id where id = unit_row.id;
  ...
  update public.rental_vacancy_marketing set status = 'archived'
  ...
  return jsonb_build_object(
    'tenancy_id', tenancy_row.id, 'lease_id', lease_row.id, 'unit_id', unit_row.id,
    'vacancy_id', vacancy_row.id, 'activation_event_id', activation_event_id,
