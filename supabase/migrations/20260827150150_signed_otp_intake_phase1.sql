...
create or replace function public.bridge_can_create_assigned_buyer_intake(
  target_org uuid,
  target_lead uuid,
  target_listing uuid
)
...
as $$
  select coalesce(exists (
    ...
    where ou.organisation_id = target_org
      ...
      and (
        public.bridge_can_create_transaction_override(target_org)
        or exists (
          select 1
          from public.leads lead
          where lead.organisation_id = target_org
            and lead.lead_id = target_lead
            and coalesce(lead.assigned_agent_id, lead.assigned_user_id) = auth.uid()
        )
        or exists (
          select 1
          from public.private_listings listing
          where listing.organisation_id = target_org
            and listing.id = target_listing
            and listing.assigned_agent_id = auth.uid()
        )
      )
  ), false)
$$;
...
declare
  ...
  v_signed_otp_evidence jsonb := coalesce(v_payload->'signed_otp_intake_evidence', '{}'::jsonb);
  v_signed_otp_intake boolean := false;
  v_buyer_onboarding_intake boolean := false;
  v_assigned_buyer_intake boolean := false;
...
begin
  ...
  v_signed_otp_intake := v_offer_id is null and v_creation_mode = 'signed_otp_intake';
  v_buyer_onboarding_intake := v_offer_id is null and v_creation_mode = 'buyer_onboarding_intake';
  v_assigned_buyer_intake := v_signed_otp_intake or v_buyer_onboarding_intake;
  v_allow_direct_lead_conversion := v_offer_id is null and v_creation_mode in (
    'onboarding_capture',
    'manual_intake',
    'manual_capture',
    'buyer_onboarding_intake',
    'signed_otp_intake'
  );

  if v_allow_direct_lead_conversion and not v_assigned_buyer_intake then
  ...
  if v_assigned_buyer_intake then
    if v_signed_otp_intake then
      if nullif(trim(coalesce(
        v_signed_otp_evidence->>'storagePath',
        v_signed_otp_evidence->>'storage_path',
        v_signed_otp_evidence->>'filePath',
        v_signed_otp_evidence->>'file_path'
      )), '') is null
        or nullif(trim(coalesce(
          v_signed_otp_evidence->>'uploadedAt',
          v_signed_otp_evidence->>'uploaded_at'
        )), '') is null
        or lower(coalesce(
          v_signed_otp_evidence->>'signedByAllPartiesConfirmed',
          v_signed_otp_evidence->>'signed_by_all_parties_confirmed',
          'false'
        )) <> 'true'
        or lower(coalesce(
          v_signed_otp_evidence->>'arch9TermsIncludedConfirmed',
          v_signed_otp_evidence->>'arch9_terms_included_confirmed',
          'false'
        )) <> 'true' then
        raise exception 'Signed OTP intake requires complete upload evidence and confirmations.' using errcode = '22023';
      end if;
    end if;

    if not public.bridge_can_create_assigned_buyer_intake(v_organisation_id, v_lead_id, v_listing_id) then
      raise exception 'Buyer intake requires the assigned agent or an authorised principal/admin actor.' using errcode = '42501';
    end if;
    ...
    if v_signed_otp_intake then
      v_profile := jsonb_set(
        ...
        jsonb_build_object(
          'version', 'arch9_signed_otp_intake_authorization_v1',
          'actorId', auth.uid(),
          'actorRole', coalesce(v_override_actor_role, 'agent'),
          'authorised', true,
          'evidence', v_signed_otp_evidence,
          'source', 'bridge_create_mvp_transaction'
        ),
        true
      );
      ...
      v_profile := jsonb_set(
        ...
        jsonb_build_object(
          'version', 'arch9_buyer_onboarding_intake_authorization_v1',
          'actorId', auth.uid(),
          'actorRole', coalesce(v_override_actor_role, 'agent'),
          'authorised', true,
          'source', 'bridge_create_mvp_transaction'
...
revoke all on function public.bridge_can_create_assigned_buyer_intake(uuid, uuid, uuid) from public, anon, authenticated;
