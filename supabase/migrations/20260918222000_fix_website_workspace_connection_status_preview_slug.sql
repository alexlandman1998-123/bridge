-- Qualify the site fields because RETURNS TABLE exposes preview_slug as a
-- PL/pgSQL variable as well as a column name on website_sites.
...
begin
  ...
  with connection as (
    ...
    select ws.id, ws.preview_slug, ws.status, ws.template_key, ws.published_revision_id, ws.updated_at
    from public.website_sites ws
