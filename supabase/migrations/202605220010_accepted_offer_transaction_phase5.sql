alter table if exists public.transactions
  ...
  add column if not exists originating_lead_id uuid references public.leads(lead_id) on delete set null,
...
begin
  ...
  if v_decision = 'accepted' then
    ...
    if v_transaction_id is null then
      ...
      values (
        ...
        'Transaction created automatically from seller-accepted buyer offer.',
        v_agent_name,
        v_agent_email,
  ...
  if v_offer.buyer_lead_id is not null then
    if v_decision = 'accepted' then
      v_stage := case when v_transaction_id is not null then 'Finance' else 'Offer Accepted' end;
      ...
      v_activity_note := case
        when v_transaction_id is not null
          then 'Seller accepted the buyer offer. Bridge created the transaction and prepared buyer onboarding.'
