...
declare
  ...
  v_artifact jsonb;
  v_launch jsonb;
  v_conditional_master_ready boolean;
  v_conditional_signing_ready boolean;
...
begin
  ...
  for update;
  if not found then
    raise exception 'Generated packet version not found.' using errcode = 'P0002';
  end if;
  ...
  if not found then
    raise exception 'Packet template was not found.' using errcode = 'P0002';
  end if;
  ...
  v_launch := coalesce(
    v_validation->'mandateTemplateLaunchReadiness',
    v_validation->'mandate_template_launch_readiness',
    v_validation#>'{generationPayload,mandateTemplateLaunchReadiness}',
    v_validation#>'{generationPayload,mandate_template_launch_readiness}',
    '{}'::jsonb
  );
  v_conditional_master_ready :=
    coalesce(v_render->>'conditionalMasterCoverageReady', '') = 'true'
    or coalesce(v_render->>'conditional_master_coverage_ready', '') = 'true'
    or coalesce(v_launch->>'conditionalMasterCoverageReady', '') = 'true'
    or coalesce(v_launch->>'conditional_master_coverage_ready', '') = 'true'
    or coalesce(v_launch->>'canGenerateWithoutFallback', '') = 'true';
  v_conditional_signing_ready :=
    coalesce(v_render->>'conditionalSigningCanPrepare', '') = 'true'
    or coalesce(v_render->>'conditional_signing_can_prepare', '') = 'true'
    or coalesce(v_launch->>'conditionalSigningCanPrepare', '') = 'true'
    or coalesce(v_launch->>'conditional_signing_can_prepare', '') = 'true'
    or (
      coalesce(v_render->>'legalDocumentScenarioComplete', '') = 'true'
      and coalesce(v_launch->>'shouldBlockGeneration', 'false') <> 'true'
    );
  ...
  insert into public.document_packet_events (
    ...
    v_version.id,
    'native_structured_legal_pdf_certified',
    jsonb_build_object(
      'contract', 'native-structured-d3-v1',
...
comment on function public.bridge_certify_native_structured_legal_pdf(uuid, uuid) is
  'Certifies native structured OTP/mandate PDFs when artifact provenance, approved template digests, and launch-readiness evidence match the active published template.';
