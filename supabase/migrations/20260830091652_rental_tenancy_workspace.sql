...
declare tenancy_row public.rental_tenancies%rowtype; property_row public.rental_properties%rowtype; unit_row public.rental_units%rowtype; lease_row public.rental_leases%rowtype; lease_version_row public.rental_lease_versions%rowtype; inspection_row public.rental_incoming_inspections%rowtype; activation_row public.rental_tenancy_activation_events%rowtype; readiness_total integer := 0; readiness_complete integer := 0;
begin
  ...
  if not found or not public.rental_branch_access(property_row.organisation_id, property_row.branch_id) then raise exception 'You are not authorized for this tenancy'; end if;
  ...
  select count(*), count(*) filter (where status in ('verified', 'waived')) into readiness_total, readiness_complete from public.rental_move_in_readiness_items where tenancy_id = tenancy_row.id;
...
returns jsonb language plpgsql security definer set search_path = '' as $$
declare tenancy_row public.rental_tenancies%rowtype; max_rows integer := least(greatest(coalesce(p_limit, 40), 1), 100); result jsonb;
begin
  if auth.uid() is null then raise exception 'Authentication is required'; end if;
  select tenancy.* into tenancy_row from public.rental_tenancies tenancy where tenancy.id = p_tenancy_id;
  ...
  select coalesce(jsonb_agg(entry order by (entry ->> 'occurred_at') desc), '[]'::jsonb) into result from (
    select entry from (
      ...
      union all select jsonb_build_object('type', event.event_type, 'occurred_at', event.occurred_at, 'detail', event.evidence_json) from public.rental_incoming_inspection_events event where event.tenancy_id = tenancy_row.id
      union all select jsonb_build_object('type', concat('readiness_', event.to_status), 'occurred_at', event.occurred_at, 'detail', event.evidence_json) from public.rental_move_in_readiness_events event where event.tenancy_id = tenancy_row.id
      union all select jsonb_build_object('type', event.event_type, 'occurred_at', event.occurred_at, 'detail', event.evidence_json) from public.rental_lease_signing_events event join public.rental_lease_versions version on version.id = event.lease_version_id join public.rental_leases lease on lease.id = version.lease_id where lease.tenancy_id = tenancy_row.id
...
begin
  ...
  select coalesce(jsonb_agg(document order by (document ->> 'created_at') desc), '[]'::jsonb) into result from (
    ...
    union all select jsonb_build_object('type', 'condition_media', 'label', coalesce(media.caption, 'Condition evidence'), 'link', media.media_link, 'created_at', media.created_at) from public.rental_incoming_inspection_media media join public.rental_incoming_inspections inspection on inspection.id = media.inspection_id where inspection.tenancy_id = tenancy_row.id
    union all select jsonb_build_object('type', 'handover_evidence', 'label', replace(item.item_key, '_', ' '), 'link', item.evidence_link, 'created_at', item.completed_at) from public.rental_incoming_inspection_items item join public.rental_incoming_inspections inspection on inspection.id = item.inspection_id where inspection.tenancy_id = tenancy_row.id and item.evidence_link is not null
    union all select jsonb_build_object('type', 'readiness_evidence', 'label', replace(item.obligation_type, '_', ' '), 'link', item.evidence_link, 'created_at', item.reviewed_at) from public.rental_move_in_readiness_items item where item.tenancy_id = tenancy_row.id and item.evidence_link is not null
  ) documents;
  return result;
end; $$;
