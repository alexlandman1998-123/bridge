...
-- The seller portal is an anonymous, token-scoped browser surface.  It may
-- receive a final-mandate identity, but never an F2 storage coordinate.  The
-- pre-Phase-4 RPC returned `to_jsonb(ver)` plus direct final file fields,
-- which leaked a final PDF path/bucket/URL to browser network responses even
-- when the React mapper later removed it.  Keep the prior phase-1 source
-- implementation private and replace only the public stable-token wrapper
-- with a deliberately whitelisted projection.
...
as $$
  select coalesce(
    ...
    -- Some legacy mirror rows contain only a signed-mandate URL.  They cannot
    -- be proven safe for the generic downloader, so withhold them too.
    or (
...
-- The RPC contains several historical JSON blobs (listing form data,
-- onboarding form data, requirement metadata).  Remove every known final
-- artifact transport key recursively before returning any of those blobs, so
-- a newly introduced nested alias cannot bypass the packet/document fence.
...
-- Key aliases are not the only historical shape.  Also remove any scalar
-- value which contains the exact known F2 path or URL.  That closes legacy
-- onboarding/form-data blobs whose field name is arbitrary but whose value
-- still embeds a storage coordinate (for example a signed URL containing the
-- final object path).
...
begin
  ...
  -- An auth challenge contains no document data and must retain its existing
  -- shape so the portal can continue its password/session flow.
  ...
  -- Do not return `to_jsonb(pkt)` or `to_jsonb(ver)`: packet source context
  -- can contain `finalSignedArtifactPath`, and a version contains all F2
  -- storage coordinates.  These small whitelists are the complete safe
  -- packet/version contract needed by the resolver and portal presentation.
  ...
  -- Strip any legacy/private-listing row that aliases the final F2 artifact,
  -- then append exactly one identity-only descriptor.  A type/stage match is
  -- deliberately treated as canonical too: failing closed is safer than
  -- sending an ambiguous signed-mandate path to the browser.
...
comment on function public.bridge_private_listing_seller_portal_payload(text, text, boolean) is
  'Phase 4 seller portal contract: returns only a final-artifact descriptor (packet/version/document IDs); F2 path, bucket, URL, and raw version JSON remain server-only.';
