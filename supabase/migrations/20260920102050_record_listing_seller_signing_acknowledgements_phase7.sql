-- Phase 7: acknowledgement choices are evidence, not transient browser state.
-- They are persisted in the same transaction as a pack signature so an audit
-- export can prove which documents the signer accepted without retaining a URL.
alter table public.private_listing_mandate_signing_sessions
  add column if not exists signing_acknowledgements jsonb not null default '{}'::jsonb;

alter table public.private_listing_mandate_signing_sessions
  drop constraint if exists private_listing_mandate_signing_sessions_acknowledgements_check;

alter table public.private_listing_mandate_signing_sessions
  add constraint private_listing_mandate_signing_sessions_acknowledgements_check
  check (jsonb_typeof(signing_acknowledgements) = 'object');
...
begin
  ...
  v_acknowledgements := v_acknowledgements || jsonb_build_object('contract', 'seller_signing_acknowledgements_v1', 'recordedAt', v_signed_at);
  update public.private_listing_mandate_signing_sessions
  set document_progress = v_progress, signing_acknowledgements = v_acknowledgements,
      status = 'signed', signed_at = v_signed_at, used_at = v_signed_at,
  ...
  return jsonb_build_object('signedAt', v_signed_at, 'complete', true, 'groupComplete', v_group_complete, 'sellerPortalWorkspace', v_portal_workspace, 'progress', v_progress, 'acknowledgements', v_acknowledgements);
end;
$$;
