-- Phase 2 completion: expose lightweight buyer/agent context from the
-- post-viewing offer portal RPC so submitted offers can notify the agent.
...
begin
  ...
  if v_session.agent_id is not null and to_regclass('public.profiles') is not null then
    select jsonb_build_object(
      'id', p.id,
      'email', coalesce(to_jsonb(p)->>'email', ''),
      'name', coalesce(to_jsonb(p)->>'full_name', to_jsonb(p)->>'name', to_jsonb(p)->>'email', 'Assigned agent')
    )
      into v_agent
    from public.profiles p
    where p.id = v_session.agent_id
    limit 1;
  end if;

  if v_session.buyer_contact_id is not null and to_regclass('public.contacts') is not null then
    select jsonb_build_object(
      'id', c.contact_id,
    ...
    where c.contact_id = v_session.buyer_contact_id
    limit 1;
  end if;
