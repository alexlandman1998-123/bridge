begin;
-- General spine access includes clients. Internal task keys need positive
-- professional assignment/membership, not merely any matter participation.
create or replace function journey_private.can_read_professional_journey(target_transaction_id uuid)
returns boolean
language sql
stable
security definer
      ...
      and (
        ...
        or exists (
          ...
          where tp.transaction_id = target_transaction_id
            and lower(coalesce(tp.role_type,'')) in ('agent','agency','developer','attorney','transfer_attorney','bond_attorney','cancellation_attorney','bond_originator','bond_registration_attorney','cancellation')
        ...
        or exists (
          ...
          where trp.transaction_id = target_transaction_id
            and lower(coalesce(trp.role_type,'')) in ('agent','agency','developer','attorney','transfer_attorney','bond_attorney','cancellation_attorney','bond_originator','bond_registration_attorney','cancellation')
...
revoke all on function journey_private.can_read_professional_journey(uuid) from public,anon,authenticated;
create or replace function public.bridge_read_professional_matter_journey(p_transaction_id uuid)
returns jsonb language plpgsql stable security definer set search_path = '' as $$
begin
  if auth.uid() is null or not coalesce(journey_private.can_read_professional_journey(p_transaction_id),false) then
