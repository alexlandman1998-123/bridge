...
create table if not exists public.lead_referrals (
  ...
  referral_type text not null default 'client_referral',
  related_listing_id uuid references public.private_listings(id) on delete set null,
  source_branch_id uuid references public.organisation_branches(id) on delete set null,
  ...
  -- CRM deal storage is deployment-optional; keep the cross-module identifier
  -- without a hard foreign key so referral deployment is independently safe.
  ...
  constraint lead_referrals_referral_type_check
  ...
  constraint lead_referrals_protection_period_days_check
...
alter table if exists public.lead_referrals add column if not exists referral_type text not null default 'client_referral';
alter table if exists public.lead_referrals add column if not exists related_listing_id uuid references public.private_listings(id) on delete set null;
alter table if exists public.lead_referrals add column if not exists source_branch_id uuid references public.organisation_branches(id) on delete set null;
...
alter table if exists public.lead_referrals add column if not exists target_branch_id uuid references public.organisation_branches(id) on delete set null;
...
alter table if exists public.lead_referrals add column if not exists converted_deal_id uuid;
...
alter table if exists public.lead_referrals add column if not exists protection_period_days integer not null default 30;
alter table if exists public.lead_referrals add column if not exists accepted_at timestamptz;
alter table if exists public.lead_referrals add column if not exists accepted_by_user_id uuid references public.profiles(id) on delete set null;
alter table if exists public.lead_referrals add column if not exists accepted_by_email text;
alter table if exists public.lead_referrals add column if not exists declined_at timestamptz;
alter table if exists public.lead_referrals add column if not exists declined_by_user_id uuid references public.profiles(id) on delete set null;
alter table if exists public.lead_referrals add column if not exists declined_by_email text;
alter table if exists public.lead_referrals add column if not exists decline_reason text;
alter table if exists public.lead_referrals add column if not exists agreement_locked_at timestamptz;
...
alter table if exists public.lead_referrals drop constraint if exists lead_referrals_referral_type_check;
...
alter table if exists public.lead_referrals
  ...
  check (status in ('draft', 'sent', 'received', 'accepted', 'declined', 'needs_review', 'contacted', 'working', 'converted', 'lost', 'commission_due', 'paid', 'cancelled'));
alter table if exists public.lead_referrals drop constraint if exists lead_referrals_protection_period_days_check;
alter table if exists public.lead_referrals
...
create table if not exists public.referral_agreements (
  ...
  constraint referral_agreements_commission_split_percentage_check
    check (commission_split_percentage is null or commission_split_percentage between 0 and 100),
  constraint referral_agreements_protection_period_days_check
    check (protection_period_days between 1 and 3650)
);
...
alter table if exists public.referral_agreements add column if not exists protection_period_days integer not null default 30;
alter table if exists public.referral_agreements add column if not exists accepted_by_user_id uuid references public.profiles(id) on delete set null;
alter table if exists public.referral_agreements add column if not exists declined_by_user_id uuid references public.profiles(id) on delete set null;
alter table if exists public.referral_agreements add column if not exists declined_by_email text;
alter table if exists public.referral_agreements add column if not exists decline_reason text;
alter table if exists public.referral_agreements add column if not exists locked_at timestamptz;
...
alter table if exists public.referral_agreements drop constraint if exists referral_agreements_protection_period_days_check;
...
alter table if exists public.referral_invites add column if not exists declined_at timestamptz;
alter table if exists public.referral_invites add column if not exists declined_by_user_id uuid references public.profiles(id) on delete set null;
alter table if exists public.referral_invites add column if not exists decline_reason text;
...
alter table if exists public.referral_commission_events add column if not exists deal_id uuid;
...
$$;
drop function if exists public.bridge_respond_referral_invite(text, text, text, text);
...
$$;
drop trigger if exists referral_status_events_lead_activity_signal on public.referral_status_events;
create trigger referral_status_events_lead_activity_signal
...
grant execute on function public.bridge_respond_referral_invite(text, text, text, text, text) to anon, authenticated;
grant execute on function public.bridge_respond_referral_terms(uuid, text, text, text, jsonb) to authenticated;
