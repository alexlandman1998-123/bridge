-- Phase 2A: one durable, idempotent boundary for MVP transaction creation.
-- A transaction, its lead linkage and its accepted-offer linkage either commit
...
begin
  for v_item in select value from jsonb_array_elements(coalesce(p_bootstrap->'requirements', '[]'::jsonb)) loop
    insert into public.transaction_participant_requirements (
      ...
      required_by = excluded.required_by, required_at_creation = excluded.required_at_creation,
      label = excluded.label, reason = excluded.reason, updated_at = now();
  end loop;
  ...
  for v_item in select value from jsonb_array_elements(coalesce(p_bootstrap->'participants', '[]'::jsonb)) loop
    insert into public.transaction_participants (
      ...
      transaction_role = excluded.transaction_role, updated_at = now()
...
begin
  for v_item in select value from jsonb_array_elements(coalesce(p_bootstrap->'requirements', '[]'::jsonb)) loop
    ...
    insert into public.transaction_required_documents (
      ...
      'pending', true, v_item->>'groupKey', initcap(replace(v_item->>'groupKey', '_', ' ')), v_item->>'description',
      v_item->>'requiredFromRole', 'client', false, v_position
    ) on conflict (transaction_id, document_key) do update set
      document_label = excluded.document_label, is_required = excluded.is_required,
      group_key = excluded.group_key, group_label = excluded.group_label,
      description = excluded.description, required_from_role = excluded.required_from_role,
...
begin
  for v_lane in select value from jsonb_array_elements(coalesce(p_bootstrap->'lanes', '[]'::jsonb)) loop
    insert into public.transaction_workflow_lanes (
      ...
      current_stage = excluded.current_stage, owner_role = excluded.owner_role,
...
begin
  ...
  if v_lead_id is null or nullif(trim(v_payload->>'listing_id'), '') is null or v_idempotency_key is null then
  ...
  perform pg_advisory_xact_lock(hashtext(v_organisation_id::text || ':' || v_idempotency_key));

  select * into v_transaction
  from public.transactions
  where organisation_id = v_organisation_id
    and creation_idempotency_key = v_idempotency_key
  limit 1;
  ...
  if not v_existing and v_buyer_id is null then
    ...
    values (v_buyer_name, v_buyer_phone, v_buyer_email)
  ...
  if not v_existing then
    ...
    values (
      ...
      nullif(trim(v_payload->>'listing_id'), '')::uuid, v_lead_id, v_lead_id, v_offer_id,
  ...
  set converted_transaction_id = v_transaction.id,
      ...
      current_stage = 'Onboarding', stage = 'Onboarding', status = 'Onboarding', updated_at = now()
