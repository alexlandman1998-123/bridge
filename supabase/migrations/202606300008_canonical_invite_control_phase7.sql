...
as $$
  select exists (
    ...
    where p.id = auth.uid()
      and (
        lower(coalesce(p.role, '')) in ('platform_admin', 'super_admin', 'founder')
        or lower(coalesce(p.system_role, '')) in ('admin', 'super_admin', 'founder')
      )
  );
$$;
...
begin
  ...
  if v_partner_sync_gap_count > 0 or v_buyer_participant_gap_count > 0 then
    v_status := 'critical';
  elsif v_stale_pending_count > 0 or v_buyer_portal_sync_gap_count > 0 or v_seller_portal_sync_gap_count > 0 or v_duplicate_pending_count > 0 then
    v_status := 'warning';
  end if;
