...
create index if not exists document_packet_versions_signable_generated_idx
  ...
  where render_status = 'generated'
    ...
    and rendered_media_type = 'application/pdf';
...
begin
  ...
  if p_packet_id is not null then
    ...
    select packet_row.*
      into v_packet
      from public.document_packets packet_row
     where (p_organisation_id is null or packet_row.organisation_id = p_organisation_id)
       and (v_packet_type is null or lower(packet_row.packet_type) = v_packet_type)
       and (p_transaction_id is null or packet_row.transaction_id = p_transaction_id)
       and (p_lead_id is null or packet_row.lead_id = p_lead_id)
     order by
       case
         when lower(coalesce(packet_row.status, '')) in ('completed', 'partially_signed', 'sent', 'signing_prep') then 0
         when exists (
           select 1
             from public.document_packet_versions generated_version
            where generated_version.packet_id = packet_row.id
              and lower(coalesce(generated_version.render_status, '')) = 'generated'
              and generated_version.rendered_document_id is not null
              and (
                nullif(trim(coalesce(generated_version.rendered_file_path, '')), '') is not null
                or nullif(trim(coalesce(generated_version.rendered_file_url, '')), '') is not null
              )
              and lower(coalesce(generated_version.rendered_media_type, '')) = 'application/pdf'
              and coalesce(generated_version.transaction_pdf_persisted, false)
         ) then 1
         when lower(coalesce(packet_row.status, '')) = 'generated' then 2
         when lower(coalesce(packet_row.status, '')) = 'ready_for_generation' then 4
         when lower(coalesce(packet_row.status, '')) = 'draft' then 5
         when lower(coalesce(packet_row.status, '')) in ('archived', 'voided') then 9
         else 3
       end,
       packet_row.updated_at desc nulls last,
       packet_row.created_at desc
     limit 1;
  end if;
