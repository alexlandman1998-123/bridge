  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'sla_breached', q.sla_due_at < now() and q.assignment_status not in ('completed', 'cancelled')
      )
      order by case when q.sla_due_at < now() then 0 else 1 end, q.sla_due_at nulls last, q.reported_at
    ),
    '[]'::jsonb
  )
  from (
    select r.*, a.assignee_name, a.sla_due_at, a.status as assignment_status
