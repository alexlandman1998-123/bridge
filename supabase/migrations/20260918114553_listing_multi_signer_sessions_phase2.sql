-- A direct-listing seller pack may require more than one legally distinct
-- signer. Sessions stay individually tokenised, while the group keeps their
-- completion state tied to one frozen pack.
alter table public.private_listing_mandate_signing_sessions
  add column if not exists signing_group_id uuid;

create index if not exists private_listing_mandate_signing_sessions_group_idx
  on public.private_listing_mandate_signing_sessions (signing_group_id, status)
  where signing_group_id is not null;

-- A mandate is only marked signed after every session in its signing group has
-- completed. Existing one-signer sessions remain compatible because their
-- signing_group_id is null.
...
begin
  ...
  return jsonb_build_object('signedAt', v_signed_at, 'complete', true, 'groupComplete', v_group_complete, 'progress', v_progress);
end;
$$;
