...
-- Phase 5 canonical bond-originator handoff repair.
-- Consolidates the reviewed partner handoff and execution forms into one
-- canonical packet so later phases only track a single migration version.
...
upserted_workflows as (
  ...
  select
    ...
    'active',
    'bond_originator',
    'Bond originator intake created from canonical handoff repair',
    now(),
    now(),
    now()
...
inserted_applications as (
  ...
  select
    ...
    jsonb_build_object(
      'source',
      'canonical_bond_originator_handoff_repair',
      'sourceFiles',
      jsonb_build_array(
    ...
    'pending',
    'Bond application workspace repaired from canonical bond-originator handoff consolidation.',
    jsonb_build_object(
      'source',
      'canonical_bond_originator_handoff_repair',
      'sourceFiles',
...
application_scope as (
  ...
  union
  select tba.transaction_id
  from public.transaction_bond_applications tba
  join workflow_scope ws
    on ws.transaction_id = tba.transaction_id
  where tba.assigned_organisation_id = ws.assigned_organisation_id
    and coalesce(tba.assignment_status, tba.status, 'active') not in (
      'removed',
      'declined',
      'rejected',
...
set
  ...
  bond_assignment_source = coalesce(t.bond_assignment_source, 'canonical_bond_originator_handoff_repair'),
  ...
  updated_at = now()
from workflow_scope ws
join application_scope app
  on app.transaction_id = ws.transaction_id
