...
Use `scripts/supabase-phase7-production-execution.mjs` only after the exact version has passed staging and its staging ledger is recorded. Production mutations require the fixed production project identity, `SUPABASE_PRODUCTION_ACCESS_MODE=linked_ephemeral`, an exact linked-project identity match, explicit runtime recovery confirmation, reviewed Phase 12 recovery evidence, and a live CLI check proving PITR or at least one physical backup exists. The runner handles one exact version per invocation, enforces recorded stream dependencies, and separates SQL application from ledger recording. Static production database URLs and passwords are intentionally not used.

Every production invocation requires reviewed staging evidence, `docs/supabase-phase-7-staging-readiness.json`, and `migration-evidence/2026-07-20-production-recovery-phase12/production-database-recovery.json`. The readiness record must prove complete staging manifest coverage and evidence, a passing attorney integrity gate with zero blocking assignments, and explicit human approval. The recovery evidence must prove an accessible independent restore, matching database fingerprints, and the full production migration baseline. Ledger recording additionally requires production evidence proving the target state and catalog, behavior, and rollback/no-residue checks. Corrective and manual-review rows remain outside this runner; `repair_only_after_smoke` can be ledger-recorded with evidence but can never replay SQL.

Before production promotion, run `scripts/supabase-phase14-production-ledger-reconciliation.mjs --plan`. The reconciler recognizes the 17 reviewed 12-digit/legacy-`…00` timestamp aliases and permits only linked migration-ledger repairs. A passing state requires all canonical rows, no legacy aliases, a stable 433-row ledger, and zero genuine remote-only versions. Evidence is retained in `migration-evidence/2026-07-20-production-ledger-phase14/production-ledger-reconciliation.json`. This history repair does not replace per-migration Phase 7 promotion evidence.
...
Live closeout also requires `docs/supabase-phase-7-staging-readiness.json` to remain approved and passing, valid Phase 12 recovery evidence, plus `SUPABASE_PRODUCTION_RECOVERY_CONFIRMED=I_HAVE_TESTED_PRODUCTION_RECOVERY`. A listed backup without both tested-recovery evidence and runtime confirmation does not make freeze retirement eligible.
