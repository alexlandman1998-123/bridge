...
export default async function ArticlePage({ params }: Props) {
  ...
  return <ResourceShell site={site} href="/blog"><article><header className={`${styles.intro} ${styles.articleIntro} ${!article.coverImageUrl ? styles.articleIntroNoHero : ''} ${longTitle ? styles.articleIntroLongTitle : ''}`}><Link href="/blog" className={styles.back}>← Back to the journal</Link><p className={styles.eyebrow}>PROPERTY JOURNAL</p><div className={styles.articleMeta}><span>{article.authorName || site.name}</span><span>{readingMinutes(article.body)} min read</span><time dateTime={article.publishedAt}>{publishDate(article.publishedAt)}</time></div><h1>{article.title}</h1>{article.summary ? <div className={styles.articlePreview}><span>ARTICLE PREVIEW</span><p className={styles.articleDeck}>{article.summary}</p></div> : null}</header>{article.coverImageUrl ? <figure className={styles.articleHero}><Image src={article.coverImageUrl} alt={article.coverImageAlt || ''} fill priority sizes="(max-width: 1240px) 100vw, 1240px" /></figure> : null}<div className={styles.prose}>{renderedBlocks.length ? renderedBlocks.map((block) => {
