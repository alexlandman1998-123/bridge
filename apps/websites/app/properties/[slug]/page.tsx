  ...
  return (
    <main className={`property-page ${templateClassName(site.templateKey)}`} style={{ '--primary': site.primaryColor, '--secondary': site.secondaryColor, '--accent': site.accentColor } as React.CSSProperties}>
      ...
      <section className="property-hero">
        ...
        <div className="property-heading"><div><p className="eyebrow">{property.transactionType === 'rental' ? 'TO RENT' : 'FOR SALE'} · {property.reference}</p><h1>{property.title}</h1><p>{property.suburb}{property.province ? `, ${property.province}` : ''}</p></div><strong>{property.price ? money.format(property.price) : 'Price on request'}</strong></div>
      </section>
      <section className="property-content">
        <article>
          <dl className="fact-grid"><div><dt>Bedrooms</dt><dd>{property.bedrooms || '—'}</dd></div><div><dt>Bathrooms</dt><dd>{property.bathrooms || '—'}</dd></div><div><dt>Parking</dt><dd>{property.parkingBays || '—'}</dd></div><div><dt>Floor size</dt><dd>{property.floorSize ? `${property.floorSize} m²` : '—'}</dd></div></dl>
          <h2>About this property</h2>
          <p className="long-copy">{property.description || 'Further property details will be supplied by the listing agent.'}</p>
          {property.features.length > 0 ? <><h2>Features</h2><ul className="feature-list">{property.features.map((feature) => <li key={feature}>{feature}</li>)}</ul></> : null}
        </article>
        <aside id="enquire"><p className="eyebrow">REQUEST INFORMATION</p><h2>Speak to a property advisor.</h2><p>Ask a question or arrange a viewing. Your enquiry goes directly to this agency’s CRM.</p><LeadForm propertyId={property.id} /></aside>
      </section>
