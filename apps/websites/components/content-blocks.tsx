...
import { hasEditorialPropertyExperience, isHomeSeekersTemplate } from '@/lib/site-templates'
...
function Block({ block, page, properties, templateKey, site }: { block: WebsiteBlock; page: PublicPage; properties: PublicProperty[]; templateKey: WebsiteTemplateKey; site: ResolvedSite }) {
  if (block.type === 'hero') {
    const lwpHome = page.kind === 'home' && hasEditorialPropertyExperience(site)
  ...
  if (block.type === 'property_collection') { const selected = properties.filter((property) => !block.transactionType || property.transactionType === block.transactionType).slice(0, Math.min(Math.max(block.maxItems || 3, 1), 12)); return isHomeSeekersTemplate(templateKey) ? <FeaturedPropertyCarousel heading={hasEditorialPropertyExperience(site) ? 'Homes worth moving for.' : block.heading || 'Featured properties'} properties={selected} showConsultant={hasEditorialPropertyExperience(site)} /> : <section className="section"><div className="content-shell"><div className="section-heading"><div><h2>{block.heading || 'Featured properties'}</h2></div><a href="/properties">View all properties <span aria-hidden="true">→</span></a></div><div className="property-grid">{selected.map((property) => <PropertyCard key={property.id} property={property} />)}</div></div></section> }
...
export function ContentBlocks({ page, properties, templateKey, site, blogPosts = [] }: { page: PublicPage; properties: PublicProperty[]; templateKey: WebsiteTemplateKey; site: ResolvedSite; blogPosts?: PublicBlogPost[] }) {
  ...
  return <>
    {page.blocks.filter((block) => !block.hidden).map((block, index) => <Fragment key={`${block.type}-${index}`}><Block block={block} page={page} properties={properties} site={site} templateKey={templateKey} />{page.kind === 'home' && homeSeekers && !hasEditorialPropertyExperience(site) && block.type === 'rich_text' && <LatestBlogPosts posts={blogPosts} site={site} />}</Fragment>)}
    ...
    {page.kind === 'home' && hasEditorialPropertyExperience(site) && <LwpJournal posts={blogPosts} />}
  </>
}
