...
assert.equal(evidence.status, 'SUPABASE_PREVIEW_PASSED_VERCEL_RATE_LIMITED')
...
assert.equal(evidence.externalChecks.supabasePreview.status, 'PASS')
assert.match(evidence.externalChecks.supabasePreview.firstPassingCommit, /^[0-9a-f]{40}$/)
...
assert.equal(evidence.externalChecks.vercelPreview.statusAtCertification, 'RATE_LIMITED')
assert.equal(evidence.externalChecks.vercelPreview.previouslyPassed, true)
assert.equal(evidence.externalChecks.vercelPreview.applicationSourceChangedSinceLastPass, false)
