...
const listing = {
  ...
  sellerCanonicalFacts: {
    ...
    landlordType: 'trust',
    addressProfile: {
      exactAddressVisibility: 'complex_only',
    },
    propertyProfile: {
      garages: 0,
      coveredParking: 1,
    ...
    rentalInfo: {
      monthlyRent: 18500,
      depositAmount: 37000,
      depositRequirement: 'Two months deposit',
      depositMultiplier: 2,
...
assert.equal(form.depositRequirement, 'Two months deposit')
assert.equal(form.occupationDate, '2026-09-03')
assert.equal(form.garages, '0')
assert.equal(form.coveredParking, '1')
assert.equal(form.pool, 'yes')
assert.equal(form.garden, 'no')
assert.equal(form.furnishedStatus, 'semi_furnished')
assert.equal(form.inspectionNotes, 'Access via security.')
assert.equal(form.mandateEndDate, '2026-12-31')
...
const editedForm = {
  ...
  monthlyRent: '19500',
  depositAmount: '39000',
  rentalExcludes: 'Water and prepaid electricity',
...
assert.equal(payload.sellerCanonicalFacts.rentalInfo.rentalExcludes, 'Water and prepaid electricity')
assert.equal(payload.sellerCanonicalFacts.propertyProfile.portalFeatures.pool, true)
...
assert.equal(publication.portalFeatures.prepaidElectricity, true)
