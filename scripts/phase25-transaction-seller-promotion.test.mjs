...
assert.equal(manifest.rows.length, 71)
assert.equal(readiness.manifestRowCount, 71)
assert.equal(readiness.stagingLedgerRecordedCount, 71)
assert.equal(readiness.certificationStatus, 'STAGING_CERTIFIED')
assert.equal(closeout.rows.filter((row) => row.productionLedgerRecorded === true).length, 59)
assert.equal(new Set(closeout.rows.map((row) => row.version)).size, closeout.rows.length)
assert.equal(scope.productionBaseline.reviewedProductionPromotions, 59)
assert.equal(scope.productionBaseline.outstandingManifestRows, 12)
assert.equal(scope.productionBaseline.productionLedgerRows, 492)
