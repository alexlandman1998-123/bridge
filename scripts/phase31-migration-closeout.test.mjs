...
assert.equal(closeout.rows.filter((row) => row.productionLedgerRecorded === true).length, 71)
assert.equal(new Set(closeout.rows.map((row) => row.version)).size, 71)
assert.equal(scope.status, 'PHASE_1_SCOPE_AMENDED_PHASE_31')
assert.equal(scope.productionBaseline.reviewedProductionPromotions, 71)
assert.equal(scope.productionBaseline.outstandingManifestRows, 0)
assert.equal(scope.productionBaseline.productionLedgerRows, 504)
