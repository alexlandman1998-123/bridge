...
assert.ok(closeout.rows.filter((row) => row.productionLedgerRecorded === true).length >= 56)
assert.equal(new Set(closeout.rows.map((row) => row.version)).size, closeout.rows.length)
assert.ok(scope.productionBaseline.reviewedProductionPromotions >= 56)
assert.ok(scope.productionBaseline.outstandingManifestRows <= 14)
assert.ok(scope.productionBaseline.productionLedgerRows >= 489)
