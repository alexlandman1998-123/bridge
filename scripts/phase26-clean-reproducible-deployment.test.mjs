...
assert.equal(fingerprint(recertification.repository.productionSourceCommit), recertification.repository.runtimeBuildInputFingerprint)
assert.equal(fullFingerprint(recertification.repository.productionSourceCommit), recertification.repository.fullRuntimeBuildInputFingerprint)
...
assert.equal(fingerprint('HEAD'), clearance.releaseCandidate.runtimeBuildInputFingerprint)
assert.equal(fullFingerprint('HEAD'), clearance.releaseCandidate.fullRuntimeBuildInputFingerprint)
assert.equal(git(['rev-parse', 'HEAD:the-it-guy/src']), clearance.releaseCandidate.runtimeSourceTree)
assert.equal(clearance.releaseCandidate.build.freshLockfileInstall, true)
assert.equal(clearance.releaseCandidate.build.applicationTestSuite, '9_of_9_pass')
assert.equal(clearance.releaseCandidate.build.guardedBuild, 'pass')
assert.equal(clearance.releaseCandidate.build.modulesTransformed, 3490)
assert.equal(clearance.releaseCandidate.build.criticalAssetCount, 427)
assert.equal(clearance.releaseCandidate.build.performanceBudget, 'pass')
assert.equal(clearance.releaseCandidate.promotedToProduction, false)

console.log('Phase 26 passed through Phase 39: the PR candidate is clean and reproducible while production remains on its certified baseline.')
