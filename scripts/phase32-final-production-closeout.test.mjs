...
assert.equal(evidence.rows.length, 78)
assert.equal(new Set(manifest.rows.map((row) => row.version)).size, 78)
assert.equal(new Set(evidence.rows.map((row) => row.version)).size, 78)
...
assert.equal(readiness.manifestRowCount, 78)
assert.equal(readiness.stagingLedgerRecordedCount, 78)
...
assert.equal(scope.productionBaseline.manifestRows, 78)
assert.equal(scope.productionBaseline.reviewedProductionPromotions, 78)
