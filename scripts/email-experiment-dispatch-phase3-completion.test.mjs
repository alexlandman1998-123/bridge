...
for (const item of ['email_campaign_experiment_allocate', 'email_campaign_experiment_decide', 'p_variants: experiment?.status === "running" ? ["control", "variant"] : null', 'variant_subject', 'arch9_experiment_variant']) assert.ok(worker.includes(item), `Missing worker experiment contract: ${item}`)
