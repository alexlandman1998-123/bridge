...
assert.equal(result.manifestRowCount, 64)
assert.equal(result.duplicateVersions.length, 0)
assert.equal(result.missingManifestFiles.length, 0)
assert.equal(result.phase7Readiness.status, 'BLOCKED_ATTORNEY_INTEGRITY')
assert.equal(result.phase7Readiness.ready, false)
assert.equal(result.phase7Readiness.attorneyIntegrityBlockingAssignments, 43)
